...
**How.** Same as A. This arm additionally passes `CHUNK=65536` explicitly, so the
default and the explicit value agree — belt and braces, because getting this
wrong is silent.
...
**Context.** Hit on the *other* arm (arm A), not this one — this arm is kv-aware
and always passed the flag. Recorded here because `scripts/ab_router.sh` in this
kit is the **post-fix** version, and a reader diffing it against the transcript
would otherwise find an unexplained change. No `.diff` file: the script is short
and shipped verbatim.
