...
# Match pd_leg_spur.sh: the aiter custom all-reduce deadlocks on gfx950 under
# concurrency. Kept independent of MTP so mix and PD arms are comparable.
CAR_ARGS=()
...
HIP_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m sglang.launch_server \
  ...
  "${DP_ARGS[@]}" "${MTP_ARGS[@]}" "${CAR_ARGS[@]}" ${EXTRA_ARGS:-} > "$LOG" 2>&1
