...
: "${CONTAINER_PREFIX:=glm52-pd-yihou-1p1d}"
...
# uses CONC=32. Concurrency is not a config knob -- agentx_bench.sh takes it as
# a required CONC=<n> argument -- so it is recorded here only as intent.

# Inherit the rest of the validated baseline: DPA on, decode MTP, HiCache off,
# max-running and graph max BS 64, index_share_for_mtp_iteration=false.
