...
: "${CONTAINER_PREFIX:=glm52-pd-yihou-fused}"                    # every container carries "yihou" and the shape, so a stray one is attributable and safe under the deletion rule
...
# --- PATH B: fused DSA indexer back ON, accept the crash cadence --------------
# Removes the workaround, restoring the configuration that decodes correctly but
# faults roughly every ~90 minutes under load (issue.md 3.3). Intended to be run
# one sweep point at a time with a relaunch between points.
DECODE_EXTRA_ENV=""
