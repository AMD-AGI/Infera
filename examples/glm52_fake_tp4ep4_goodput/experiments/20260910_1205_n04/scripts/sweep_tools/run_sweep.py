...
def main():
    ...
    if min(args.concurrencies) < 1 or max(args.concurrencies) > 64:
        parser.error("concurrencies must be 1..64")
    ...
    if info["max_running_requests"] != 64 or info["enable_profile_cuda_graph"]:
