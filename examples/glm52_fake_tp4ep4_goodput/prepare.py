...
def main():
    ...
    parser.add_argument("--graph-batch-sizes", type=int, nargs="+", default=None)
    ...
    if not 0 < args.mem_fraction < 1:
        parser.error("mem-fraction must be between zero and one")
    graph_batches = sorted(set(args.graph_batch_sizes or (
        [b for b in GRAPH_BATCHES if b <= args.max_running_requests] + [args.max_running_requests])))
    if min(graph_batches) < 1 or max(graph_batches) != args.max_running_requests:
        parser.error("Graph batches must be positive, within capacity, and include maximum capacity")
    ...
    text = replace_once(text, 'MEM=${3:-0.85}', f'MEM=${{3:-{args.mem_fraction}}}')
    ...
    state = {"image": IMAGE, "archived_image": ARCHIVED_IMAGE, "exact_image_reproduction": False,
             ...
             "max_running_requests": args.max_running_requests, "profiling": False, "packet_capture": "runtime default (no override)",
    ...
    print(f"Prepared unprofiled sweep: bash {launch} sim server {args.mem_fraction} {args.max_running_requests}")
