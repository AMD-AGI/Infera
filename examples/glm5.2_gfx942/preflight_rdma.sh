...
# RDMA preflight. Run on the host of each node BEFORE the PD bring-up: cross-node
# PD moves the KV cache over the fabric on every request, and a container whose
# RDMA provider does not match the host driver falls back to TCP silently -- the
# pair still answers, just slower than a single node.
#
# With DUMP_PATH set and one task per node it also runs infera's cross-node
# netperf and Mooncake checks.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
source "$HERE/env.sh"

echo "[preflight] active RDMA ports visible in $IMAGE (expect the node's port count, not 0):"
IONIC_ARGS=()
