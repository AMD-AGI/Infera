...
# script here.
#
# Do not edit this file to adapt to a cluster. Put the cluster's values in
# cluster.env, which this reads first -- see cluster.env.example.
#
...
# Your cluster's values live in cluster.env (start from cluster.env.example) and
# are read first, so that every ${VAR:-default} below defers to them. Sourced
# here rather than left to the caller because forgetting it is not an error you
# would notice: the placeholders resolve, the engines start, and the first thing
# that fails is the KV hand-off twenty minutes later.
# Sourcing it yourself as well remains harmless -- the exports are the same.
# shellcheck source=/dev/null
[[ -f "$EXAMPLE_DIR/cluster.env" ]] && source "$EXAMPLE_DIR/cluster.env"
...
require_ips() {
  ...
  (( bad == 0 )) || {
    echo "[env] set them in cluster.env (cp cluster.env.example cluster.env), or" >&2
    echo "[env] point PREFILL_NODE/DECODE_NODE at names that resolve" >&2
    exit 1
  }
}
...
export DATA_DIR="${DATA_DIR:-${EXAMPLE_DIR}/data}"  # holds the agentic trace, mounted rw
...
# --- bench sizing -----------------------------------------------------------
# Deliberately small: bench.sh sizes the deployment, it is not a sweep. See §5.
export ISL="${ISL:-4096}"
export OSL="${OSL:-1024}"
export CONC="${CONC:-16}"

# NUM_PROMPTS is deliberately NOT defaulted here, for two reasons. It means
# different things to the two benches -- requests to bench.sh, CONVERSATIONS to
# run_agentic_trace.sh -- so one default cannot serve both. And a default derived
# from CONC would be evaluated once, at source time, so raising CONC later in the
# same shell would silently keep the old count. Each bench derives its own.
export NUM_PROMPTS="${NUM_PROMPTS:-}"

# --- agentic trace ----------------------------------------------------------
# The multi-turn workload the recipe was actually tuned on, built by
# weka_to_agentic_trace.py and replayed by run_agentic_trace.sh. Unlike bench.sh's
# random dataset it has real prefix reuse, so it is the only one of the two that
# can say anything about the kv-aware router or the cache. See README §5.
#
# OUTPUT_LEN must be the value the dataset was BUILT with. The converter sizes
# each turn's filler with the reply length baked in, so a mismatch drifts every
# turn's length and quietly invalidates the scorer's ideal.
export TRACE="${TRACE:-${DATA_DIR}/cc_traces_100k.json}"
export OUTPUT_LEN="${OUTPUT_LEN:-220}"

mkdir -p "$LOG_DIR" "$RESULT_DIR" "$DATA_DIR" 2>/dev/null || true
