...
| `KVD_IO_MODE=auto` default | `--io-mode direct` | what the docker recipe actually ran. `auto`'s classifier walks the mount to the block device and, from inside a container, ends at a `/dev/mapper` node it cannot see — so it takes the conservative branch and picks buffered even on NVMe |
