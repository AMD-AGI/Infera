...
# what: throughput sweep against the router (:8000, sglang-oai backend).
...
CTR="${CTR:-dsv4_sgl_mix}"; NODE_IP="${NODE_IP:-127.0.0.1}"
ISL="${ISL:-8192}"; OSL="${OSL:-1024}"; CONCS="${*:-64}"; OUT="${OUT:-/tmp/mix_sgl_out}"
docker exec "$CTR" mkdir -p "$OUT"
for C in $CONCS; do
  docker exec "$CTR" bash -lc "python3 -m sglang.bench_serving --backend sglang-oai \
    ...
    --request-rate inf --output-file $OUT/mix_c${C}.jsonl 2>&1 | tail -20"
done
log "bench done -> in-container $OUT/*.jsonl"
