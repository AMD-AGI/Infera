...
# what: one-shot atom mix bring-up: container -> etcd -> router -> worker -> smoke.
...
CTR="${CTR:-dsv4_atom_mix}"; NODE_IP="${NODE_IP:-127.0.0.1}"; export CTR NODE_IP
start_container "$CTR"
start_etcd repro-etcd "$NODE_IP"
start_router "$CTR" "$NODE_IP"
bash "$DIR/engine.sh"
smoke "http://$NODE_IP:$ROUTER_PORT"
log "atom mix ready on http://$NODE_IP:$ROUTER_PORT (bench: bash $DIR/bench.sh <conc>)"
