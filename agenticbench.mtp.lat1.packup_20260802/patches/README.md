...
and their refutations are in `../notes/notes.config.CASEA.md` (carried from the Case A kit).
...
## Applicability to the lat1 run (appended 2026-08-02)

All three artifacts were **already in effect** for lat1 and none was re-applied:
lat1 ran against the same two legs Case A used, 21 h into the same containers,
without a restart. Verified live immediately before the run:

* 0001 — in the image both legs run; the bytecode gate passed at bring-up.
* 0002 — `--mem-fraction-static 0.80` still on the prefill leg (it is baked into
  `../scripts/glm52_leg_spur_mtp.sh`, not passed at launch).
* 0003 — `GLM52_P1V3` still loaded in the decode container:
  `inspect.getsource(dsa_indexer).count('GLM52_P1V3')` = **3**.

A cold reproducer of lat1 must apply all three exactly as documented above.
The lat1 run itself added **no new patch** — its own two defects
(cache contamination via a shared `random_seed`, and a numpy seed-range error)
were **configuration** faults, fixed in `../spec/lat1_full.yaml` and written up in
`../notes/notes.lat1.md`. Nothing in the engine or the driver was modified.
