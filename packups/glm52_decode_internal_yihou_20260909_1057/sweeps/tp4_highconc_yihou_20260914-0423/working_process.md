...
## 2026-09-14T13:30Z — a peer started GPU work on node 254; deliberately not touched

A new foreign container appeared on `crsuse2-m2m-254`: `baseix_dsv4`
(`harbor.crusoe.primus-safe.amd.com/hyperloom-image/sglang:v0.5.17-rocm720-mi35x-profilerfix`, up
19 minutes), running `/shared_nfs/zihao/arena/deep/BASEIX-dsv4/run_tl.sh` and
`/shared_nfs/zihao/arena/deep_bench.sh`. 11 KFD processes. `squeue` shows **zihaoan2 holding
allocation 136568 on 254**, concurrent with our 136670.

**Not killed.** The standing rule to clear GPU-heavy foreign processes is aimed at residuals; this is
a peer actively running a benchmark under their own allocation, and the same judgement was applied
(and confirmed by the user) for the `dsv4` container on 267 earlier today. Killing it would destroy
someone's running work.

It also costs us nothing: this sweep finished at 07:45Z, every point is measured and packed, and our
container `yihou-hc-254-0914` holds **no** GPU processes and no VRAM, so we are neither blocked by
the peer nor blocking them.

Node 267 remains idle (0 KFD, no foreign containers). Allocations 136669 / 136670 have ~1 h 49 m
left and were not cancelled, per the standing rules.

## 2026-09-15 — consolidated cross-campaign bench report written

Wrote `../../results/glm52-mxfp4_internal-decode_tp4_dpa-on_isl70000_osl10000_accept3.61_eagle-s5d6k1_fp8kv_c4-c128_ep1-vs-ep4_yihou_20260915.md`,
covering every gate-passing internal-method result from all four campaigns (2026-09-09 .. 09-14),
not just this sweep.

**Placement (workspace-isolation rule):** it sits in the experiment root's `results/`, not in this
sweep's workspace, because it spans four campaigns and belongs to none of them exclusively -- the
same reasoning used for the packup. No temporary experimental activity occurred outside this
workspace; this is a finished document.

All 37 quoted values were re-extracted from the source `result_yihou.json` files and string-matched
against the finished document; the `tok/s = C x 1000 / TPOT` identity was re-verified at C=4 and
C=128. Deliberately excluded, each with the reason stated in the document: the TP8 reference point
(its EP/DPA state was never recorded, so it cannot be read as TP8-vs-TP4) and the
`comparisons/` ISL10000/OSL500 baselines (different workload).

A CUDA-graph on/off table was **not** produced. The only graph-off run in the project
(`iterations/002_eager_smoke_yihou`) used ISL 1024, `max_steps 2` and `warmup_steps 0`, so its 99.5 s
for two iterations is dominated by first-call compilation, and its graph-on neighbour used a
different `max_steps`, `warmup_steps` and OSL. Comparing them would yield a ~2000x figure that is an
artefact. Recorded in the document as required follow-up work with the conditions for a valid A/B.
