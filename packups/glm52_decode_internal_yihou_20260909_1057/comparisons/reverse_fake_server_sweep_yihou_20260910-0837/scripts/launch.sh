...
MEM=${3:-0.85}
CONC=${4:-16}
NAME=${NAME:-yihou-glm52-reverse-20260910}
...
docker run --name "$NAME-probe" --device=/dev/kfd --device=/dev/dri \
...
GRAPH_BS=$CONC
if [[ ${DPA:-off} == on ]]; then
    EXTRA+=(--dp-size 4 --enable-dp-attention)
    GRAPH_BS=$((CONC / 4))
fi
ENV=()
...
CMD=(python3 -m sglang.launch_server
    ...
    --max-running-requests "$CONC" --cuda-graph-max-bs "$GRAPH_BS"
...
docker run -d --name "$NAME" --label experiment=glm52-faked-1016 -w / \
    ...
    -e HIP_VISIBLE_DEVICES=0,1,2,3 -e PYTHONNOUSERSITE=1 -e PYTHONDONTWRITEBYTECODE=1 \
