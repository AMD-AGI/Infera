...
# kernel path: /torch-cache/sd/csdtqeqknaesbpn3xlfo2uab4gtw5un5kcxmaxtwx7adiwbdhzyi.py
...
#   view => view
# Graph fragment:
#   %arg2_1 : Tensor "f32[s97, 1][1, 1]cuda:2" = PlaceHolder[target=arg2_1]
#   %arg3_1 : Tensor "f32[s97, 1][1, 1]cuda:2" = PlaceHolder[target=arg3_1]
#   %mul_3 : Tensor "f32[s97, 1, 1][1, 1, 1]cuda:2" = PlaceHolder[target=mul_3]
#   %unsqueeze : Tensor "f32[s97, 1, 1][1, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg2_1, 2), kwargs = {})
#   %view : Tensor "f32[s97, 1, 1][1, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%arg3_1, [-1, 1, 1]), kwargs = {})
#   %mul_3 : Tensor "f32[s97, 1, 1][1, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %view), kwargs = {})
#   %view_1 : Tensor "f32[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg1_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %getitem : Tensor "f32[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=operator.getitem](args = (%max_1, 0), kwargs = {})
...
# kernel path: /torch-cache/vi/cvihddkyacsvv3k35zni3zdgfi5ydx5bvczqmfrqyjjquebh2yzj.py
...
#   max_1 => max_1
# Graph fragment:
#   %arg4_1 : Tensor "i64[s97, 1][1, 1]cuda:2" = PlaceHolder[target=arg4_1]
#   %view_1 : Tensor "f32[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg1_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %gather : Tensor "i64[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%arg4_1, 1, %getitem_1), kwargs = {})
#   return %gather
...
# kernel path: /torch-cache/rr/crra3yxxbr3uk4rq6zojiza42kqj4be4eehlj7mgxkoxuj5aawhq.py
...
# Graph fragment:
#   %arg6_1 : Tensor "bf16[s97, s87][s87, 1]cuda:2" = PlaceHolder[target=arg6_1]
#   %view_1 : Tensor "f32[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg1_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %view_4 : Tensor "i64[s97][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%getitem_1, [%arg1_1]), kwargs = {})
#   %div : Tensor "i64[s97][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor_mode](args = (%view_4, 1), kwargs = {rounding_mode: floor})
#   %iota : Tensor "i64[s97][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (%arg1_1,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:2, requires_grad: False})
#   %unsqueeze_1 : Tensor "i64[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%iota, 1), kwargs = {})
#   %view_5 : Tensor "i64[s97][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%expand, [%arg1_1]), kwargs = {})
#   %add_47 : Tensor "i64[s97][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%div, %view_5), kwargs = {})
#   %index : Tensor "bf16[s97, s87][s87, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg6_1, [%add_47]), kwargs = {})
#   return %index
...
# kernel path: /torch-cache/z4/cz4xxgux7m2vnrxamkjcxq3sjtb3mke6ppdquvkwct5jl6k4cjoa.py
...
#   %view_1 : Tensor "f32[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg1_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %add_53 : Tensor "i64[s97, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%getitem_1, %arg7_1), kwargs = {})
#   return %add_53
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_max_mul_unsqueeze_view_0.run(arg2_1, arg3_1, buf0, buf1, s97, stream=stream2)
            del arg2_1
            del arg3_1
            buf2 = empty_strided_cuda((s97, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, gather], Original ATen: [aten.view, aten.max, aten.gather]
            stream2 = get_raw_stream(2)
            triton_poi_fused_gather_max_view_1.run(arg4_1, buf2, s97, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2.run(arg6_1, buf3, triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2_xnumel, stream=stream2)
            del arg6_1
            buf4 = empty_strided_cuda((s97, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, add_2], Original ATen: [aten.view, aten.max, aten.add]
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_max_view_3.run(buf4, s77, s97, stream=stream2)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = 1
    arg1_1 = 16
    arg2_1 = rand_strided((16, 1), (1, 1), device='cuda:2', dtype=torch.float32)
    arg3_1 = rand_strided((16, 1), (1, 1), device='cuda:2', dtype=torch.float32)
    arg4_1 = rand_strided((16, 1), (1, 1), device='cuda:2', dtype=torch.int64)
    arg5_1 = 6144
    arg6_1 = rand_strided((16, 6144), (6144, 1), device='cuda:2', dtype=torch.bfloat16)
    arg7_1 = 1
