...
# kernel path: /torch-cache/dn/cdn6h4klsysz57lcjukpigsdx7v7quyfu7jqtjtr3glsdgf6v5w4.py
...
#   %mul_1 : Tensor "f32[1, 1, 1][1, 1, 1]cuda:0" = PlaceHolder[target=mul_1]
...
#   %mul_1 : Tensor "f32[1, 1, 1][1, 1, 1]cuda:0"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %view), kwargs = {})
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
...
# kernel path: /torch-cache/sy/csyi54jqsmmg3byq74itd2frontvo4cwverj6qhmtverazlala5u.py
...
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %gather : Tensor "i64[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%arg3_1, 1, %getitem_1), kwargs = {})
#   return %gather
...
# kernel path: /torch-cache/g4/cg4wxar342dfb6wgjvsfks3erha2xjn74cvjm7pdfzb4dazk7vpp.py
...
# Graph fragment:
#   %arg5_1 : Tensor "bf16[1, s98][s98, 1]cuda:0" = PlaceHolder[target=arg5_1]
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
...
#   %index : Tensor "bf16[1, s98][s98, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg5_1, [%add]), kwargs = {})
#   return %index
...
# kernel path: /torch-cache/5t/c5t3eyhfu4d6jonandrzvpuj6y5fcwmoxo6mnqmbzscfdu4cgneu.py
...
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %add_3 : Tensor "i64[1, 1][1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%getitem_1, %arg6_1), kwargs = {})
#   return %add_3
...
@triton_heuristics.pointwise(
    size_hints={'x': 1}, 
    filename=__file__,
    triton_meta={'signature': {'out_ptr0': '*i64', 'ks0': 'i64', 'xnumel': 'constexpr', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=0, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {'xnumel': 1}, 'configs': [{(0,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(0):
            ...
            # Topologically Sorted Source Nodes: [flatten, max_1, gather], Original ATen: [aten.view, aten.max, aten.gather]
            stream0 = get_raw_stream(0)
            triton_poi_fused_gather_max_view_1.run(arg3_1, buf2, 1, stream=stream0)
            buf3 = empty_strided_cuda((1, s98), (s98, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [flatten, max_1, flat_cs, floordiv, batch_offsets, repeat_interleave, selected_input_index, hidden_states], Original ATen: [aten.view, aten.max, aten.floor_divide, aten.arange, aten.unsqueeze, aten.add, aten.index]
            stream0 = get_raw_stream(0)
            triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2.run(arg5_1, buf3, s98, stream=stream0)
            del arg5_1
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg4_1 = 6144
    arg5_1 = rand_strided((1, 6144), (6144, 1), device='cuda:0', dtype=torch.bfloat16)
    arg6_1 = 1
