...
# kernel path: /torch-cache/nb/cnbhbl54cg6szkdnkyz776ikznj5daohevcuo33hvddztkt7bxe5.py
...
#   view => view
# Graph fragment:
#   %arg2_1 : Tensor "f32[s0, 1][1, 1]cuda:1" = PlaceHolder[target=arg2_1]
#   %arg4_1 : Tensor "f32[s0, 1][1, 1]cuda:1" = PlaceHolder[target=arg4_1]
#   %mul_3 : Tensor "f32[s0, 1, 1][1, 1, 1]cuda:1" = PlaceHolder[target=mul_3]
#   %unsqueeze : Tensor "f32[s0, 1, 1][1, 1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg2_1, 2), kwargs = {})
#   %view : Tensor "f32[s0, 1, 1][1, 1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%arg4_1, [-1, 1, 1]), kwargs = {})
#   %mul_3 : Tensor "f32[s0, 1, 1][1, 1, 1]cuda:1"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %view), kwargs = {})
#   %view_1 : Tensor "f32[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg3_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %getitem : Tensor "f32[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=operator.getitem](args = (%max_1, 0), kwargs = {})
...
@triton_heuristics.pointwise(
    size_hints={'x': 16}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'out_ptr0': '*fp32', 'out_ptr1': '*fp32', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=1, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {}, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]]}]},
...
# kernel path: /torch-cache/kw/ckwmvmkqk7xnpj5d3ycqiyun6sfouoknfyc3gditfkrlbo4a6fvv.py
...
#   max_1 => max_1
# Graph fragment:
#   %arg5_1 : Tensor "i64[s0, 1][1, 1]cuda:1" = PlaceHolder[target=arg5_1]
#   %view_1 : Tensor "f32[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg3_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %gather : Tensor "i64[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%arg5_1, 1, %getitem_1), kwargs = {})
#   return %gather
...
# kernel path: /torch-cache/hd/chdcazwy43736ponwhwjgpfm4u72ufox3gwm6ifvxdtdethkuvhs.py
...
# Graph fragment:
#   %arg7_1 : Tensor "bf16[s0, s87][s87, 1]cuda:1" = PlaceHolder[target=arg7_1]
#   %view_1 : Tensor "f32[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg3_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %view_4 : Tensor "i64[s0][1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%getitem_1, [%arg3_1]), kwargs = {})
#   %div : Tensor "i64[s0][1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor_mode](args = (%view_4, 1), kwargs = {rounding_mode: floor})
#   %iota : Tensor "i64[s0][1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (%arg3_1,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:1, requires_grad: False})
#   %unsqueeze_1 : Tensor "i64[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%iota, 1), kwargs = {})
#   %view_5 : Tensor "i64[s0][1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%expand, [%arg3_1]), kwargs = {})
#   %add_47 : Tensor "i64[s0][1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%div, %view_5), kwargs = {})
#   %index : Tensor "bf16[s0, s87][s87, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg7_1, [%add_47]), kwargs = {})
#   return %index
...
# kernel path: /torch-cache/c2/cc2cr56wlzckhyvo4sadrmqfatv54qisnfdqgqve5sf73szexh3q.py
...
#   %view_1 : Tensor "f32[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_3, [%arg3_1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %add_53 : Tensor "i64[s0, 1][1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%getitem_1, %arg8_1), kwargs = {})
#   return %add_53
...
@triton_heuristics.pointwise(
    size_hints={'x': 16}, 
    filename=__file__,
    triton_meta={'signature': {'out_ptr0': '*i64', 'ks0': 'i64', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=1, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {}, 'configs': [{(0,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            triton_poi_fused_max_mul_unsqueeze_view_0.run(arg2_1, arg4_1, buf0, buf1, s0, stream=stream1)
            del arg2_1
            del arg4_1
            buf2 = empty_strided_cuda((s0, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, gather], Original ATen: [aten.view, aten.max, aten.gather]
            stream1 = get_raw_stream(1)
            triton_poi_fused_gather_max_view_1.run(arg5_1, buf2, s0, stream=stream1)
            ...
            stream1 = get_raw_stream(1)
            triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2.run(arg7_1, buf3, triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2_xnumel, stream=stream1)
            del arg7_1
            buf4 = empty_strided_cuda((s0, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, add_2], Original ATen: [aten.view, aten.max, aten.add]
            stream1 = get_raw_stream(1)
            triton_poi_fused_add_max_view_3.run(buf4, s77, s0, stream=stream1)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = 1
    arg1_1 = 16
    arg2_1 = rand_strided((16, 1), (1, 1), device='cuda:1', dtype=torch.float32)
    arg3_1 = 16
    arg4_1 = rand_strided((16, 1), (1, 1), device='cuda:1', dtype=torch.float32)
    arg5_1 = rand_strided((16, 1), (1, 1), device='cuda:1', dtype=torch.int64)
    arg6_1 = 6144
    arg7_1 = rand_strided((16, 6144), (6144, 1), device='cuda:1', dtype=torch.bfloat16)
    arg8_1 = 2
