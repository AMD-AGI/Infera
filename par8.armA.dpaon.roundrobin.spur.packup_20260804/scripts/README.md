...
| `collect_env.sh` | 11 | environment snapshot — **run on both live nodes**; output in `../env/` |

> Invoke it as `spur exec <job> bash -c "bash <abs-path>/collect_env.sh"`. Neither
> `spur exec <job> bash -s < collect_env.sh` (stdin is not forwarded — produces an
> empty file) nor `spur exec <job> "bash …/collect_env.sh"` (no shell parsing —
> `nsenter: failed to execute`) works. Both were tried here.
