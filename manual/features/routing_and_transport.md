...
|---|---|---|---|
| `--nats-req-idle-timeout` | `INFERA_NATS_REQ_IDLE_TIMEOUT` | `0` (off) | max wait for the *next* reply chunk (reset per chunk). Expiry → 504 + cancel. |
...
| `INFERA_NATS_REQ_IDLE_TIMEOUT` | `0` (off) | Max wait for the next reply chunk; expiry → 504 + cancel. |
