...
def _find_primus_root(start: Path) -> Path:
    ...
    cur = start.parent
    for _ in range(10):
        if (cur / "configs" / "models").is_dir():
            return cur
        if cur.parent == cur:
            break
        cur = cur.parent
    # First-class-in-Infera fallback: this file lives at
    # infera/projection/agents/tuning_agent/workload.py, so parents[2] ==
    # infera/projection, which contains configs/models. Lets tuning run with
    # zero env setup.
    project_root = Path(__file__).resolve().parents[2]
    if (project_root / "configs" / "models").is_dir():
        return project_root
    return Path.cwd()
