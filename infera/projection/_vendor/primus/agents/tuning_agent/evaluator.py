...
def write_trial_yaml(arch: ArchitectureRecord, cfg: TrialConfig, out_dir: Path, tag: str) -> Path:
    ...
    if cfg.pp_schedule and cfg.pp_schedule != "auto":
        from infera.projection._vendor.primus.core.projection.config_validation import (
...
def _ladder_anchor_from_cache(
    ...
    """
    try:
        from infera.projection._vendor.primus.core.projection.inference_projection.regime import confidence_ladder
    except Exception:
        return None
...
class Evaluator:
    ...
    * ``"full"`` — call ``projection memory`` and ``projection performance``
    ...
    def _select_regime_base_anchor(self, cfg, cache: Path, ep: int) -> Path | None:
        ...
        from infera.projection._vendor.primus.core.projection.inference_projection.regime import select_anchor
