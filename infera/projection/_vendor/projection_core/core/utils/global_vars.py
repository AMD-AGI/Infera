...
def _set_target_platform(cfg: PrimusConfig):
    ...
    if platform_config.name and platform_config.name != "local":
        from infera.projection._vendor.projection_core.platforms import RemotePlatform

        _GLOBAL_TARGET_PLATFORM = RemotePlatform(platform_config.name)
    else:
        from infera.projection._vendor.projection_core.platforms import LocalPlatform
