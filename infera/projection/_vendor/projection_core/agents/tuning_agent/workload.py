...
def resolve_workload(workload_yaml: Path, primus_root: Path | None = None) -> ArchitectureRecord:
    ...
    model_path = primus_root / "projection_core" / "configs" / "models" / framework / model_filename
...
def _find_primus_root(start: Path) -> Path:
    ...
    if env_root:
        p = Path(env_root)
        if (p / "projection_core" / "configs" / "models").is_dir():
            return p
    cur = start.parent
    for _ in range(10):
        if (cur / "projection_core" / "configs" / "models").is_dir():
            return cur
        if cur.parent == cur:
            break
        cur = cur.parent
    ...
    if (vendored_root / "projection_core" / "configs" / "models").is_dir():
