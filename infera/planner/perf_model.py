...
class DecodePerfModel:
    ...
    def find_best_throughput_per_gpu(
        ...
        point. The search walks the KV-usage axis from most loaded to least and
        takes the first point within budget, rather than bisecting: interpolated
        ITL is not guaranteed monotonic in KV usage, so a bisection could settle
        on a dip and overstate capacity.
        ...
        for i in range(self._kv_search.size - 1, -1, -1):
            if itl_curve[i] <= itl_ms:
                return float(thpt_curve[i]), float(itl_curve[i]), float(self._kv_search[i])
        return float(thpt_curve[0]), float(itl_curve[0]), float(self._kv_search[0])
