...
###############################################################################
"""Loading and validation of pre-deployment profiling results.
...
KV cache fills. That comes from an offline sweep, handed to the planner as a
single JSON file::
...
MISSING_PROFILE_HELP = (
    "The SLA planner needs pre-deployment profiling results. Run a sweep over "
    "ISL (prefill) and concurrency x context length (decode) and write the "
    "measurements to a JSON file; see the SLA planner guide in manual/ for the "
...
def parse_profile_data(raw: dict) -> ProfileData:
    ...
    return ProfileData(
        ...
        prefill_engine_num_gpu=max(1, int(raw.get("prefill_engine_num_gpu", 1))),
        decode_engine_num_gpu=max(1, int(raw.get("decode_engine_num_gpu", 1))),
    )
