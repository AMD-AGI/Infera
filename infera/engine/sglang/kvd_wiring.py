...
def _finish_wiring(args: Any, socket_path: str) -> None:
    ...
    # finding one is normal rather than a problem: on SGLang bases where the
    # threshold lives in the backend's extra config instead of ServerArgs,
    ...
    if not overridden:
        logger.info(
            ...
            'carrying {"prefetch_threshold": 64} in the backend extra config below '
            "instead, which is the copy the engine subprocess reads.",
