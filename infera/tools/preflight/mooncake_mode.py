...
def _routable_gid(dev: str) -> tuple[int | None, str | None, str | None]:
    ...
    for gi in gid_files:
        ...
        if "v2" not in gtype.lower() and "RoCE v2" not in gtype:
            # Only RoCE v2 is routable for KV transfer; v1 GIDs are not usable.
            continue
...
def probe_pci_p2pdma() -> str | None:
    ...
    P2PDMA is load-bearing for no-ODP dma-buf: such a NIC's dma-buf session is
    stable only when the kernel has P2PDMA compiled in (isKernelDmabufSupported()
    returns true). Observed both ways on otherwise-identical ionic hardware -- a
    kernel without it dies after the first transfer, one with it sustained a
    conc=128 bench. So detecting this correctly gates whether Mode C (capped
    no-ODP dma-buf) is viable or a blocker.
    """
...
def _mooncake_pkg_dir() -> str | None:
    """The installed mooncake package directory, via import (no filesystem guess).

    ``os.path.dirname(mooncake.__file__)`` is the authoritative location of the
    package whose engine the running sglang/vLLM will actually load -- far more
    robust than globbing /usr for a *.so and hoping we pick the right one. Returns
    None if mooncake isn't importable in this interpreter (e.g. a bare host)."""
    try:
    ...
    if spec is None:
        return None
    # a package spec exposes its dir via submodule_search_locations; fall back to
    # the origin file's dir for a single-file module layout.
...
def probe_dmabuf_engine() -> dict:
    ...
    locate the engine .so by IMPORTING mooncake (the same package sglang loads) and
    globbing *inside its package dir only* -- not a slow, ambiguous recursive walk
    of /usr. Then nm-grep for the external symbol ibv_reg_dmabuf_mr (a strings grep
    is always empty -- it is a call symbol, not a literal). Unknown if mooncake
    isn't importable or nm is unavailable."""
    if not have("nm"):
        return {"compiled_in": None, "reason": "nm not available"}
    pkg = _mooncake_pkg_dir()
    if pkg is None:
        return {"compiled_in": None, "reason": "mooncake not importable"}
    # engine.*.so is the transfer engine; scan every .so in the package to be safe
    # (store.so/libasio.so are cheap to skip via the symbol grep).
    sos = sorted(set(glob.glob(f"{pkg}/**/*.so", recursive=True)))
    for so in sos:
    ...
    if sos:
        # prefer the engine .so as the representative "not compiled in" artifact
        engine = next((s for s in sos if "engine" in os.path.basename(s)), sos[0])
        return {"compiled_in": False, "so": engine}
    return {"compiled_in": None, "reason": f"no .so under {pkg}"}
...
# GID-index fallbacks, used ONLY when a NIC exposes no routable RoCE-v2 GID for us
# to auto-detect (the normal path uses the probed n["gid_index"]). These encode a
# common layout, not a hard rule: ionic typically puts its routable RoCE-v2 GID at
# index 1 and Mellanox mlx5 its IPv4-mapped one at index 3. A fabric that differs
# overrides via MC_GID_INDEX in the launch env.
_GID_FALLBACK_IONIC = 1  # bare ibv_reg_mr / no-ODP paths (modes A, C) are ionic-led
_GID_FALLBACK_MLX5 = 3  # the dma-buf-on-ODP path (mode B) is mlx5-led
...
# --------------------------------------------------------------------------- #
def nic_capability(
    nics: list[dict], peermem: dict, dmabuf_engine: dict, p2pdma: str | None
) -> list[dict]:
...
def _eval_mode_a(active: list[dict], peermem: dict) -> dict:
    ...
    gid = next((n["gid_index"] for n in active if n["gid_index"] is not None), _GID_FALLBACK_IONIC)
...
def _eval_mode_b(active: list[dict], peermem: dict, fast: list[dict], dmabuf_engine: dict) -> dict:
    ...
    pick = (
        sorted(odp_nics, key=lambda n: (-(n["link_gbps"] or 0), n["device"]))[0]
        if odp_nics
        else None
    )
    gid = pick["gid_index"] if pick and pick["gid_index"] is not None else _GID_FALLBACK_MLX5
    ...
    if not odp_nics:
        ...
        reason = (
            f"ODP NIC {pick['device']} ({pick['vendor']}, {pick['link_gbps']}G) -- no-pin dma-buf"
        )
    return {
        "mode": "B",
...
def _eval_mode_c(active: list[dict], p2pdma: str | None, dmabuf_engine: dict) -> dict:
    ...
    2x(pin) fit VRAM -- always a user-confirmed tradeoff. Without P2PDMA it is a
    dead end (the session dies after the 1st transfer)."""
    p2p = _p2pdma_present(p2pdma)
    engine_ok = dmabuf_engine.get("compiled_in") is not False
    warnings = list(_engine_warnings(dmabuf_engine))
    devices = [n["device"] for n in active]
        ...
        warnings.append(
            {
                ...
                "text": (
                    ...
                    f"(probed: {p2pdma or 'unknown'}) -- a no-ODP dma-buf session dies "
                    "after the first transfer. No safe KV path."
                ),
            }
        )
        ...
        warnings.append(
            {
                ...
                "text": (
                    ...
                    "so weights + 2x(pin) fit VRAM. The cap is MODEL/TP/VRAM-specific: "
                    "you must compute it, not copy a default. For reference, one "
                    "validated point (DSv4-Pro TP8, MI355X 288 GiB) was "
                    "--mem-fraction-static 0.75 --max-total-tokens 262144 -> 3.6 GB/card "
                    "KV (2x pin 7.2 GB), avail 140 GB, conc=128 0-fail. CONFIRM THE CAP "
    ...
    return {
        ...
        "launch_flags": (["--disaggregation-ib-device " + ",".join(devices)] if devices else [])
        + [
            "--mem-fraction-static <GMU>   # COMPUTE: leave room for weights + 2x KV pin",
            "--max-total-tokens <N>   # COMPUTE the cap; ref DSv4-Pro TP8 MI355X = 262144",
        ],
        "needs_dmabuf_image": True,
        "needs_user_cap": True,
...
def _rank_key(m: dict) -> tuple:
    ...
    (viable, no-cap-needed, no-pin, aggregate-bandwidth). Worked example: mode B on
    a single slow ODP NIC (no cap, no pin) OUTRANKS mode C on eight faster no-ODP
    rails (needs cap + 2x pin) -- i.e. we accept a bandwidth hit to avoid capping
    the KV pool. Bandwidth only breaks ties among equally-safe modes."""
    return (
        m["viable"],
...
_HELP_DESC = (
    "Enumerate how Mooncake can register the KV cache for PD disaggregation on THIS "
    "node: modes A (bare ibv_reg_mr+peer-mem), B (ibv_reg_dmabuf_mr on an ODP NIC), "
    "C (capped dma-buf, needs kernel P2PDMA + a user-confirmed KV cap). Each is "
    "marked viable/blocked with the reason and ranked safety-first then bandwidth; "
    "the top pick is tagged '★ best'. Prints the exact env + launch flags per mode "
    "for you (or an agent) to copy. Run it INSIDE the engine container on the GPU "
    "node -- ODP and the engine.so dma-buf check need the image; on a bare host ODP "
    "reads absent and the image capability is 'unknown'."
)

_HELP_EPILOG = """\
examples:
  # colorized table, read the '★ best' block and copy its env: / flag: lines
  python -m infera.tools.preflight.mooncake_mode

  # drop all three formats to files (json/md/txt). JSON is the machine-readable
  # one -- point an agent at mooncake_mode.json to pick a mode programmatically:
  python -m infera.tools.preflight.mooncake_mode --out-dir /tmp/mm

  # print JSON (agent) and/or Markdown (paste into a doc/PR) to stdout:
  python -m infera.tools.preflight.mooncake_mode --emit json
  python -m infera.tools.preflight.mooncake_mode --emit table,md

exit code: 0 if the best pick is a full-KV mode (A/B, no user cap); 2 if the only
viable path needs a KV cap (C) or nothing is viable -- i.e. 2 == a human decision
or an image rebuild is required.
"""


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="infera.tools.preflight.mooncake_mode",
    ...
    p.add_argument(
        ...
        default="table",
        metavar="FMTS",
        help="comma-separated formats to print to stdout: table,json,md "
        "(default: table). json = machine-readable, best for agents; md = paste-able.",
    )
    p.add_argument(
        "--out-dir",
        default="",
        metavar="DIR",
        help="also write mooncake_mode.{json,md,txt} into DIR (all three at once). "
        "Use the .json for agent/programmatic consumption.",
    )
    p.add_argument("--json", action="store_true", help="back-compat alias for --emit json")
    p.add_argument(
        "--quiet", action="store_true", help="with --json, suppress the human table on stderr"
    )
