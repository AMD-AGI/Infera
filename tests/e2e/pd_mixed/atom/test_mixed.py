...
###############################################################################
"""ATOM e2e — mixed worker (prefill-decode-mix, no PD).

The test body is engine-agnostic and shared (see
:mod:`tests.e2e.harness.mixed_suite`); only the parametrize list is
engine-specific. All bind to the ATOM ``worker`` fixture (this dir's conftest).
"""

import pytest

from ...harness.mixed_suite import run_mixed_case
from .matrix import atom_mixed_params


@pytest.mark.slow
@pytest.mark.parametrize("params", atom_mixed_params())
