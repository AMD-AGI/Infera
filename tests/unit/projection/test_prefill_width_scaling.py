...
def _ratio(bench_tp, tgt_tp, fit=None, restore=True, origami=None, mode="fit",
           probed=None, curves=None, budget=0, conc=0):
    ...
    p._measured_width_ratio = (
        InferencePerformanceProjector._measured_width_ratio.__get__(p))
    p._scaling_mode = mode
    p._origami_steps = (origami if callable(origami)
                        else lambda b, n, ph: origami)
    p._bench_prefill_curves = curves or {}
    p._step_tokens = InferencePerformanceProjector._step_tokens.__get__(p)
    p._measured_ratio_at = (
        InferencePerformanceProjector._measured_ratio_at.__get__(p))
    p._meas_ref_input = 8192
    p.cfg = SimpleNamespace(request_config=SimpleNamespace(
        input_seq_len=8192, max_num_batched_tokens=budget,
        max_concurrency=conc, batch_size=0))
...
def test_the_simulator_ratio_is_preferred_over_both_and_moves_the_whole_step():
    ...
    """
    rate, fixed = _ratio(4, 8, fit={16: (100.0, 3.6)},
                         origami=(76.8, 100.0), mode="origami")
...
def test_a_missing_simulator_falls_back_to_the_measured_fit():
    ...
    failure."""
    rate, fixed = _ratio(4, 8, fit={16: (100.0, 400.0)},
                         origami=None, mode="origami")
...
def test_the_split_is_reported_but_the_whole_step_ratio_is_what_moves(capsys):
    ...
    rate, fixed = _ratio(4, 8, origami=sim, mode="origami",
...
def test_a_simulator_slope_that_comes_out_flat_does_not_scale_prefill_away():
    ...
    rate, fixed = _ratio(4, 8, origami=flat, mode="origami",
                         probed=[1024, 8192])
...
def test_the_simulator_cannot_claim_better_sharding_than_the_anchors_showed(capsys):
    ...
    """
    rate, fixed = _ratio(4, 8, fit={16: (100.0, 400.0)},
                         origami=(40.0, 100.0), mode="origami")
    assert abs(rate - 412.5 / 425.0) < 1e-9
    assert rate == fixed
...
def test_the_floor_does_not_bind_when_the_simulator_is_already_above_it():
    ...
    rate, _fixed = _ratio(4, 8, fit={16: (100.0, 0.0)},
                          origami=(76.8, 100.0), mode="origami")
    assert abs(rate - 0.768) < 1e-9
...
def test_the_floor_is_read_at_the_length_being_asked_about(capsys):
    ...
    """
    rate, fixed = _ratio(4, 8, origami=(40.0, 100.0), mode="origami",
                         curves={2: _curve(15.20, 0.01328),
                                 4: _curve(11.53, 0.00896)})
...
def test_a_width_jump_of_a_different_size_is_not_compared():
    ...
    rate, _fixed = _ratio(4, 8, origami=(40.0, 100.0), mode="origami",
                          curves={1: _curve(7.34, 0.01701),
                                  4: _curve(11.53, 0.00896)})
