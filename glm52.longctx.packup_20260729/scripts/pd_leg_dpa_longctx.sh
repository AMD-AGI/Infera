...
# Localisation probe for the multi-chunk KV corruption: TCP is a completely different data
# path (no RDMA, no dmabuf/peermem). If corruption survives TCP, the bad KV is produced
# BEFORE the transport, i.e. on the prefill side.
[ "${FORCE_TCP:-0}" = "1" ] && export MC_FORCE_TCP=1
...
# multi-chunk KV corruption probe: does disabling the chunked PREFIX cache sidestep it?
[ "${NO_CHUNKED_PREFIX:-0}" = "1" ] && EXTRA_ARGS+=(--disable-chunked-prefix-cache)
# Prime suspect for the multi-chunk KV corruption. In disaggregation/prefill.py the
# intermediate-chunk send is overlap-gated:
#   L715 (overlap ON) : send_kv_chunk(req, last_chunk=False, end_idx=req.tmp_end_idx)
#                       "Delay KV transfer ... to ensure results are resolved"
#   L944 (overlap OFF): send_kv_chunk(self.chunked_req)   <- synchronous path
# If the corruption is a race between the chunk forward and its KV send, disabling overlap
# should clear it.
[ "${NO_OVERLAP:-0}" = "1" ] && EXTRA_ARGS+=(--disable-overlap-schedule)
