#!/bin/bash
# ---------------------------------------------------------------------------
# Experiment 02 — one infera-sglang PD leg, Qwen3-1.7B, single node.
#
# This copy exists to reproduce round r1's TWO independent startup failures:
#   (a) the derived-ZMQ-port collision between two legs launched close together
#   (b) the --hicache-ratio host-memory blow-up (354.94 GB per DP rank)
#
# HICACHE_MODE selects which side of (b) you get:
#   ratio  -> pass NO --hicache-size, so sglang's default hicache_ratio=2.0
#             sizes the host pool off max_total_num_tokens  -> the blow-up
#   size   -> pass --hicache-size <HICACHE_GB> (absolute GB)  -> the fix
#
...
BASE_GPU="${BASE_GPU:-0}"
TP="${TP:-4}"
DPA="${DPA:-1}"                 # DP-attention on both legs
KVD="${KVD:-1}"                 # 1 = wire the infera-kvd hicache backend
KVAWARE="${KVAWARE:-1}"         # 1 = publish kv events (router kv-aware source)
...
# ratio: omit --hicache-size entirely. sglang then uses hicache_ratio (default
#        2.0) x the KV pool. On Qwen3-1.7B at TP4 the KV pool came out at
#        max_total_num_tokens=1,547,424 -> 354.94 GB of HOST memory PER DP RANK,
#        x4 ranks = ~1.4 TB on a 3 TB box. Round r1 died here.
# size:  --hicache-size is an ABSOLUTE GB-per-rank cap and overrides the ratio.
...
# Both legs live on ONE host here. Same-host mooncake RDMA has to loop back
# across rails and does not work on this fabric, so force the TCP transport.
# It is irrelevant to THIS experiment (r1 never got far enough to move a byte
# of KV), but keeping it identical across rounds keeps the rounds comparable.
...
# kv-aware: infera publishes engine KV events (default ON).
# --kv-events-bind is infera's OWN publisher socket; its default 5557 is the
# same on every leg, so co-located legs each need their own.
if [ "$KVAWARE" = "1" ]; then
...
if [ "$KVD" = "1" ]; then
  ARGS+=(--infera-kvd-socket "$KVD_SOCK")
  if [ "$HICACHE_MODE" = "size" ]; then
    ARGS+=(--hicache-size "$HICACHE_GB")
  else
    echo "[leg] !! HICACHE_MODE=ratio: no --hicache-size. Expect the host-pool blow-up." >&2
  fi
fi
...
echo "[leg] hicache_mode=$HICACHE_MODE hicache_gb=$HICACHE_GB port=$PORT nic=$NIC ib=$IB_DEVICES -> $LOG"
