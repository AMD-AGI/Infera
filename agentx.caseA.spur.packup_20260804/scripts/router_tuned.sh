...
#   --kv-prefill-overlap-weight 5.0   prefill workers are COMPUTE-bound; a
...
docker exec $CTR bash -c \"printf '%s\n' '#!/bin/bash' 'exec python3 -m infera.server --host 0.0.0.0 --port $PORT --discovery-backend etcd --etcd-endpoint $MY_IP:2379 --request-transport http --kv-event-transport zmq --router-policy kv-aware --router-tokenizer-path $MODEL --kv-prefill-overlap-weight 5.0 --kv-decode-overlap-weight 1.0 --kvd-socket-path $KVD_SOCK' > /run_router.sh && chmod +x /run_router.sh\"
