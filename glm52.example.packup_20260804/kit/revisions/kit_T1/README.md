...
**Weights.** GLM-5.2-MXFP4 (~400 GB) on **each** node. A mount that merely looks
shared may be an NFS export of the peer's array, which turns an 8-minute weight load
into a ~95-minute one, exceeds the ready timeout, and presents as a crash loop.

**Image.** `inferaimage/infera-sglang:0.2.0`.

> **The image tag above is a placeholder.** Replace it with the released
> infera-sglang tag once published. Both nodes must run the same image.
...
```bash
IMAGE=inferaimage/infera-sglang:0.2.0 bash preflight_rdma.sh mode
```
...
```bash
export IMAGE=inferaimage/infera-sglang:0.2.0 DUMP_PATH=<fresh-shared-dir>
...
For a throughput reference:

```bash
bash cluster/cluster.peermem.sh bench 8 16 32
```

This uses SGLang's own `bench_serving`, which ships inside the image. Note what its
cache-hit column means on this dataset: `--dataset-name random` builds every prompt
independently, so there is **no shared prefix by construction** and any nonzero value
is residue from the previous round, not a cache result. Prefix reuse is an
agentic-workload property — see [`results/`](results/README.md).
...
**Which policy to pick.** `kv-aware` for the lower latency; `round-robin` if you need
every DP rank utilised evenly — kv-aware's concentration is self-reinforcing (no
traffic → empty cache view → never the cheapest candidate → no traffic), and the
round-robin arm measured ±4 % spread across all 8 ranks against kv-aware leaving six
of eight ranks at one batch each for an entire run.
