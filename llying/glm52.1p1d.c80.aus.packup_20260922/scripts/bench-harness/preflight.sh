...
# Artifact paths: OUT_DIR, default results/<UTC>-preflight.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-$DIR/results/$(date -u +%Y%m%dT%H%M%SZ)-preflight}"
...
run_rank() {
    ...
    ssh_run "$node" timeout "${PREFLIGHT_TIMEOUT:-1800}" \
        ...
        -e "MOONCAKE_DISABLE_HIP_DMABUF=${MOONCAKE_DISABLE_HIP_DMABUF:-1}" \
        -e "INFERA_PREFLIGHT_GID_INDEX=${MC_GID_INDEX:-1}" \
...
for rank in "${!nodes[@]}"; do
    node="${nodes[rank]}"
    name="${CONTAINER_PREFIX:-llying-aus-1p1d}-preflight-$run_token-rank$rank"
    active_nodes+=("$node")
    active_names+=("$name")
