...
: "${CONTAINER_PREFIX:=glm52-pd-yihou-p8d8}"                    # every container carries "yihou" and the shape, so a stray one is attributable and safe under the deletion rule
...
# WITHDRAWN 2026-09-19 11:35 UTC. `SGLANG_DSA_FUSE_TOPK=0` stopped the crash --
# two full 3,600 s points, zero rail faults -- but the deployment then emitted
# garbled text (`1!au!au!au!...`) while the forced acceptance gauge still read a
# healthy 3.5-3.8. It trades a visible crash for silent output corruption, which
# is the worse failure. Kept here, disabled, so nobody re-derives it.
#   : "${DECODE_EXTRA_ENV:=SGLANG_DSA_FUSE_TOPK=0}"

# --- issue.md 3.3 mitigation that KEEPS the fused indexer --------------------
# Turn off the MTP IndexShare seed instead. Verified first-hand in the image that
# this does NOT disable fused top-k on a decode worker
# (`dsa/utils.py::should_use_dsa_fused_topk`): with IndexShare off,
# `pd_index_share_seed` is False, so the function returns `SGLANG_DSA_FUSE_TOPK`
# unchanged -- and its own docstring states "PD Decode worker: Draft decode /
# target verify / draft extend: fused TopK enabled." It removes the cross-step
# seed/carry, not the kernel, so the #36714 PD seed remap stays in play.
#
# Evidence comes from the peer session working the same defect: with IndexShare
# ON their stack faulted in 4 of 5 runs (at 32 / 39 / 54 / 71 min, four different
# GPUs); with it OFF it has run **4.5 hours continuously with zero memory access
# faults**, through two completed 3,600 s AgentX points. Second-hand but
# first-hand to them, and the code path is verified here.
#
# THE UNTESTED CELL IS OURS. Their clean long runs use simulated acceptance, so
# they do not test correctness, and their one coherent real-acceptance run also
# had custom all-reduce disabled. "IndexShare=false + custom all-reduce ON" --
# exactly this configuration -- is untested by either of us for output
# correctness. **Probe the text before trusting any number from it.** Garbling is
# visible in the text even under simulation; only the acceptance gauge is masked.
#
# Note this restores the value `config.sh:82` already carries; `config.full.sh`
# deliberately empties it. Cost: the indexer top-k is recomputed on each of the
# speculative_num_steps-1 in-loop draft-decode forwards rather than reused.
# THE ASSIGNMENT IS AFTER THE SOURCE, not here -- config.full.sh:90 is a PLAIN
# assignment `JSON_MODEL_OVERRIDE_ARGS=""`, so anything set before the source is
# overwritten. Setting it here and verifying by sourcing produced an EMPTY value;
# this is the same trap the DSA_TOPK_BACKEND note below documents.
...
# Post-source, for the reason given above: config.full.sh:90 clears this
# unconditionally. A caller can still override it on the command line because
# engine.sh/agentx_bench.sh export KEY=VALUE before sourcing, and this is a plain
# assignment only when the caller left it empty.
