> 当前状态（22:40 UTC后）：G4因n04-29上的Decode原生段错误失败，没有benchmark结果。替代job31679已被UID0取消（原因未提供），后续申请等待澄清；31644仅等待显存释放后归还。唯一完整路由对照仍是A0/G0，不增加新的收益声明。
...
n04-33的AMDGPU6.14.14环境下Prefill正常就绪；n04-29的AMDGPU6.19.16环境改为Decode后仍在HIP stream路径发生原生段错误。故障在router启动之前，与路由优化代码执行无关；这支持排除n04-29当前环境，不证明仅GPU驱动版本就是唯一根因。该节点的独立PyTorch测试创建260个流、64个不同handle通过，说明问题需要完整SGLang初始化上下文，不能用简单流测试判为整个栈健康。

G4未发出benchmark流量，启动检查在22:08:20失败；改进后的health helper识别了原生fatal，而非一直等待外层容器。配置文件覆盖DIR曾导致引用base helper，新启动入口已明确使用campaign helper路径。旧P停止时docker报告未收到退出事件，显存仍约173GiB/卡，不能声称已经释放；collector/etcd和坏D已停止。31644已禁用requeue并改名llying-hicache-cleanup，前台监控脚本确认GPU/host达到释放门槛后才取消旧分配。

完成的16个AITER库已保存共享bundle；438,001,945,864字节权重预读在398.9秒完成，均是准备工作，不计作性能收益。

同账户emad/Compute-DCPT/batch的替代job31679明确请求2节点×8张MI355X并排除n04-29，于22:33:53获得n01-33/n05-21，但22:36:09被UID0取消，sacct无原因，未在这些节点启动模型。没有自动重提来绕过取消。xiewen12/Compute-Multinode仅做过test-only检查，没有实际提交；切换额度及取消原因均需用户澄清。

## 当前资源收尾完成

22:53:16 UTC，清理监控确认n04-33的8张GPU均约0.28GiB、busy0，host内存门槛通过后取消31644。该次释放超过先前29分44秒的样本，不能将30分钟当固定上限。n04-29上的本轮D此前已停止。当前无活动benchmark；替代31679由UID0取消，原因待澄清，没有转用其他账户或重复提交来规避。当前唯一完整优化对照仍是A0/G0，后续无新增吞吐收益声明。
