...
A0 全阶段 rocm-smi 采样每张P GPU 1,087次，VRAM整数百分比从87%到90–91%。3,143,424→3,400,000 tokens按当前KV bytes/token估算额外约13.2 GiB/GPU，约4.6个百分点，因此观测峰值外推约95–96%。这支持尝试受控容量上限，而不是无限放大；5秒采样不能排除未采到的瞬时峰值，仍须检查启动容量、OOM/retraction和正式期内存。首次容量试验前另读退休后的MemAvailable/Shmem，确认主机HiCache释放和重新分配余量。

## 用户最新约束与优先级修正

沿用已有基线，不反复运行A/回滚任务。只使用管理员已分配的QOS权限；当前继续batch，不尝试提升权限。每个后续case是否必须完整重启P/D另行确认，当前待启动的优化轮次本身将按可用节点状态初始化。

撤回将P overlap weight 20→10固定排第二的建议：降低缓存奖励可能把请求送往更低负载但更少命中的rank，增加miss/重算。G0仍有P排队并不能证明cache affinity过高；winner-only日志不足以重建其他rank选择。10没有模型校准，不能称推荐值。该点暂不执行。

现有候选顺序：先完成路由优化开启轮次；再考虑固定host tokens的P KV容量试验（有明确减少miss的预言，但要有完整测试/资源释放时间和显存空间）；host路径按观测决定；权重调整等有候选rank/排队证据后再做。任何节点变化均记录比较限制，不作为重复baseline的前提。

## 排队期间的可复用编译缓存准备

G1新节点首次AITER编译持续到20:24抢占时尚未完成。新增aiter_cache_bundle.py：仅打包镜像hash目录内无对应build lock、读取前后stat稳定、具有ELF头的顶层.so；记录SHA256，不复制锁和中间产物。安装仅允许全新目标目录，先验hash再落盘，避免覆盖运行中的库。构造样本检查已通过。尚未取得可访问节点的实际缓存包；不能声称缓存已备份。下一次分配若含已使用节点（例如n02-21），先保存其完成库到共享存储，再按相同镜像/硬件约束为新节点准备独立本地缓存。此项用于缩短准备时间，不是一次性能实验，也不追加baseline。
