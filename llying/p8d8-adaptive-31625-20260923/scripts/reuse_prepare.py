...
(r/'snapshot/flush-start-epoch.txt').write_text(str(int(time.time()))+'\n')
for role,(node,ip,port) in roles.items():
...
from check_empty import verify_reset
verified=verify_reset('reuse-cache-reset.json')
for role,(node,ip,port) in roles.items():
 after=inspect(node,f'{prefix}-{role}-0')
 after_info=get_json(f'http://{ip}:{port}/get_server_info')
 assert after['Id']==before[role]['Id'] and after['State']['Pid']==before[role]['State']['Pid'] and after['State']['Running']
 assert after_info.get('scheduler_pids')==report['roles'][role]['server_info'].get('scheduler_pids')
 report['roles'][role].update(after_metrics=verified[role]['metrics'],checks=verified[role]['checks'],scheduler_pids_after=after_info.get('scheduler_pids'))
report['passed']=True;report['checked_at']=time.time()
