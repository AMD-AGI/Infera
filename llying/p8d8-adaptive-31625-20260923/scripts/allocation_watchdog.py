...
import argparse,datetime,json,re,signal,subprocess,time
from pathlib import Path
p=argparse.ArgumentParser();p.add_argument('--job',required=True);p.add_argument('--prefix',required=True);p.add_argument('--role',required=True);p.add_argument('--root',type=Path,required=True);a=p.parse_args()
...
def cleanup(reason):
 ...
 for name in names:
  ...
  if c['Image']!=expected:record({'event':'unexpected_image_refuse_cleanup','container':name});continue
  x=subprocess.run(['docker','stop','--timeout','90',name],capture_output=True,text=True,timeout=120)
  record({'event':'container_stopped','container':name,'returncode':x.returncode,'stdout':x.stdout,'stderr':x.stderr})
while True:
 ...
 try:
  ...
  reason=None
  if fields.get('PreemptTime') not in (None,'None'):reason='preemption_announced'
