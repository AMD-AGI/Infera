#!/usr/bin/env bash
# Optimized configuration only, on job 31644 restart 4. No follow-up baseline.
source /perf_apps/liyingli/bench_agentx/p8d8-adaptive-31625-20260923/config/a0-guard-decode.sh
RUN_ID=g4-guard-completion
RUN="$TRACE_RUNTIME/runs/$RUN_ID"
BASELINE_RUN="$TRACE_RUNTIME/runs/a0-guard-decode"
PREFILL_NODE=smci355-ccs-aus-n04-33
PREFILL_IP=10.235.192.139
DECODE_NODE=smci355-ccs-aus-n04-29
DECODE_IP=10.235.192.57
CONTROL_NODE="$PREFILL_NODE"
BUILDER_NODE="$PREFILL_NODE"
TOPOLOGY="$TRACE_RUNTIME/config/topology-31644-restart4.tsv"
...
AITER_JIT_CACHE_ROOT=/tmp/aiter-jit-100078-g4
export AITER_JIT_CACHE_ROOT

STARTUP_PREFETCH_LOG="$TRACE_RUNTIME/events/restart4/weight-prefetch.log"
export STARTUP_PREFETCH_LOG
