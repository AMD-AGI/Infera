...
22:38:20.124 UTC：第四次尝试正式profiling开始，目标3600秒。warmup共884条，原始导出1条空内容解析错误（引擎P/D同room、输出1token），已保留；当前正式请求持续成功。

最终：23:38:20完成3600秒发送，收尾后7167条正式请求成功、错误0、取消49。23:44:50开始停P，23:45:29停止调用全部返回成功，23:45:44自动验收通过。2026-09-25 00:04:40确认16GPU均约0.096%VRAM，随后确认无KFD进程、模型容器均退出。00:05:46主动释放自己申请的31719，Slurm最终CANCELLED为资源收尾，不是实验失败。R1+R4相对G0输出-31.32%、平均TTFT+255.32%；完整结果与匹配分析已保存。不继续占用机器重做基线或追加策略。
