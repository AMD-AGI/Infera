...
for role in ('prefill', 'decode'):
    ...
    if not expected_capacity:
        ...
        if relative_delta is None:errors.append(f'{role}: capacity unavailable')
        elif abs(relative_delta)>0.001:report.setdefault('warnings',[]).append(f'{role}: automatic capacity drift {relative_delta:.4%}; recorded for comparison')
