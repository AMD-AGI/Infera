...
def detect_hardware(options: list[str], nodes: list[str], image: str) -> str:
    models = set()
    for node in nodes:
        output = ssh(options, node, ["rocm-smi", "--showproductname"])
        match = re.search(r"\bMI[0-9]{3}[A-Z]*\b", output, re.IGNORECASE)
        if not match:
            # Older host PCI databases may report only "AMD Radeon Graphics".
            output = ssh(options, node, [
                "docker", "run", "--rm", "--device=/dev/kfd", "--device=/dev/dri",
                "--group-add", "video", image, "rocm-smi", "--showproductname",
            ])
            match = re.search(r"\bMI[0-9]{3}[A-Z]*\b", output, re.IGNORECASE)
        if not match:
            raise ValueError(f"{node}: cannot detect GPU model on host or in image")
