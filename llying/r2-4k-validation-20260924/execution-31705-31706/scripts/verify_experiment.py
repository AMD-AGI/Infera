...
expected={'INFERA_PD_PREFILL_GUARD_RELEASE':'decode','INFERA_R2_DECODE_DEMAND':'on','INFERA_R3_CACHE_TIERS':'off','INFERA_R4_PREFILL_WORK':'off','RUST_LOG':os.environ['RUST_LOG']}
...
if not any('router experiment controls' in x and 'decode: On' in x and 'tiers: Off' in x and 'prefill: Off' in x for x in logs.splitlines()):errors.append('Rust experiment settings not observed')
if 'routing experiment candidate' in logs:errors.append('Verbose candidate logging was not disabled')
...
report={'passed':not errors,'errors':errors,'environment':expected,'validation_scope':'R2-on configuration, template parity, and logging settings; paired real requests checked by validate_smoke.py'}
