...
**验证计划调整（2026-09-23）：当前不做“给短请求保留份额／改变 chunk 续跑顺序”的单变量实验。** 解释排队机制不等于建议改变公平性。计算预算不变时，短请求提前执行可能让长请求更晚完成 Prefill，延长其在 D 侧已分配输入 KV 的等待；输入较大的请求每多等一秒，会增加更多 token-seconds，可能加重 D prealloc 阻塞，并反馈到 P bootstrap 和后续调度。现阶段优先验证降低服务成本、减少 miss 的方案。

这是实验优先级判断，不是已证实改顺序必然有害。需要注意：短 miss 不等于短输入；P 完成后 D 仍要生成，KV 并不立即释放；提前完成短请求也可能抵消部分驻留。因此整体排队和占用的净变化不能仅凭候选顺序确定，也不能只看短请求 queue 是否变短。原报告第 9 节已同步移除该实验并记录原因。
