#!/usr/bin/env bash
set -Eeuo pipefail
ROOT=/perf_apps/liyingli/bench_agentx/p8d8-chunk8k-31609-20260923
set -a
...
for attempt in $(seq 1 120); do
    ...
    for node in "$PREFILL_NODE" "$DECODE_NODE"; do
        actual=$(ssh -F /dev/null -o BatchMode=yes -o ConnectTimeout=10 "$node" docker image inspect "$IMAGE" --format "{{.Id}}" 2>/dev/null) || actual=missing
