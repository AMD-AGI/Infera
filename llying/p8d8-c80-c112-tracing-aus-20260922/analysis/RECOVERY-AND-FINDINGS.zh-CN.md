...
后续专题：[Dynamo 调度与缓存调研及联合优化方案](DYNAMO-SCHEDULING-AND-CACHE-RESEARCH.zh-CN.md)。该文已完成本地证据审计、debug 方案及 Dynamo 固定 commit `ff3ac59e` 的源码核验，并明确区分源码机制与尚未执行的部署/优化验证；其中同时纳入了后续 8K 实验复核，不应将本报告中的历史候选方案视为全部尚未验证。
