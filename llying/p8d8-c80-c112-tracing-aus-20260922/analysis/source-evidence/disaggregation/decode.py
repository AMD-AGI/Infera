    ...
    def pop_preallocated(
        ...
        for i, decode_req in enumerate(self.queue):
            ...
            if self.req_to_token_pool.available_size() <= 0:
                ...
                admission(self, decode_req.req, "request_pool")
                break

            if self.req_to_metadata_buffer_idx_allocator.available_size() <= 0:
                from sglang.srt.observability.aus_diag import admission
                admission(self, decode_req.req, "metadata_pool")
                break

            if hisparse_req_budget <= 0:
                from sglang.srt.observability.aus_diag import admission
                admission(self, decode_req.req, "hisparse_pool")
                break
