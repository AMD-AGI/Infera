#!/usr/bin/env python3
"""Fail closed before smoke/load if the isolated 8K run deviates from baseline."""
import hashlib
import json
import os
import re
...
for role in ('prefill', 'decode'):
    ...
    expected_chunk = 8192 if role == 'prefill' else 4096
    ...
    if role == 'prefill':
        old_cmd[old_cmd.index('--chunked-prefill-size')+1] = '65536'
...
(run/'chunk8k-config-validation.json').write_text(json.dumps(report, indent=2)+'\n')
print(json.dumps({'passed': not errors, 'errors': errors, 'report': str(run/'chunk8k-config-validation.json')}))
if errors:
    raise SystemExit(1)
