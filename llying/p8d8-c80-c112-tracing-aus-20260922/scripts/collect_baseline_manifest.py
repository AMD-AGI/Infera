...
result={'schema_version':1,'generated_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'run':str(r),
 ...
 'profiling_coverage':summary['phases']['profiling']['coverage'],
 'limitations':['Model metadata hashes do not hash every weight shard.','One baseline does not establish run-to-run variability.','Reuse requires same nodes/configuration and matched cache initialization/warmup; collect adjacent controls after material environment changes.','Profile errors and runner errors have different accounting; both are retained.']}
