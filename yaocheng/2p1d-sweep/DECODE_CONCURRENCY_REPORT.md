**GLM-5.2 MXFP4 Decode 并发分析报告：2P1D 与 1P1D 对比**
...
![对比图](analysis/decode_concurrency_comparison.png)

**1. 对齐部署和统计口径**

参考数据目录对应的部署是 **1P1D、两台机器、16 张 MI355X**：P 在 137，D 在 136，每实例 TP8/DP8。
...
与上节服务端 gauge 分列，保持同口径比较。参考包没有归档可供逐档复算的完整 server gauge CSV。
...
可复现脚本：[`compare_decode_concurrency.py`](analysis/compare_decode_concurrency.py)。
产物：[`对比 CSV`](analysis/decode_concurrency_comparison.csv)、[`2P1D 队列 CSV`](analysis/2p1d_queue_metrics.csv)、
[`PNG`](analysis/decode_concurrency_comparison.png)、[`SVG`](analysis/decode_concurrency_comparison.svg)。
...
| 内容 | 文件或目录 |
|---|---|
| 本轮部署与配置 | [README](README.md)、[config.sh](config.sh)、[run.json](results/run.json) |
| 2P1D 五档吞吐与延迟 | [results.csv](results/results.csv)，以及 `results/c080/` 至 `results/c256/` 内的 `profile_export_aiperf.csv` |
| 1P1D 最终 Run 2 结果 | [参考 results.csv](../../yihou/glm52.p8d8.agentx-sweep.packup_20260920/results/results.csv)，以及相邻 `c080/` 至 `c256/` 的 `profile_export_aiperf.csv` |
| 原图的缓存与 Decode 并发 | [cache_decode_metrics.csv](results/cache_decode_metrics.csv)、[decode_running_by_rank.csv](results/decode_running_by_rank.csv) |
| 本报告统一口径的对比数据 | [decode_concurrency_comparison.csv](analysis/decode_concurrency_comparison.csv) |
| 本报告的 2P1D 服务端队列 | [2p1d_queue_metrics.csv](analysis/2p1d_queue_metrics.csv)；`Source` 列给出原始 gauge 文件位置 |
| C256 人工恢复、取消和错误口径 | [measurement_notes.md](results/measurement_notes.md) |

报告已包含关键数值、计算式和日志摘录，可独立阅读；配套图表与 CSV 位于 `analysis/`。
链接到 `.tmp/` 的源码与完整运行日志属于本地补充证据，由 Git 忽略；重新生成服务端队列表需要保留这些原始文件。
