...
# Artifact paths: OUT_DIR defaults to .tmp/raw/<UTC>-agentx-c<CONC>; the primary
...
OUT_DIR="${OUT_DIR:-$SWEEP_TMP_DIR/raw/$(date -u +%Y%m%dT%H%M%SZ)-agentx-c$CONC}"
[[ "$OUT_DIR" == /* ]] || OUT_DIR="$SWEEP_TMP_DIR/$OUT_DIR"
[[ ! -e "$OUT_DIR" ]] || { echo "output path already exists: $OUT_DIR" >&2; exit 1; }
mkdir -p "$OUT_DIR/tmp"
OUT_DIR="$(cd "$OUT_DIR" && pwd)"
cache="${AGENTX_CACHE_DIR:-$SWEEP_TMP_DIR/cache/agentx}"
