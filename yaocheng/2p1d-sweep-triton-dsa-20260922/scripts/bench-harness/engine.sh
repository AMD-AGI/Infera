...
# Artifact paths: AITER cache defaults to .tmp/cache/aiter/<node>/<image-id>;
...
aiter_jit_cache="${AITER_JIT_CACHE_ROOT:-$SWEEP_TMP_DIR/cache/aiter}/$(hostname -s)/$image_key"
triton_cache="${TRITON_CACHE_ROOT:-$SWEEP_TMP_DIR/cache/triton}/$(hostname -s)/$image_key"
mkdir -p "$aiter_jit_cache" "$triton_cache"
