#!/usr/bin/env bash
# GLM-5.2 MXFP4 8P8D on native ATOM + Infera: prefill TP8 + DPA, decode TP8 +
# DCP8. Same image as ../8p4d-atom-infera (built by its scripts/build_image.sh).
...
PREFILL_NODE="${PREFILL_NODE:-smci355-ccs-aus-n10-29}"
PREFILL_IP="${PREFILL_IP:-10.235.192.140}"
DECODE_NODE="${DECODE_NODE:-smci355-ccs-aus-n04-21}"
DECODE_IP="${DECODE_IP:-10.235.192.135}"
...
# ../8p4d-atom-infera/patch/atom-moe-eager-decode-pad.diff (DP MoE gather padding).
PREFILL_GRAPH_ARGS="${PREFILL_GRAPH_ARGS:---enforce-eager}"
PREFILL_GPUS="${PREFILL_GPUS:-0,1,2,3,4,5,6,7}"
DECODE_GPUS="${DECODE_GPUS:-0,1,2,3,4,5,6,7}"
GPU_MEM_UTIL="${GPU_MEM_UTIL:-0.85}"
# amdgpu 6.19.x counts the RDMA-registered KV pool twice against allocatable
# VRAM, so such nodes need a lower utilization (8p4d: about 0.74 for a TP4
# decode, 0.70 for the prefill, which still ran out under C80 long prefills).
...
if (( PREFILL_OFFLOAD_GB > 0 )); then
    ...
    PREFILL_ENV+=" LMCACHE_CHUNK_SIZE=256 OFFLOAD_MIN_LOAD_TOKENS=8192"
fi
