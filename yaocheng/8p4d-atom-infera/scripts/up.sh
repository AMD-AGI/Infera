...
engine() {
    ...
    local role_env="${role^^}_ENV" extra_env="${role^^}_EXTRA_ENV"
    shift 4
...
engine prefill "$PREFILL_NODE" "$PREFILL_IP" "$PREFILL_GPUS" \
    ...
    --kv-transfer-config "$(kv_config kv_producer "$PREFILL_HANDSHAKE_PORT" "$PREFILL_PORT" "$PREFILL_IP")" \
    $PREFILL_EXTRA_ARGS

cudagraph="[1,2,4,8"
for ((size = 12; size <= 2 * CONC; size += 4)); do cudagraph+=",$size"; done
decode_args=(-tp 4 --decode-context-parallel-size 4
