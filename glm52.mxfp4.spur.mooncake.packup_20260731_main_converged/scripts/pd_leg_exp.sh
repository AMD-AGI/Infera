...
# ---- MTP: EAGLE steps=3 for PD KV-pool stability (06 recipe) ----
# PREFILL_MTP=1 enables MTP on the prefill leg too. Off by default because
# every deadlock we measured was under decode-only MTP, and changing it would
# confound a patch comparison. Arm E2 turns it on: without it the prefill
# worker never runs _draft_extend_for_prefill, never fills
# req.output_dsa_topk_indices, and never registers the draft KV pool for RDMA,
# so the IndexShare seed can never reach the decode leg at all.
MTP_ARGS=()
