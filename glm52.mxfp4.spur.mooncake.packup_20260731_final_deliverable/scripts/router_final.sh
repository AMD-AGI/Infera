...
PJOB=17443; PIP=10.245.152.84; DIP=10.245.151.183; RP=8160; PP=29160

echo "router FINAL on $PIP:$RP  (prefill $PIP:30000 / decode $DIP:30001)"
spur exec "$PJOB" bash -c "export DOCKER_CONFIG=/tmp/dockercfg
docker exec final bash -c 'pkill -f sglang_router.launch_router 2>/dev/null; true'
sleep 2
docker exec -d final bash -c 'python3 -m sglang_router.launch_router \
  --pd-disaggregation \
  --prefill http://$PIP:30000 8998 \
  --decode  http://$DIP:30001 \
  --host 0.0.0.0 --port $RP --prometheus-port $PP \
  > $W/final/router.log 2>&1'
" 2>&1 | grep -v libtinfow
echo "  -> $W/final/router.log   endpoint http://$PIP:$RP"
