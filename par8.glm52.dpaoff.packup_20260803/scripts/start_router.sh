...
docker exec "$CTR" bash -c "printf '%s\n' '#!/bin/bash' \
  ...
  'exec python3 -m infera.server --host 0.0.0.0 --port $PORT --router-backend $BACKEND --discovery-backend etcd --etcd-endpoint $MY_IP:2379 --request-transport http --kv-event-transport zmq --router-policy kv-aware --router-tokenizer-path $MODEL --kv-prefill-overlap-weight $PW --kv-decode-overlap-weight $DW $EXTRA' \
  > /run_router.sh && chmod +x /run_router.sh"
docker exec -d "$CTR" bash -c 'nohup /run_router.sh > /tmp/router.log 2>&1'
sleep 20
docker exec "$CTR" bash -c "curl -sf -m5 http://$MY_IP:$PORT/health && echo '  router healthy (backend=$BACKEND pw=$PW dw=$DW)' || { echo '  ROUTER NOT READY'; tail -30 /tmp/router.log; }"
