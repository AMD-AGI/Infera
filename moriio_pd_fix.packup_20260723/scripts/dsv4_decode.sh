#!/bin/bash
# DSv4 MoRIIO DECODE (kv_consumer) INSIDE container glm_pd on chi2866 (.47). TP4 gpu4-7.
set -u
IP=10.2.122.47; ROUTER=10.2.122.10; ETCD=10.2.122.10:2379; PREFIX=/infera/pd-dsv4/
MODEL=/mnt/vast/d_huggingface/models/DeepSeek-V4-Pro-fixed
LOG=/mnt/vast/c_huggingface/vllm_patch_verify/dsv4_decode.log
...
nohup python3 -m infera.engine.vllm \
  --model "$MODEL" --served-model-name DeepSeek-V4-Pro --port 30012 --host 0.0.0.0 --advertise-host "$IP" \
