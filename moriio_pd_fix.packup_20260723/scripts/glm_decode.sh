#!/bin/bash
# GLM-5.1-FP8 MoRIIO DECODE (kv_consumer) INSIDE container glm_pd on chi2866 (.47). TP4 gpu4-7.
set -u
IP=10.2.122.47; ROUTER=10.2.122.10; ETCD=10.2.122.10:2379; PREFIX=/infera/pd-glm/
MODEL=/mnt/vast/xiaobo/models/GLM-5.1-FP8
LOG=/mnt/vast/c_huggingface/vllm_patch_verify/glm_decode.log
...
export VLLM_HOST_IP=$IP MC_GID_INDEX=1 MORI_IB_GID_INDEX=1 HIP_VISIBLE_DEVICES=4,5,6,7
KVCFG=$(cat <<JSON
{"kv_connector":"MoRIIOConnector","kv_role":"kv_consumer","kv_connector_extra_config":{"host_ip":"$IP","proxy_ip":"$ROUTER","proxy_ping_port":36000,"http_port":30012,"handshake_port":36100,"notify_port":36200,"backend":"rdma","tp_size":"4"}}
JSON
)
nohup python3 -m infera.engine.vllm \
  --model "$MODEL" --served-model-name GLM-5.1-FP8 --port 30012 --host 0.0.0.0 --advertise-host "$IP" \
