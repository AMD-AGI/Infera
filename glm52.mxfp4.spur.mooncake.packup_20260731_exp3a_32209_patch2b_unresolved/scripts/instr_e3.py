...
def _e3i_log(global_tokens, local_tokens, forward_batch):
    ...
    try:
        ...
        # `global_num_tokens` does NOT exist on ForwardBatch -- the first
        # revision of this probe read that name and logged None on every
        # record, which is why round 1 could not test the buffer-sizing
        # hypothesis. The real fields are below (forward_batch_info.py:452-492).
        # `global_num_tokens_cpu` is the per-rank plan the MLP-sync all-gather
        # agreed on; `global_dp_buffer_len` is the buffer sized from it. The
        # local tensor is whatever the forward actually produced, so
        # local vs plan[rank] is the divergence we are hunting.
        gnt = getattr(forward_batch, "global_num_tokens_cpu", None)
        ognt = getattr(forward_batch, "original_global_num_tokens_cpu", None)
        blen = getattr(forward_batch, "global_dp_buffer_len", None)
        bs = getattr(forward_batch, "batch_size", None)
        print(
            "GLM52_E3INSTR rank={{}} global={{}} local={{}} ratio={{}} "
            "pad_mode={{}} fwd_mode={{}} bs={{}} buflen={{}} "
            "global_num_tokens_cpu={{}} orig_global_num_tokens_cpu={{}}".format(
                rank,
                g,
                l,
