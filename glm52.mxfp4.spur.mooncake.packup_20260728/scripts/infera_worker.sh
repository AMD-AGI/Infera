#!/bin/bash
# Single-node GLM-5.2 sglang worker via the INFERA engine wrapper (infera.engine.sglang),
# with etcd self-registration + KV-event publishing (kv-aware routing) + infera-kvd L3 cache.
# Runs INSIDE the pd_spur container (infera pip-installed). Single-node mix (no PD).
set -u
...
HIP_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m infera.engine.sglang \
  ...
  --hicache-ratio 1.2 --hicache-io-backend kernel \
