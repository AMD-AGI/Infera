...
if [ "$MTP" = "1" ] && [ "$ROLE" = "decode" ]; then
  ...
  # EAGLE on gfx942/gfx950: the aiter custom all-reduce kernel deadlocks during
  # EAGLE verify at high concurrency (sglang GLM-5.1 cookbook; #28815/#31071/PR
  # #31478). Distinct from the DSA rank-divergent D2H deadlock -- disable it so a
  # persisting hang cannot be misattributed.
  [ "${CUSTOM_AR:-0}" = "1" ] || MTP_ARGS+=(--disable-custom-all-reduce)
fi
