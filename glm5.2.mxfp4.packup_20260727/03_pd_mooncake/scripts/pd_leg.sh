#!/bin/bash
# GLM-5.2-MXFP4 cross-node PD leg over mooncake on infera/engine-sglang:pd-unified (PR#19).
# Merges: the pd-unified packup's proven mooncake env (dmabuf OFF / HIP-transport OFF / all-8-ionic
# / RDMAV_FORK_SAFE) with the GLM-5.2 DSA-ROCm recipe (tilelang indexer, no dsv4 backend). Runs
# INSIDE the pd_uni container. No MTP (per user). No MC_FORCE_TCP — this is the REAL RDMA path.
set -u
...
# ---- GLM-5.2 DSA-ROCm recipe (my proven Phase 1/3 envs; NOT the DSv4 R4 block) ----
...
echo "[glm52-mc] role=$ROLE ip=$MY_IP nic=$NIC gmu=$GMU dmabuf=$DMABUF ib=$IB_DEVICES -> $LOG"
