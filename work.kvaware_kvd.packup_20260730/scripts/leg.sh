#!/bin/bash
# One infera-sglang PD leg with kv-aware (kv events) + kvd (hicache) knobs.
# Runs INSIDE the container. Entry point is `infera.engine.sglang` (NOT
# sglang.launch_server) because kvaware/kvd wiring lives in the infera wrapper.
set -u
...
KVD="${KVD:-1}"                 # 1 = wire infera-kvd hicache backend
...
CONC="${CONC:-64}"
CHUNK="${CHUNK:-4096}"
# hicache host pool. MUST be an absolute size: the default ratio=2.0 sizes the host
# pool off max_total_num_tokens, and with a small model the KV pool is huge
# (1.5M tokens) -> 355 GB *per DP rank* -> 1.4 TB and the node OOMs.
HICACHE_GB="${HICACHE_GB:-8}"
# sglang derives a block of internal ZMQ/nccl ports from --port; two legs on one
# host must be spaced far enough apart or the second leg hits "Address already in use".
...
# Both legs live on ONE host here, so mooncake RDMA would have to loop back
# across rails (ionic_0 -> ionic_4) and dies with "transport retry counter
# exceeded" / "received packet mismatch". That is a same-host RDMA limitation,
# orthogonal to the kvaware/kvd wiring under test, so force the TCP transport
# (known-good correct-but-slower path) to keep the correctness check honest.
...
ARGS=(--model-path "$MODEL" --served-model-name qwen3 --tp-size "$TP" --trust-remote-code
...
# kv-aware: infera publishes engine KV events (default ON). --no-enable-kv-events kills it.
# --kv-events-bind is infera's OWN publisher socket (distinct from the sglang
# --kv-events-config port); its default 5557 is the same for both legs, so give
# each leg its own when they share a host.
if [ "$KVAWARE" = "1" ]; then
...
# kvd: infera wrapper probes the daemon then appends
#   --enable-hierarchical-cache --hicache-storage-backend dynamic --hicache-...-extra-config
# --hicache-size overrides hicache_ratio (absolute GB per rank) -> bounded host pool.
[ "$KVD" = "1" ] && ARGS+=(--infera-kvd-socket "$KVD_SOCK" --hicache-size "$HICACHE_GB")

# DP-attention (symmetric both legs)
if [ "$DPA" = "1" ]; then ARGS+=(--dp-size "$TP" --enable-dp-attention --ep-size "$TP"); fi

echo "[leg] role=$ROLE ip=$MY_IP tp=$TP base=$BASE_GPU dpa=$DPA kvaware=$KVAWARE kvd=$KVD nic=$NIC ib=$IB_DEVICES -> $LOG"
