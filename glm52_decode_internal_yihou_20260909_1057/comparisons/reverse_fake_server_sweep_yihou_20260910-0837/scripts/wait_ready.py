...
a = p.parse_args()
w = Path('/home/yihou/dev/git/infera.dev.yihou.sglang.bench.fast.script/glm52_decode_internal_yihou_20260909_1057/comparisons/reverse_fake_server_sweep_yihou_20260910-0837')
r = w/'rounds'/a.round
start = time.monotonic()
while time.monotonic() - start < a.seconds:
    now = datetime.now(timezone.utc).isoformat()
    result = subprocess.run(['spur','exec',a.job,'curl','-fsS','--max-time','5',
                             'http://127.0.0.1:31916/server_info'],capture_output=True,text=True,timeout=25)
    ...
    if result.returncode == 0:
        ...
        if info is not None:
            ...
            raise SystemExit(0)
    check = subprocess.run(['spur','exec',a.job,'docker','inspect',os.environ.get('NAME', 'yihou-glm52-reverse-0910-off-c16'),
