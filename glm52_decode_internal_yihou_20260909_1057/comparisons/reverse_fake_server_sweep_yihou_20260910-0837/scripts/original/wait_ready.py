...
a = p.parse_args()
w = Path('/shared_nfs/yihou/playground/glm52_fake_tp4ep4_10k500_20260910-0452')
r = w/'rounds'/a.round
start = time.monotonic()
while time.monotonic() - start < a.seconds:
    now = datetime.now(timezone.utc).isoformat()
    result = subprocess.run(['spur','exec',a.job,'curl','-fsS','--max-time','5',
                             'http://127.0.0.1:31816/server_info'],capture_output=True,text=True,timeout=25)
    ...
    if result.returncode == 0:
        ...
        if info is not None:
            ...
            raise SystemExit(0)
    check = subprocess.run(['spur','exec',a.job,'docker','inspect','g52-fake-short-tp4ep4',
