...
# kernel path: /torch-cache/mu/cmu2jynnygjgspdcfh2cw2qr4mutl7zepsznbujicdjxbb6ifvwa.py
...
#   view => view
# Graph fragment:
#   %arg1_1 : Tensor "f32[1, 1][1, 1]cuda:2" = PlaceHolder[target=arg1_1]
#   %arg2_1 : Tensor "f32[1, 1][1, 1]cuda:2" = PlaceHolder[target=arg2_1]
#   %mul_1 : Tensor "f32[1, 1, 1][1, 1, 1]cuda:2" = PlaceHolder[target=mul_1]
#   %unsqueeze : Tensor "f32[1, 1, 1][1, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg1_1, 2), kwargs = {})
#   %view : Tensor "f32[1, 1, 1][1, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%arg2_1, [-1, 1, 1]), kwargs = {})
#   %mul_1 : Tensor "f32[1, 1, 1][1, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %view), kwargs = {})
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %getitem : Tensor "f32[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=operator.getitem](args = (%max_1, 0), kwargs = {})
...
@triton_heuristics.pointwise(
    size_hints={'x': 1}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'out_ptr0': '*fp32', 'out_ptr1': '*fp32', 'xnumel': 'constexpr', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=2, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {'xnumel': 1}, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]]}]},
...
# kernel path: /torch-cache/cj/ccji5zhcgcyw635n7fek7wa5sj23kqpcx667qxq43ahhxzr4e4js.py
...
#   max_1 => max_1
# Graph fragment:
#   %arg3_1 : Tensor "i64[1, 1][1, 1]cuda:2" = PlaceHolder[target=arg3_1]
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %gather : Tensor "i64[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%arg3_1, 1, %getitem_1), kwargs = {})
#   return %gather
...
@triton_heuristics.pointwise(
    size_hints={'x': 1}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i64', 'out_ptr0': '*i64', 'xnumel': 'constexpr', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=2, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {'xnumel': 1}, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]]}]},
...
# kernel path: /torch-cache/yu/cyu2qkho2py7n5ssazaueexpnlsypolm74j42djqlkcwynh74sgu.py
...
# Graph fragment:
#   %arg5_1 : Tensor "bf16[1, s98][s98, 1]cuda:2" = PlaceHolder[target=arg5_1]
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %view_4 : Tensor "i64[1][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%getitem_1, [1]), kwargs = {})
#   %div : Tensor "i64[1][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor_mode](args = (%view_4, 1), kwargs = {rounding_mode: floor})
#   %iota : Tensor "i64[1][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (1,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:2, requires_grad: False})
#   %unsqueeze_1 : Tensor "i64[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%iota, 1), kwargs = {})
#   %view_5 : Tensor "i64[1][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%expand, [1]), kwargs = {})
#   %add : Tensor "i64[1][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%div, %view_5), kwargs = {})
#   %index : Tensor "bf16[1, s98][s98, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%arg5_1, [%add]), kwargs = {})
#   return %index
...
# kernel path: /torch-cache/d2/cd2je3blkb2uz7wj5h2yzfxzumwbjxzl5xzx5msbvtow5nkrrk2n.py
...
#   %view_1 : Tensor "f32[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_1, [1, 1]), kwargs = {})
#   %max_1 : [num_users=2] = call_function[target=torch.ops.aten.max.dim](args = (%view_1, -1, True), kwargs = {})
#   %add_3 : Tensor "i64[1, 1][1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%getitem_1, %arg6_1), kwargs = {})
#   return %add_3
...
@triton_heuristics.pointwise(
    size_hints={'x': 1}, 
    filename=__file__,
    triton_meta={'signature': {'out_ptr0': '*i64', 'ks0': 'i64', 'xnumel': 'constexpr', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='hip', index=2, multi_processor_count=256, cc='gfx950', major=9, regs_per_multiprocessor=131072, max_threads_per_multi_processor=2048, warp_size=64), 'constants': {'xnumel': 1}, 'configs': [{(0,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_max_mul_unsqueeze_view_0.run(arg1_1, arg2_1, buf0, buf1, 1, stream=stream2)
            del arg1_1
            del arg2_1
            buf2 = empty_strided_cuda((1, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, gather], Original ATen: [aten.view, aten.max, aten.gather]
            stream2 = get_raw_stream(2)
            triton_poi_fused_gather_max_view_1.run(arg3_1, buf2, 1, stream=stream2)
            buf3 = empty_strided_cuda((1, s98), (s98, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [flatten, max_1, flat_cs, floordiv, batch_offsets, repeat_interleave, selected_input_index, hidden_states], Original ATen: [aten.view, aten.max, aten.floor_divide, aten.arange, aten.unsqueeze, aten.add, aten.index]
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_arange_floor_divide_index_max_unsqueeze_view_2.run(arg5_1, buf3, s98, stream=stream2)
            del arg5_1
            buf4 = empty_strided_cuda((1, 1), (1, 1), torch.int64)
            # Topologically Sorted Source Nodes: [flatten, max_1, add_2], Original ATen: [aten.view, aten.max, aten.add]
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_max_view_3.run(buf4, s77, 1, stream=stream2)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = 1
    arg1_1 = rand_strided((1, 1), (1, 1), device='cuda:2', dtype=torch.float32)
    arg2_1 = rand_strided((1, 1), (1, 1), device='cuda:2', dtype=torch.float32)
    arg3_1 = rand_strided((1, 1), (1, 1), device='cuda:2', dtype=torch.int64)
    arg4_1 = 6144
    arg5_1 = rand_strided((1, 6144), (6144, 1), device='cuda:2', dtype=torch.bfloat16)
    arg6_1 = 1
