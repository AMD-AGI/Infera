...
def deepseek_v4_attention_with_output(
    ...
    try:
        from sglang.dsv4_trace import dsv4_trace

        dsv4_trace(
            "model.custom_op:deepseek_v4_attention_with_output",
            layer_id=layer_id,
...
class MQALayer(nn.Module):
    ...
    def forward(
        ...
        try:
            ...
            dsv4_trace(
                "model.MQALayer.forward",
                layer_id=self.attn_mqa.layer_id,
                forward_batch=forward_batch,
                compress_ratio=self.compress_ratio,
                extra="ntok=%s cls=%s" % (x.shape[0], type(attn_backend).__name__),
            )
        except Exception:
            pass
        ...
        if is_unified_kv_triton():
            try:
                ...
                dsv4_trace(
                    "model.MQALayer.branch1_unified_kv",
                    layer_id=self.attn_mqa.layer_id,
                    forward_batch=forward_batch,
                    compress_ratio=self.compress_ratio,
                )
            ...
            if forward_batch.forward_mode.is_extend() and is_in_breakable_cuda_graph():
                try:
                    ...
                    dsv4_trace(
                        "model.MQALayer.branch2_bcg_customop",
                        layer_id=self.attn_mqa.layer_id,
                        forward_batch=forward_batch,
                        compress_ratio=self.compress_ratio,
                    )
                ...
                try:
                    ...
                    dsv4_trace(
                        "model.MQALayer.branch3_nonunified_direct",
                        layer_id=self.attn_mqa.layer_id,
                        forward_batch=forward_batch,
                        compress_ratio=self.compress_ratio,
                    )
