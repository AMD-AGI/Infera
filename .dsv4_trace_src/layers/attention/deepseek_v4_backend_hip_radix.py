    ...
    def _forward_unified_kv(
        ...
        *,
        ...
        try:
            ...
            dsv4_trace(
                "HipRadixBackend._forward_unified_kv:%s"
                % ("decode_or_idle" if is_decode else "prefill"),
                layer_id=layer_id,
                forward_batch=forward_batch,
                compress_ratio=compress_ratio,
                extra="is_idle=%s" % forward_batch.forward_mode.is_idle(),
            )
        except Exception:
            pass
        if is_decode:
        ...
        **_,
        ...
        if is_unified_kv_triton():
            try:
                ...
                dsv4_trace(
                    "HipRadixBackend.forward->unified_kv",
                    layer_id=layer.layer_id,
                    forward_batch=forward_batch,
                    compress_ratio=compress_ratio,
                )
        ...
        if isinstance(core_attn_metadata, DSV4AttnMetadata):
            ...
            try:
                ...
                dsv4_trace(
                    "HipRadixBackend.forward->flashmla_inline",
                    layer_id=layer_id,
                    forward_batch=forward_batch,
                    compress_ratio=compress_ratio,
                )
