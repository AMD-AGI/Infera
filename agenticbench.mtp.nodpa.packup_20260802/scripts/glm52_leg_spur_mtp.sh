...
ISL="${ISL:-8192}"
# ---- chunked-prefill-size: the silent confounder in a DPA A/B --------------
# --chunked-prefill-size is a GLOBAL token budget per prefill step, not a
# per-rank one. Under dp-attention the batch is split across $TP ranks, so
# 8192*8 = 65536 globally IS 8192 per rank -- which is where the default below
# comes from and why the lat1/Case A runs all carried chunk=65536.
#
# The bug this comment exists for: the `else` branch used to hardcode 8192, so
# flipping DPA off ALSO cut the global chunk 8x. A 74K prompt would go from 2
# prefill steps to 10, and a 235K one from 4 to 29. Per-step overhead
# (scheduling, the kv-event publish, the mooncake handoff bookkeeping) is paid
# per step, so TTFT would rise -- and the rise would be attributed to
# "turning off DPA" when most of it is just a smaller chunk.
#
# So CHUNK is a caller-supplied override here and the noDPA arm passes 65536
# explicitly, matching lat1 exactly. If a leg then OOMs on activation memory,
# that is a REAL finding about non-DPA prefill (one rank must now hold the whole
# 65536-token chunk's activations rather than 8192) and must be reported as
# such -- not silently papered over by shrinking the chunk. Fix it with GMU
# first, and if the chunk genuinely has to move, the comparison is broken and
# the run needs a matched chunk on BOTH arms.
if [ "$DPA" = "1" ]; then CHUNK="${CHUNK:-$((ISL * TP))}"; else CHUNK="${CHUNK:-$((ISL * TP))}"; fi
...
# SGLANG_DP_USE_GATHERV selects the variable-length gather used to collect
# per-rank DP-attention outputs. With dp-attention off there is no DP group to
# gather across and the variable is read by nothing, so it correctly leaves with
# DPA -- unlike CHUNK above, this one is genuinely inert and needs no override.
...
if [ "$DPA" = "1" ]; then
  ...
  # ---- DPA=0: keep EP, drop ONLY DP-attention -----------------------------
  # GLM-5.2 is a MoE. --ep-size selects EXPERT parallelism, which is a different
  # axis from attention data-parallelism: with dp-attention on, each of the 8
  # ranks holds the full KV for its own subset of requests and attention is
  # computed locally; with it off, attention is tensor-parallel across all 8 and
  # the KV pool is sharded by head. Neither statement is about experts.
  #
  # The original `if` gated all three flags together, so DPA=0 also collapsed
  # the MoE from ep8 to the TP default -- changing the expert-dispatch collective
  # at the same time as the attention layout. That makes "DPA on vs off" a
  # two-variable comparison and there is no way to attribute the delta afterward.
  # Keeping --ep-size $TP holds the MoE side fixed so the ONLY thing that moves
  # is attention.
  #
  # --enable-prefill-delayer is deliberately NOT carried over. It is not a
  # general-purpose knob: it delays a prefill batch so that DP ranks with
  # different arrival times can be batched together, which is meaningless when
  # there are no independent DP ranks to align. It is DPA's own machinery, so it
  # leaves with DPA. At concurrency 1 it is inert either way -- there is never a
  # second request to align with -- so this costs nothing here, but stating it
  # keeps the reasoning honest rather than resting on "N=1 hides it".
  DP_ARGS+=(--ep-size "$TP")
fi
