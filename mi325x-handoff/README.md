...
| 130–180 tok/s/user 的目标 | **中位数达成**（133–154），均值 122–133 压在下沿；见 §10.6 |
...
| 配置 | Mean TPOT | **tok/s/user** | Median TPOT | **中位 tok/s/user** |
| --- | --- | --- | --- | --- |
...
**关掉 dp-attention 之后，中位 tok/s/user 稳定在 133 – 154，落在 130–180 目标区间内；
均值 122 – 133，压在区间下沿。** 相对出厂配方是 2.3 – 2.5×。
缓存效率三次都是 100.00%，accept len 也没有退化（4.29 → 4.32），
说明收益不是拿正确性或缓存换来的。

**为什么要报区间而不是单个数：** `CHUNK=2048` 的第一次测量看起来比 `CHUNK=8192`
好 8%（8.18 → 7.52 ms），但 `CHUNK` 名义上只影响 prefill leg 的分块，**不应该影响
decode 侧的 TPOT**。所以补测了第二次 —— 8.02 ms，那 8% 落回了噪声里。
两次的 Mean ITL 几乎相同（33.55 / 33.72 ms），accept len 汇总后 4.32，
和 `CHUNK=8192` 的 4.29 一致，**证实 `CHUNK` 对 decode 没有影响，理论和实测对上了**。

差异全部来自 accept len 的run 间抖动（单批次范围 1.73 – 6.00）。这条对客户很重要：
**在这个负载上单点 conc=1 测量有约 ±7% 的抖动，任何小于 10% 的调参结论都必须重复测量。**
本次能识破这一点，正是因为先有"CHUNK 不该影响 decode"的理论预期，
再拿它去质疑一个看起来不错的结果。
