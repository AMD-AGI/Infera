#!/usr/bin/env bash
# Re-anchor the two patches/sglang_disagg/ scripts onto a POST-v0.5.17 sglang
# tree. Both consumers need it and both need the SAME two edits:
#
#   Dockerfile.sglang        stock v0.5.18  (glm_moe_dsa family)
#   Dockerfile.sglang.glm53  PR #36607 head (glm5_next family), stacked on #36507
#
# The name still says glm53 because that is where it was first needed; it is not
# GLM-5.3-specific. Verified 2026-09-01 inside lmsysorg/sglang:v0.5.18-rocm720-mi35x
# that both drifted anchors below read EXACTLY as they do on the GLM-5.3 branch,
# that disaggregation/common/utils.py still has the OLD typing import (so the
# conn.py edit stays per-file rather than a global sed), and that conn.py still
# has no wait_event/synchronize() call — i.e. the fix is still needed there too.
#
...
# prompts should not ship. Between the v0.5.17 base and these trees, exactly two
# anchor LINES moved, and neither fix moved upstream:
#
