...
> **先读这一条,它决定下面每一条能不能生效。**
> **把规则写下来,不等于拥有它。** readme-cn 今天第二次撞上「`git commit` 因
> pathspec 加倍而失败,而链在它后面的 `&& push` 照跑」——**第一次之后他已经把
> 教训写下来了。** 他自己的结论:*「两次真正救回来的,都是我读了命令的输出,
> 而不是读了『push 发生了』这个事实。」*
> **推论:一条需要「记得」才生效的规则会失效;一条改变命令本身的规则才有效。**
> 他的修法是机械的——**用绝对路径;不要用 `&&` 跨过一个会静默失败的步骤。**
> 写进这个文件的每一条,都该问一句:它是要求人记得,还是改变了命令?
