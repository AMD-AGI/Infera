#!/usr/bin/env bash
# GLM-5.2 mooncake PD with DP-attention on infera/engine-sglang:pd-unified.
# prefill=chi2878, decode=chi2879. Both legs symmetric-DPA (dp8+ep8) so KV shard layout
# matches across mooncake. Preps containers (libionic inject), launches both DPA legs, then
# leaves router-start as a printed next step (legs cold-start ~5-8min for DP).
set -euo pipefail
...
ROUTER_PORT=8002
MAX_RUNNING="${MAX_RUNNING:-2048}"    # server scheduler cap; sized for the whole sweep
DMABUF="${DMABUF:-0}"
CTX="${CTX:-32768}"
CTR=pd_uni
...
# 2. stage scripts into both containers
for h in "$PREFILL_HOST" "$DECODE_HOST"; do
  J "$h" "docker cp $KIT/pd_leg_dpa.sh $CTR:/pd_leg_dpa.sh; docker cp $KIT/probe.py $CTR:/tmp/probe.py 2>/dev/null || true"
done

# 3. launch both DPA legs (decode :30001)
echo "== 2. launch prefill DPA leg ($PREFILL_HOST :30000) =="
# NOTE: use `env VAR=... bash`, NOT `bash -lc '...'` — the login-shell -d form does not persist.
J "$PREFILL_HOST" "docker exec -d $CTR env ROLE=prefill MY_IP=$PREFILL_IP MODEL=$MODEL SERVED=$SERVED PORT=30000 DPA=1 DMABUF=$DMABUF CTX=$CTX MAX_RUNNING=$MAX_RUNNING bash /pd_leg_dpa.sh"
echo "== 3. launch decode DPA leg ($DECODE_HOST :30001) =="
J "$DECODE_HOST" "docker exec -d $CTR env ROLE=decode MY_IP=$DECODE_IP MODEL=$MODEL SERVED=$SERVED PORT=30001 DPA=1 DMABUF=$DMABUF CTX=$CTX MAX_RUNNING=$MAX_RUNNING bash /pd_leg_dpa.sh"

echo "== legs launching (mooncake RDMA + DPA, dmabuf=$DMABUF). DP cold start ~5-8min. =="
echo "   watch: J $PREFILL_HOST \"grep -E 'ready to roll|error|Traceback' $KIT/pd_prefill_30000_dpa.log | tail\""
