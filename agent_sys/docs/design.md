...
├── examples/ok.filetree_grounded_report.4/          the demo task package — YAML and data, NOT a Python
...
nothing to say why. `examples/ok.filetree_grounded_report.4`'s own history holds that bug: an unfilled
...
| 13 | Swapping the backend changes no other component | — | **`agent` and `cli` designs** |
