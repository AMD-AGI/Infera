...
| 4c | **P0 — a validator cannot reach the artefact its target was produced *from*, so five task packages scan the store instead** — *and the route this row previously proposed does not exist* | Was scoped to `examples/ok.filetree_grounded_report.4/logic/store.py`; the file is now `examples/ok.filetree_grounded_report.4/assets/lib/store.py` and **five copies** of it (`demo`, `ok.algorithms_solve_grade.14`, and three under `examples/llm_e2e_performance_optimization/`). See below |
...
| 4h | **the per-caller install pins are not yet repointed at the shared root** | §9.1 states the rule — a declared install lands in one shared root, and only a `.claude/` tree is per-agent — and `AGENT_SYS_HOME` (`~/.infera_agent_sys`, `bin/ share/ state/ run/`) is its single owner. That constant now exists (`env_mgr/prefix.py`), but the installs still pin their destinations one variable at a time (`UV_TOOL_DIR`, `UV_TOOL_BIN_DIR`, `UV_CACHE_DIR`, and serena's `SERENA_HOME`), each into a scratch path chosen by the caller. **A second root must not be introduced**: that would be exactly the parallel mechanism `engineer_principle.md` §2 forbids, and the two would drift over which one is authoritative. Remaining: repoint those four pins under `<root>` and delete the per-caller choice. Nothing about the rule changes — only where the string comes from |
...
is `inputs: [problems]` (`examples/ok.algorithms_solve_grade.14/steps/problems.yaml:48`) while its body
