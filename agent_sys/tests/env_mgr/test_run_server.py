...
pytestmark = pytest.mark.skipif(not Path("/proc/self/stat").exists(), reason="needs procfs")
...
def _alive(pid: int) -> bool:
    """Is `pid` a running, non-zombie process — read from `/proc`, independent
    of `env_mgr.servers` itself, since `os.kill(pid, 0)` succeeds for a zombie."""
    try:
    ...
    for pid in started:
        for sig in (15, 9):
            try:
                ...
                break
            time.sleep(0.1)
        # Reaped, not merely killed, so CPython does not warn about a still-
        # running subprocess at shutdown. `WNOHANG` so a pid we never parented,
        # or one already reaped, does not block the next test's teardown.
...
def test_a_second_declaration_of_the_same_server_warns_and_starts_nothing(zone, reaper):
    """A duplicate declaration warns and starts nothing new; asserted on the
    registry, not only the level, since two entries behind a `warn` is the
...
def test_an_occupied_port_held_by_the_same_program_warns(tmp_path, reaper):
    """The holder reports as `python3`, but the declared program token
    `myserver` still appears in its command line, so it must warn, not fail."""
...
def test_a_holder_whose_command_line_cannot_be_read_fails(tmp_path):
    """Occupied, with an unknowable holder, fails rather than warns — it can
    never be shown to be the same program."""
...
def test_a_wrapped_command_still_names_its_program(declared):
    """`exec myserver` declares `myserver`, not `exec` — a wrapper word is
    skipped, and one outside the closed list degrades to `fail`, never a
    false `warn`."""
...
# The guard here is where being wrong harms something outside the run:
# `stop_all` sending SIGTERM/SIGKILL to a process `agent_sys` does not own. A
# sacrificial child plus a fabricated record with its real pid but a wrong
# `starttime` simulates pid reuse without aiming a kill at pytest itself.
...
def sacrifice(reaper):
    ...
    `start_new_session=True` makes it its own process-group leader, so
    `stop_all`'s `killpg` can never reach pytest's own group.
    """
...
def test_the_harness_can_actually_kill_the_sacrifice(tmp_path, sacrifice):
    """The positive control the two tests below mean nothing without: with the
    correct `starttime`, `stop_all` kills the same child it would otherwise
    refuse to touch."""
    registry = tmp_path / "servers.json"
...
def test_a_reused_pid_is_never_signalled(tmp_path, sacrifice):
    """A stale entry whose pid has been recycled must not be signalled: the
    record carries the sacrifice's real pid but a `starttime` that is not
    its own."""
    registry = tmp_path / "servers.json"
