...
#
# `Item.layer` is removed: where an installed thing lands is derived, not
# declared (`docs/spec.md` §9.1). A stale `layer:` key is rejected, not
# silently carried into `item.spec`.
...
def test_a_stale_layer_key_is_rejected_and_says_where_the_concept_went(tmp_path):
    # A stale `layer:` is rejected rather than silently carried into
    # `item.spec`; the message must name the replacement, not merely refuse.
    p = _write(
        tmp_path,
        """
        version: 1
