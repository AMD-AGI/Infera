...
def _run_harness(monkeypatch: pytest.MonkeyPatch, tmp_path, *, settle_error=None):
    ...
    registry = SimpleNamespace(get=lambda name: {"runner": runner, "task_mgr": task_mgr}[name])
...
def test_real_run_shuts_down_executors_when_settle_raises(monkeypatch, tmp_path) -> None:
    ...
    args, stream, runner, monitors = _run_harness(monkeypatch, tmp_path, settle_error=failure)
