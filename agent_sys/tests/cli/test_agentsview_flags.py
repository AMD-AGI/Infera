...
def test_a_failed_install_does_not_go_on_to_start_a_daemon(monkeypatch) -> None:
    ...
    ran = []
    monkeypatch.setattr(
        cli_main, "ensure_installed", lambda prefix, install_item: Status(False, "no network")
    )
...
def test_the_installers_two_ok_messages_still_discriminate() -> None:
    ...
    (skip_msg,) = [o.message for o in BinInstaller().install(satisfied, target) if o.level == "ok"]
...
def test_the_install_closure_runs_nothing_until_it_is_called(monkeypatch, tmp_path) -> None:
    ...
    ran = []
    monkeypatch.setattr(env_mgr.runner, "run", lambda *a, **k: (ran.append((a, k)), ([], "ok"))[1])
...
def test_show_never_reaches_the_panel(monkeypatch) -> None:
    ...
    ran = []
    monkeypatch.setattr(
        cli_main, "ensure_running", lambda *a, **k: ran.append(1) or Status(False, "x")
    )
...
def test_a_skipped_panel_says_nothing_to_the_user(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(cli_main, "ensure_installed", _installed("agentsview already present"))
    monkeypatch.setattr(
        cli_main, "ensure_running", lambda prefix, port: Status(False, "port in use")
    )
    monkeypatch.setattr(cli_main, "_run", lambda args, stream, stack, panel_url=None: 0)
    cli_main.main(["run", "--package", "pkg"])
