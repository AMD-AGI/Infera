...
def main() -> int:
    ...
    # *Grounded in its input* compares two handoffs while an output phase
    # stages only the one it validates, so a body that reaches outside itself for
    # the `facts` needs a route out — and both are shut: a store scan gets
    # `EACCES` from a confined body, and adding `facts` to this validator's own
    # inputs is not the system's job. The second artefact travels **inside the
    # first**, so there is nothing to reach for.
    results = {}
