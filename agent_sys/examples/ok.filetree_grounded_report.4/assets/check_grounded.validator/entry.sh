#!/bin/sh
# The body of `check_grounded`. Run by `validator.ScriptBodyRunner` as
# `/bin/sh <package_root>/assets/check_grounded.validator/entry.sh` with `cwd` set to a freshly
...
exec "${AGENT_SYS_DEMO_PYTHON:-python3}" \
  "${AGENT_SYS_TASK_PACKAGE:-${AGENT_SYS_DEMO_PACKAGE:?the demo runner exports one of these}}/assets/check_grounded.validator/check.py"
