...
`items_schema`, the same way `examples/ok.filetree_grounded_report.4`'s `summary` admits `grounding`.
...
That is the ruling `examples/ok.filetree_grounded_report.4/steps/describe.yaml` records in its `summary`
