...
there never will be. Like `../ok.filetree_grounded_report.4/`, it may use nothing a task package outside
...
```bash
agent-sys run --package agent_sys/examples/ok.algorithms_solve_grade.14 --var n_problems=2
```

---

## What it is meant to prove that `../ok.filetree_grounded_report.4/` does not

`../ok.filetree_grounded_report.4/` is a chain of three and proves the format loads and a run completes.
...
| **Depth-2 nesting** | `grade` is a subgraph inside a subgraph: root → `grade` → six leaves. `../ok.filetree_grounded_report.4/` is one level |
| **Ten validators** | `../ok.filetree_grounded_report.4/` has two. Four of these compile and run real C++, so *it does not build* and *it never terminates* are recorded verdicts rather than a hang |
...
It is byte-for-byte `../ok.filetree_grounded_report.4/assets/lib/store.py` apart from two docstring
