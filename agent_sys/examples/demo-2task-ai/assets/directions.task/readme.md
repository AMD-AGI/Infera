...
Choose exactly **`$TWO_TASK_N_DIRECTIONS`** directions. That is an environment
variable and it is set for you; read it rather than assuming a number — this
package defaults to a small count and `--var n_directions=N` changes it.
...
Choose **`$TWO_TASK_N_DIRECTIONS` different** rows. Two directions with the same
