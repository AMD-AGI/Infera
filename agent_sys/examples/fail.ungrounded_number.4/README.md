# `fail.ungrounded_number.4` — an ungrounded number is refused, not sealed
...
This is the deterministic, pure-program version of `examples/ok.filetree_grounded_report.4`'s
...
under `fail.ungrounded_number.4` are `grounded_verdict_fails` and
`consumer_waits`; observing both is exit 0.

## Run

```bash
AGENT_SYS_NO_PERMISSIONS=1 agent-sys run --package agent_sys/examples/fail.ungrounded_number.4
```
