...
def main() -> int:
    ...
    for hid in store.inputs():
        ...
        # `examples/ok.filetree_grounded_report.4` types its `summary` as `text`, whose one required item
        # is `items/content`. Here the kind is `structured_text`, so the document
        # is `items/text.json` and `additionalProperties: false` means
        # `items/content` cannot exist. Reading the wrong name would raise before
        # a verdict is written, which reports as a broken validator rather than
        # as the ungrounded numeral this package exists to show.
        claimed = numerals((content / "items" / "text.json").read_text(encoding="utf-8"))
