# `fail.judge_disagreement.5` — two judges disagree at runtime
...
This is the pure-program version of `examples/ok.algorithms_solve_grade.14`'s
...
`fail.judge_disagreement.5` are `agree_verdict_fails` and `downstream_waits`;
observing both is exit 0.

## Run

```bash
AGENT_SYS_NO_PERMISSIONS=1 agent-sys run --package agent_sys/examples/fail.judge_disagreement.5
```
