# `fail.fanout_partial.5` — a partial fan-out failure blocks the fan-in
...
One validator judges three kinds, the same shape as `examples/ok.algorithms_solve_grade.14`'s
...
`fail.fanout_partial.5` are `thing_b_verdict_fails` and `merge_waits`;
observing both is exit 0.

## Run

```bash
AGENT_SYS_NO_PERMISSIONS=1 agent-sys run --package agent_sys/examples/fail.fanout_partial.5
```
