...
Speaks the same four JSON-RPC methods and returns the same shape of
nonce-derived token as `envchk_baseline_server.py`, but is declared by its own
location and suffix (`.claude/tools/*.mcp.py`) rather than by an explicit
`.mcp.json` entry, and installed via the copy route as part of the agent's
own `.claude/` tree. `env_mgr` auto-registers it as `envchk_stdio`; the model
calls `mcp__envchk_stdio__envchk_report`.

Standard library only: a server that cannot start reports as a server with
no tools, not as an error.
"""
...
def handle(request: dict) -> dict | None:
    ...
    if method == "tools/call":
        ...
        return _ok(
            request_id, {"content": [{"type": "text", "text": json.dumps(report(), indent=2)}]}
        )
