...
#: `ok.sglang_real_model.2`'s shape check, for the same reason: a document that is
...
#: is not a URL or an e-mail address. Applied to prose only, with code spans
#: and fenced blocks removed first, so a format description like
#: `` `ENVCHK-<LABEL>-<12 hex>` `` in a code span is not mistaken for an
...
def check_section(label: str, section: object, min_how: int) -> list[str]:
    ...
    Every rule is a key present or absent, a pattern match, or a count
    against a floor -- nothing in it is a judgement.
    """
    ...
    # `installed_by` names one of the two install routes an agent may claim.
