...
Every token in the report is checked against the token that capability
actually produces. Two of the six are re-derived by running the capability:
both MCP servers are started here and spoken to over the protocol. The other
four are recomputed from the salt in the installed artefact.

See the readme beside this file for what each treatment proves and does not,
by capability.

No credential is read, echoed or written here, and neither is the nonce:
`ENVCHK_NONCE` is only compared and hashed.
"""
...
def zone_config(package: Path) -> Path:
    ...
    `<staged package>/../config`: the two are siblings inside the zone.
    """
...
def artefact_of(capability: envchk.Capability, package: Path) -> Path:
    ...
    Either in the staged package or in the zone's config directory. A missing
    file is caught where it is read, by `replay_mcp` or `envchk.salt_of`.
    """
...
def replay_mcp(server: Path, nonce: str, timeout: float) -> tuple[dict | None, str]:
    ...
    Three frames written to stdin and the pipe closed: both servers here are
    stateless and read stdin to EOF, so this is the whole conversation.
    """
...
def replay_salt(artefact: Path, label: str, nonce: str) -> tuple[dict | None, str]:
    ...
    Establishes only that the artefact is installed where the capability
    puts it, not that the agent reached it through the capability.
    """
...
def check_liveness(where: str, produced: dict) -> list[str]:
    ...
    Checked for shape only; there is no run window available to this body
...
def check_hook_payload(where: str, proof: object) -> list[str]:
    ...
    `payload.session_id` and `payload.hook_event_name` are supplied only by
    the harness; an agent that ran the script by hand gets neither.
    """
    ...
    if event != "SessionStart":
        faults.append(
            f"{where}.proof.record.payload.hook_event_name: {event!r}, needs 'SessionStart'"
        )
    return faults


#: The keys serena's `find_symbol` puts on each hit:
...
def serena_hits(raw: object) -> list[dict] | None:
    ...
    Accepts either the raw JSON string or the already-parsed array; anything
    else is not a `find_symbol` response.
    """
...
def check_serena_proof(where: str, proof: object, salt: str) -> list[str]:
    ...
    Raises the cost of forgery, not proof. Checks the salt, since the token
    never appears in `find_symbol`'s response.
    """
...
def check_raw_token(where: str, proof: object, token: str) -> list[str]:
    ...
    Caught separately from a simply-wrong token, since a `token`/`proof.raw.token`
    disagreement means the section was assembled from two different sources.
    """
    if not isinstance(proof, dict):
        return [f"{where}.proof: missing or not an object"]
    raw = proof.get("raw")
...
def serena_excused(installs: object) -> tuple[bool, str]:
    ...
    Uses `env_mgr.outcome.LEVELS`: `ok`/`info` is not a failure, `warn`/`fail`
    is. An entry counts only when it names serena and looks non-`ok`.
    """
    ...
    for entry in installs:
        ...
        if isinstance(level, str) and level.lower() in ("ok", "info"):
            continue
        if isinstance(level, str) or any(
            word in text for word in ("warn", "refus", "fail", "error")
        ):
            return True, ""
    return False, (
...
def check_capability(
    ...
    if capability.label == "serena":
        # Re-read the salt from the artefact rather than threading it through
        # `replay_salt`'s return, since that function's contract is "what the
        # capability produces", and the salt is an input, not a result.
        salt, why = envchk.salt_of(artefact)
...
def main() -> int:
    ...
    if not nonce:
        ...
        print(
            "check_capabilities_genuine: FAIL: $ENVCHK_NONCE is not set in this body's environment"
        )
