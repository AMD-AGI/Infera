# Acceptance criteria for the `ok.agent_capabilities.2` run
...
Throughout this document: **`$AGENT_SYS`** is the checkout's `agent_sys/`
directory; **`$RUN_ROOT`** is the scratch root passed to `--demo-root`;
**`$MAIN_REPO`** is the main repository's `.git` directory (distinct from a
worktree checkout, when the checkout running this package is itself a
worktree). Export all three before running any command block below.
...
python3 -m cli.main run \
  --package examples/ok.agent_capabilities.2 \
  --demo-root $RUN_ROOT/runroot \
  --json      $RUN_ROOT/runroot/stream.json \
  --var nonce="${ENVCHK_RUN_NONCE}" \
  --var uv_root=$RUN_ROOT/uv \
  --timeout 3600
```

**`python3 -m cli.main`, from inside `$AGENT_SYS`, never a globally installed
console script.** A console script on `PATH` can resolve to a different
checkout's `agent_sys` entirely, silently loading this package against another
`spec_loader`: a schema error that is not this package's bug, or worse, a pass
that means nothing.

**`--demo-root` on local disk.** Runs, handoffs, playground and workspace all
relocate together under it. Use a scratch root on local disk, not a shared
filesystem — `$RUN_ROOT` is where this task's scratch belongs.
...
non-empty for `examples/ok.agent_capabilities.2` or `addons/`.**

A commit SHA recorded after the fact does not guarantee reproducibility if the
working tree carried uncommitted edits when the run happened: anyone
reproducing from that SHA gets a different package than the one that ran, and
**no pin recorded after the fact can repair that** — the run is simply not
attributable.
...
header because the script lives in scratch that survives nothing — a rule that
exists only in the tool it governs disappears with it. Both gates are: *the
package is committed*, and *the demo-root is unused*.
...
grep -m1 -o '"run_id"[^,]*' $RUN_ROOT/runroot/stream.json
```
...
| 3b | plugin **source path** | copied | the zone's `config/settings.json` | its marketplace entry's `{"source":"directory","path": …}` is **not** under the run root — e.g. it points outside the zone entirely. A marketplace loading from a component's *source* directory rather than from a copy is a plugin that installs cleanly and then fails to load under confinement with nothing naming the cause. This is its own row and not a footnote precisely because it is a condition that must fail loudly rather than be checked if someone remembers |
...
| 7 | serena | **recipe install + a declaration in the agent's own `.mcp.json`** | same → `capabilities.serena` | **the agent's `.claude/.mcp.json` declares no serena MCP server** — an install without a declaration gives the agent `No such tool available`. Then: `.status == "ok"` and `.token != T("serena")` from the salt in `assets/env_probe.agent/serena_probe.py`; **or** `.status == "unavailable"` and `install_report` carries no non-`ok` entry mentioning serena; **or** `.proof.raw` is not a `find_symbol` response whose hit for `envchk_serena_token` carries `name_path`, `kind`, `relative_path` naming `serena_probe.py`, a `body_location` with integer `start_line`/`end_line`, and a `body` **containing the salt**. That schema was measured against Serena 1.28.1, not remembered — see *What a PASS does not prove* for what it is and is not worth |
...
- **Row 6b claimed something wider than one capability, and it is the loss worth
  ...
  **The `sys.modules` detector is proved on every run** — the test imports a
  ...
  not. A prose reference to a test rots silently.
...
- An in-process `ToolDef` ran in the **supervisor's** process, and
  ...
  to the **CLI child**. The supervisor never saw it. A tool built this way
  therefore returned a well-formed token computed from an **empty**
  `$ENVCHK_NONCE`: it did not fail, it lied to the agent, and the agent quoted it
  correctly. Any future in-process route inherits that, and a token that is
  byte-identical across two runs is its fingerprint.
...
- **Serena being *used* is not established**, only that it was installed and
  ...
  **shape** Serena 1.28.1 actually returns — measured by driving the installed
  binary, not remembered — and that is a **forgery-cost increase, not a
  closure**: it moves the bar from *read one file* to *read the file **and**
  know serena's response schema*. **A model that has seen serena's schema can
  still fabricate it.** Two further consequences, stated rather than
  discovered: a serena release that changes that schema turns an honest run red,
  which is a validator update and not a capability failure (the failure message
  says so); and the salt sits inside the function body rather than at module
  scope purely so that `find_symbol`'s body carries it — without that, an agent
  could call serena correctly and still have no salt.
- **One green run is not three.** Acceptance here is written criteria plus one
  real run; this file does not claim reproducibility.
...
Run in order, from inside `$AGENT_SYS`. Each item is a command and the
...
`$RUN_ROOT/logs/PREFLIGHT.md`, with the git HEAD and worktree state it ran
against. A reported 10/10 is a claim; that file is the artefact — which is the
standard §2 applies to the run, turned on the gate that precedes it. Re-run it
after any commit that touches `env_mgr`. **Do not run any of these against an
`env_mgr/` tree that is being edited concurrently** — a failure seen then is a
failure that cannot be attributed.
...
|---|---|---|
| 1 | `python3 -m cli.main show --package examples/ok.agent_capabilities.2 --var nonce=x --var uv_root=$RUN_ROOT/x` | `2 tasks in the graph`, and both `output validation runs 2` |
...
| 3 | `python3 -c 'from env_mgr.recipe import load_recipe; print(len(load_recipe("env_mgr/recipes/serena.yaml")[1]), "items"); print(len(load_recipe("examples/ok.agent_capabilities.2/assets/main.env_recipe.yaml")[1]), "items")'` | `3 items` then `1 items`. **Both recipe layers, because section 4 and section 7 now depend on one each**: the second places the `envchk-baseline` server, the first installs serena. A `FileNotFoundError` on either means that capability cannot pass |
| 4 | `python3 -m cli.main run --package examples/ok.agent_capabilities.2 --demo-root $RUN_ROOT/dryrun --var nonce=x --var uv_root=$RUN_ROOT/x --dry-run` | completes, dispatches nothing, no `REJECTED` |
| 5 | `claude plugin validate examples/ok.agent_capabilities.2/assets/env_probe.agent/.claude/plugins` | `Validation passed` |
...
| 6b | `python3 -c "import shutil,subprocess;p=shutil.which('claude');print(p);print(subprocess.run([p,'--version'],capture_output=True,text=True).stdout.strip())"` | prints a path and a version; record both. That is the build this run pins **and** the build every probe conclusion now holds on — see *Measured on the pinned build* below. A different version on re-run is a **stop**, and it is a stop for two reasons at once: the run would invoke a CLI nobody characterised, **and** the probe evidence would no longer apply to it. The discharge is re-measuring the probes on the new build, not proceeding carefully |
| 7 | `python3 $RUN_ROOT/selftest/run.py` | `ALL OK` — 20 cases, both validators driven through their real `entry.sh` over a synthetic handoff. Case 2 is `every-capability-is-declared`: every capability reached through MCP has an artefact declaring its server name, which is the class of failure this row exists to catch with no run at all. Case 1 is `salts-are-isolated`: six `ENVCHK_SALT:` tags across the package and `addons/`, all distinct, **each value in exactly one authored file**, and none of them in the three documents that describe the scheme. That is the property every other case assumes, so it is checked mechanically rather than argued |
| 8 | `git --git-dir=$MAIN_REPO config --get extensions.preciousObjects` | prints `true`. **If it prints nothing, stop and ask the user** — see below |
| 9 | `df -h /tmp \| tail -1` | room to spare on the scratch filesystem. The workspace is a `git clone --shared`, so the main repository's objects are reached through alternates rather than copied; disk is not usually a constraint for this run, but the row exists so that a full disk is ruled out rather than assumed |
...
for. It is deliberately *not* `command -v`: a host can carry more than one
`claude` build on `PATH` at once, and which one answers depends on whose `PATH`
is asking.
...
**Pin `cli_path` explicitly when building probe or run options.**
`ClaudeAgentOptions` built without `cli_path` resolves through
`SubprocessCLITransport._find_cli`, which returns the SDK's *bundled* binary
before it ever consults `PATH`. A probe (or a run) that omits it silently
exercises a different `claude` build than the one row 6b just pinned — three of
this package's own probe conclusions were originally measured this way,
against the bundled binary rather than the pinned one, and no artefact would
have said so.

They were re-measured with `cli_path` pinned explicitly to the version row 6b
names, and all three came back positive: the `SessionStart` hook fires and its
token matches; an external `mcp_servers` entry reaches the model, returning a
value only the server process holds; the plugin is visible and again names the
**marketplace source path** as its skill base directory. That measurement's
plugin install also went through the pinned CLI, so the plugin-visibility
conclusion holds on the pinned build by repetition rather than by inference.

**Cite conclusions measured on the pinned build, not on whatever build the SDK
happened to bundle.** Neither is wrong; they are evidence about different
builds, and only one of them is the build this run uses.
...
and succeed without them"* — which is exactly the failure the row-6b caveat
...
It should already read `true` on the main repository's `.git`. **So
`--allow-repo-config` is neither needed nor permitted here** and must not
appear in the invocation: that flag exists to let the demo set the config
*itself*, and writing config into a repository shared across worktrees is
exactly the host-state change this task may not make without asking. If item 8
ever comes back empty, that is a question for the user, not a flag.
...
> **Symbols, not line numbers.** A line number in a document that outlives the
> code is a maintenance trap that reads as precision — the real call sites can
> move between commits without any signal in this file. Cite the **function**;
> if a line helps a reader find it, give it as a hint and expect it to rot. The
> same applies to every other reference in this file.
