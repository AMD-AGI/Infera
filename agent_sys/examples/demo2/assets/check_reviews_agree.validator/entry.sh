#!/bin/sh
# The body of `check_reviews_agree`. `validator.ScriptBodyRunner` runs it as
# `/bin/sh <package_root>/assets/check_reviews_agree.validator/entry.sh` with `cwd`
...
exec "${AGENT_SYS_DEMO_PYTHON:-python3}" \
  "${AGENT_SYS_TASK_PACKAGE:-${AGENT_SYS_DEMO_PACKAGE:?the runner exports one of these}}/assets/check_reviews_agree.validator/check.py"
