...
**而最该记住的是:数字没变,含义全变了。**(leader 的措辞,比我原来那句准。)
`0/469` 之前的意思是「一个从来没有机会去报告的 body」;现在它的意思是
**「一个机会到了、而它沉默的 body」**。**同一个分数,两种完全不同的东西**——这也正是
本文件此前拒绝写「永远」的那条道理的另一面:计数本身不携带它的含义。

*(级别归档的影响见 `WHAT-GREEN-ESTABLISHES.md` §2.10:这次拒绝是否能替代一次负控制,
是一个待 leader 裁决的问题,我没有自行改判据。)*

### 第二次机会,同样沉默 —— 而这次「原因」只存在于一份诊断里

**运行 `20260905T162032-4473f1`(m5 离线判分:29 PASS / 13 REFUSE / 3 NOT_GRADEABLE)。**
`check_environment` 在**三个 kind** 上拒绝:`e2e_packup`、`integration_report`、
`kernel_optimization`。**m5 的探针在三个上都没有找到任何 `PROBLEM` 行。**

所以计数要再改一次含义,而分数照旧:

```
0 / 469   第一次:机会到了一次,沉默一次
0 / …     第二次:机会到了三次,沉默三次
```

**而这一次比上一次更贵,因为贵在别处。** 上一次的代价是 m1 花十五分钟**重新推**出那句
话;这一次那句话被推出来了,**但它只存在于我给 leader 的诊断和 T76(`c56adb8`)里,
不在任何产物里**。下一个从 `verdict.json` 出发的人,拿到的仍然是三个 `false`,
**外加一个和上次一样的通用 escalation**。

一句话就够,而这个 body 已经把它算出来了——`find_record()` 的返回值第二项**就是**那句
话(*"no environment.yaml at any of [...]"*),它被 append 进 `findings` 并 print 到
**stderr**,然后随 stderr 一起丢掉。**这不是「没有理由」,是「理由算出来了、写在了
一个不保留的地方」**——`bug.record` 里「validator stdout 哪儿都不留」那条,在这里
第二次收费。

**三个 kind 的失败条件和成因,记在 T76;此处只记它对本文件那个分数的影响。**
**并且这一条没有关闭:** 修 T76 的三个站点会让这次拒绝消失,**但不会让下一次拒绝
开口说话**。两件事,不要合并。
