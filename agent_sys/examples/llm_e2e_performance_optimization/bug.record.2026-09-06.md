...
## 一个未加限定的时长进了收尾记录,而限定词晚到一分钟 —— `3d683bf1`

(leader,2026-09-07 07:11–07:17。**不是框架缺陷,是记录缺陷,写在这里因为下一个
读 `work.checkpoint.summary.md` 的人会先读到那句话。**)

`work.checkpoint.summary.md` 的收尾章节(提交 `3d683bf1`,07:11:21)写着:

```
Module 4 的 body 在不到 18 分钟内产出了产物
远低于此前所担心的:第一个集群测到一次 campaign 113 分钟
……是这个集群的 forge 配置更短,还是工作量更小,我没读
```

**没有 campaign。** `forge_mock=1` 在 run `20260907T045327-13a18e` 的发车行上,
读自 `/proc/3140102/cmdline`,06:41 读的。m2 精确量过 body:

```
06:30:04 → 06:34:00   四分钟           (其中测量步骤 06:31:28 → 06:34:00)
18 分钟是把输出验证的开头也算进去的上界,同样是 mock 的
```

**两个数字都是真的,而它们说的是两件事。** 而那句「是不是配置更短」,给读者提供了
一个本来就不该做的比较的解释 —— 比单纯写错更容易被接受下来。

**当前正确的状态:**
- **模块 4 在 `forge_mock=1` 下的 body:四分钟(实测一次)。**
- **真实 KernelForge campaign 在本集群的代价:零个测量。** mission 里那个未决问题
  没有被回答,而一个 mock 的四分钟顶替它,是最糟的一种过期数字 —— 精确、第一手、
  有出处,而且说的是另一件事。
- 阶段 1–3 的 90–96 分钟(实测三次)成立;**≈110 分钟这个链条总数必须带着
  「m4 为 mock」这个限定,而且要写在同一句话里。**

**为什么这条记在这里而不是改那份文件:** `work.checkpoint.summary.md` 是只追加的
记录,归 checkpoint;今晚已经有过一次「重建一份共享文件、把别人同一小时的更正一起
生成掉」的教训。**我在自己的地盘更正,不动他们的文件 —— 代价是读那份文件的人要
读到这里才知道,这一点如实写下。**

**而这是今晚第三次「限定词没跟着数字一起走」**(前两次:`check_kernel_table` 的
豁免、`stack window` 的命中率)。**这一次伤害最大,因为它落在了一份提交里。**
> **一个错的数字进了提交,会活得比能纠正它的那场对话久。**
