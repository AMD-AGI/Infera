...
def staged(kind: str) -> Path | None:
    value = os.environ.get(f"AGENT_SYS_INPUT_{kind.upper()}")
...
def main() -> int:
    ...
    manifest = read_json(sources["kernel_patch"] / "items" / "codes" / "manifest.json", {}) or {}
    ...
    for arm in ("stock", "patched"):
        accept = sources[f"acceptance_{arm}"] / "items" / "result"
        ...
        bench = sources[f"bench_{arm}"] / "items" / "result"
    ...
    for kind in ("deployment_stock", "deployment_patched", "acceptance_stock",
                 "acceptance_patched", "bench_stock", "bench_patched", "patch_overlay"):
    ...
    for kind in ("patch_overlay", "deployment_stock", "deployment_patched",
                 "acceptance_stock", "acceptance_patched", "bench_stock", "bench_patched"):
    ...
    # ---- the patch itself ----------------------------------------------------
    shutil.copytree(
        sources["kernel_patch"] / "items" / "codes", codes / "handoffs" / "kernel_patch"
    )
    shutil.copyfile(
    ...
    # ---- environment.md ------------------------------------------------------
    env_stock = read_json(sources["deployment_stock"] / "items" / "env" / "deployment.json", {}) or {}
    context = read_json(sources["acceptance_stock"] / "items" / "env" / "context.json", {}) or {}
    engine_argv = ""
    argv_file = sources["deployment_stock"] / "items" / "env" / "engine_argv.txt"
...
{len(manifest.get('files', []))} file(s), applied as `{manifest.get('apply_mode')}`.
...
and `deployment_patched`'s `env/container_hashes.tsv` records what those paths
