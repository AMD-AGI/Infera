...
5. **`max_throughput_regression` 默认 0.05 是否合适。** 这个数字目前没有实测依据——需要先测出同一臂两次运行的自然离散度，才知道 5% 是宽是严。落地后第一轮就能拿到这个数。
   ...
   **2026-09-02 实测，spur 集群 `crsuse2-m2m-276`，Qwen3.6-27B tp=2（见
   `integration.debug.help.info.md` 第 14–18 节）：这个问题问错了方向。**

   同一个部署实例内连续重放四次，离散度很小——inter-token latency **2%**，
   吞吐 **15%**，TTFT 45%。**所以 5% / 10% 这两个默认值对它们能看见的东西
   而言是合适的，不需要放宽。** 我在 r5 里把它们放宽到 0.35 / 0.30 是错的，
   那是拿一个没有被控制的变量去标定阈值。

   真正的问题不在阈值，而在**两臂之间有一个设计没有控制的变量**：

   > 本设计控制了 session、节点、trace、顺序和镜像，**没有控制「测量时刻的
   > 节点负载」**。在共享节点上，这一项是主导项。

   证据是一次对照实验：在同一节点、同一邻居负载下新起一个 **stock** 部署，
   与 15 分钟前的 patched 部署逐轮对比——

   ```
   patched  46.24 / 47.38 / 47.40 / 53.05 tok/s   itl 480.45 / 473.72 / 471.31 / 477.15 ms
   stock    46.96 / 47.56 / 47.27         tok/s   itl 467.36 / 470.33 / 473.12         ms
   ```

   两者 inter-token latency 相差 **1.1%**，即 patch 本身没有可测代价（读代码
   也能得到同样结论：首次调用之后它只是一个模块级全局量上的布尔分支，落在
   64 层里的 16 个 full_attention 层上）。而同一天 r5 的 stock 臂在邻居空闲时
   测到 193.59 tok/s，在邻居把八张卡跑满时测到 47。

   **所以要补的是一道「可比性闸门」，不是更宽的阈值**：在每臂测量时记录节点
   负载，两臂差异过大就拒绝比较；或者把两臂交错测量而不是顺序测量。
   `check_service_live` 现在只证明部署是活的，不证明它和另一臂**可比**。
   放宽阈值不解决这件事，只是把一个没被控制的变量放行得更晚。
