...
| | |
|---|---|
| binary | version **2.1.246**, resolved through `PATH` — which is what `installers/base.py:31` gets (`shell=True`, inherited env). No SDK bundle involved |
| config | `CLAUDE_CONFIG_DIR` into a dedicated scratch config directory throughout |
| host isolation | `~/.claude/plugins` md5 `1214bce39983fb0398ebaec87ade0989` before and after seven installs — **unchanged**. (`~/.claude.json` was *not* used as the isolation test: live Claude Code sessions rewrite it continuously, so its checksum proves nothing) |
...
non-discriminating and which is the decoy, are kept alongside the probe scratch
directory used for this investigation.
