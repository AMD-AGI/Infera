...
#   run_baseline.task   PD_CAPTURE=0   one handoff  (aiperf_baseline)
#   run_profiled.task   PD_CAPTURE=1   two handoffs (aiperf_profiled, torch_trace)
#
# **Why the profiled round is one task and not two.** The profiler window has to
...
say "endpoint=$R trace=$(basename "$PD_AIPERF_TRACE") window=0..${PD_TRACE_END_MS}ms capture=$CAPTURE"
...
on "NODE_IP='$PD_NODE_IP' ROUTER_PORT='$PD_ROUTER_PORT' SERVED='$PD_SERVED' \
    MODEL='$PD_MODEL' MODEL_MOUNT='$(dirname "$PD_MODEL")' \
    AIPERF_IMAGE='$PD_AIPERF_IMAGE' AIPERF_OUT='$AIPERF_OUT' \
    AIPERF_TRACE='$PD_AIPERF_TRACE' SCRIPTS='$LOAD' \
    TRACE_END_MS='$PD_TRACE_END_MS' MAX_CONC='$PD_MAX_CONC' \
    WORKERS='$PD_WORKERS' BLOCK_SIZE='$PD_BLOCK_SIZE' \
    REQ_TIMEOUT='$PD_REQ_TIMEOUT' TAG='$TAG' \
...
# exists only so `kernel_scan` can name the Python frame that launched each
# ranked kernel.
STACK_OK=0
if [ "$CAPTURE" = "1" ]; then
  say "cutting the measurement window (warmup ${PD_WARMUP_S}s, window ${PD_WINDOW_S}s, with_stack=0)"
  on "NODE_IP='$PD_NODE_IP' ROUTER_PORT='$PD_ROUTER_PORT' CTR='$CTR' \
      TRACE_OUT='$TRACE_OUT' WARMUP_S='$PD_WARMUP_S' WINDOW_S='$PD_WINDOW_S' \
  ...
  if [ "${PD_STACK_WINDOW_S:-0}" -gt 0 ]; then
    say "cutting the stack window (window ${PD_STACK_WINDOW_S}s, with_stack=1)"
    on "NODE_IP='$PD_NODE_IP' ROUTER_PORT='$PD_ROUTER_PORT' CTR='$CTR' \
        TRACE_OUT='$TRACE_OUT' WARMUP_S=0 WINDOW_S='$PD_STACK_WINDOW_S' \
    ...
    say "stack window disabled (PD_STACK_WINDOW_S=${PD_STACK_WINDOW_S:-0})"
  fi
fi
...
json.dump({
    ...
    "trace": "$(basename "$PD_AIPERF_TRACE")",
    "trace_window_ms": [0, $PD_TRACE_END_MS],
    "concurrency_ceiling": $PD_MAX_CONC,
    "aiperf_workers": $PD_WORKERS,
    "isl_block_size": $PD_BLOCK_SIZE,
    "aiperf_image": "$PD_AIPERF_IMAGE",
    "profiler_window": {"captured": $CAPTURE, "warmup_s": $PD_WARMUP_S, "window_s": $PD_WINDOW_S},
...
NODE_IP=$PD_NODE_IP ROUTER_PORT=$PD_ROUTER_PORT SERVED=$PD_SERVED \\
MODEL="\$MODEL_MOUNT/$MODEL_NAME" MODEL_MOUNT="\$MODEL_MOUNT" \\
AIPERF_IMAGE=$PD_AIPERF_IMAGE AIPERF_OUT="\$WORK_ROOT/aiperf" \\
AIPERF_TRACE="\$AIPERF_TRACE" SCRIPTS="\$SCRIPTS" \\
TRACE_END_MS=$PD_TRACE_END_MS MAX_CONC=$PD_MAX_CONC WORKERS=$PD_WORKERS \\
BLOCK_SIZE=$PD_BLOCK_SIZE REQ_TIMEOUT=$PD_REQ_TIMEOUT \\
...
One AIPerf replay of a Mooncake production trace against GLM-5.3-Flash, at the
...
if [ "$CAPTURE" = "1" ]; then
  ...
  # **Only PD_STACK_RANKS of them are carried.** Every rank runs the same Python,
...
NODE_IP=$PD_NODE_IP ROUTER_PORT=$PD_ROUTER_PORT CTR=$CTR \\
TRACE_OUT="\$WORK_ROOT/profiles" WARMUP_S=$PD_WARMUP_S WINDOW_S=$PD_WINDOW_S \\
WITH_STACK=0 OUT_SUBDIR=mixed \\
bash "\$SCRIPTS/capture.sh"

NODE_IP=$PD_NODE_IP ROUTER_PORT=$PD_ROUTER_PORT CTR=$CTR \\
TRACE_OUT="\$WORK_ROOT/profiles" WARMUP_S=0 WINDOW_S=${PD_STACK_WINDOW_S:-3} \\
WITH_STACK=1 OUT_SUBDIR=mixed_stacks \\
bash "\$SCRIPTS/capture.sh"
EOF
...
during this window is not a baseline. The graphs-on numbers are in the baseline
round's handoff.
...
first PD_STACK_RANKS rank files. It exists so kernel_scan can name the Python
...
One torch-profiler window cut out of a running AIPerf replay: ${PD_WINDOW_S}s,
opened after ${PD_WARMUP_S}s of warm-up and only once the engine had reported an
...
\`with_stack\` on for ${PD_STACK_WINDOW_S:-0}s, kept for the first
${PD_STACK_RANKS:-2} rank(s) only, with its own
\`items/result/stacks_manifest.json\`. It is the input to \`kernel_scan\`'s
...
backend in it is what identifies this as the profiled round.

## Watch out

See \`items/watchout\`: prefill and decode kernels share this trace, and the
throughput during the window is not a baseline.
EOF
fi
