...
OUT="${PD_OUTPUT_KERNEL_TABLE:?}"
IN="${PD_INPUT_TORCH_TRACE:?}"
...
if ! on "test -d '$TRACES'" >/dev/null 2>&1; then
  say "ABORT: the staged trace directory is not visible on $PD_NODE: $TRACES"
  exit 1
fi
...
on "MAGPIE_ROOT='${PD_MAGPIE_ROOT:?}' bash '$ANALYZE/megapie.sh' '$TRACES' '$MEGAPIE_OUT'" \
...
python3 "$ANALYZE/top_kernels.py" "$CSV" "$ITEMS/result/top_kernels.json" \
  "${PD_TOP_N:-25}" || { say "ABORT: could not summarise the gap analysis"; exit 1; }
...
if [ "${n_stacks:-0}" -gt 0 ]; then
  on "python3 '$ANALYZE/launchers.py' --csv '$CSV' --stacks '$STACKS' \
      --out '$LAUNCHERS' --top-n '${PD_LAUNCHER_TOP_N:-50}' \
      --max-files '${PD_STACK_RANKS:-2}'" 2>&1 | tee "$WORKDIR/launchers.log"
...
on "cd '${PD_MAGPIE_ROOT}' && git rev-parse HEAD 2>/dev/null || echo unknown" \
