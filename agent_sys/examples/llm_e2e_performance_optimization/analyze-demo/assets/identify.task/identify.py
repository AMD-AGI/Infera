...
CONTAINER_ROOTS = {
    owner: "${%s}" % spec["placeholder"] for owner, spec in (_ROOTS.get("owners") or {}).items()
}
...
SCHEMA = {
    ...
    "title": "operator_identity",
    "type": "object",
    "required": ["generated_at", "resolver", "summary", "operators"],
    "properties": {
        ...
        "operators": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "image_repo_path": {"type": "string"},
                    "repo_subdir": {"type": "string"},
                    "repository_language": {"type": "string"},
                    "source_file_path": {"type": "array", "items": {"type": "string"}},
                    "editable_sources": {"type": "array", "items": {"type": "string"}},
                    "target_kernel_functions": {"type": "array", "items": {"type": "string"}},
                    "source_resolution_method": {
                        "enum": ["trace_python_stack", "symbol_search", "agent_recovered"]
                    },
...
def main() -> int:
    ...
    magpie_root = Path(os.environ.get("AD_MAGPIE_ROOT") or "/nonexistent")
    repos = [r for r in (os.environ.get("AD_SGLANG_SRC"), os.environ.get("AD_AITER_SRC")) if r]
    timeout = int(os.environ.get("AD_RESOLVE_TIMEOUT_S") or 1800)
    ...
    out = {
        ...
        "container_root_placeholders": {
            "${%s}" % spec["placeholder"]: spec.get("description", "")
            for spec in (_ROOTS.get("owners") or {}).values()
        },
        "summary": {
    ...
    placeholders = "\n".join(
        f"| `${{{spec['placeholder']}}}` | {spec.get('description', '')} |"
        for spec in (_ROOTS.get("owners") or {}).values()
    )
