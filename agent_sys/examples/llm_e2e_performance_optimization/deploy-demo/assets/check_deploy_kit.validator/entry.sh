...
# `/bin/sh <package_root>/assets/check_deploy_kit.validator/entry.sh`, with
...
exec "${AGENT_SYS_DEMO_PYTHON:-python3}" \
  "${AGENT_SYS_TASK_PACKAGE:-${AGENT_SYS_DEMO_PACKAGE:?the runner exports one of these}}/assets/check_deploy_kit.validator/check.py"
