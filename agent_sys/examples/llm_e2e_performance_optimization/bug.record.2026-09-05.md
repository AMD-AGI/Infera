...
**最后核对:2026-09-05T08:35Z**(**19 条**;第 17 条已积累 **4 个实例**)。
...
任何一份操作文档上这都是**有界**的活,而且**两个实例它都能抓到**。
...
### 实例 C —— 这份文件自己,08:12Z 用这条检查抓到的

**推荐一条检查之前先跑一遍它。** 我拿本文件开头那张速查表逐格对正文核,
**方向单一,一次跑完**:

**表格停在第 13 条,正文已经有 17 条。** 第 14–17 条**在表里根本不存在**——
而这四条里有三条是**我自己**今天写进正文的(`e9e4534` 写了 14、15,今天写了
16、17),每一次我都在正文里补完就收手了。

**这就是这条机制预言的形态,一字不差:正文长,表格不长;作者在正文里,读者在
表格里。** 它在**第一次应用**上就抓到了**推荐它的人自己的文件**。

**已修:表格补齐至 17 条。** 代价:一次 `sed` 加四行。**如果不是被这条检查逼着
看,我不会去看那张表——我今天改了这个文件五次,五次都没看它。**
