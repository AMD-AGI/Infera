    ...
    for op in operators:
        ...
        if not operator_id or op.get("operator_id") == operator_id:
            ...
            if not block:
                return (
                    {},
                    f"the workset declares operator {op.get('operator_id')!r} with no integration block",
                )
            block = dict(block)
            ...
            return block, ""
    return {}, (
        f"{_DISAGREES}the workset declares no operator {operator_id!r} "
...
def check_against_workset(manifest: dict, integration: dict) -> list[str]:
    ...
    for i, entry in enumerate(manifest.get("files") or []):
        ...
        if path not in declared and not any(
            d.endswith(tail) or tail.endswith(d.lstrip("@").split("/", 1)[-1]) for d in declared
        ):
            bad.append(
    ...
    for gate in sorted(declared):
        ...
        if results is None:
            print(
                f"apply: NOTE the workset declares a {gate!r} gate and m4's handoff carries no "
...
def check_apply_manifest(manifest: dict, apply_dir: Path, packup: Path) -> list[str]:
    ...
    **Not `patchkit.check_manifest`**, and the difference is the one field that
    ...
    if manifest["schema_version"] != patchkit.SCHEMA_VERSION:
        bad.append(
            f"schema_version is {manifest['schema_version']!r}, this stage reads {patchkit.SCHEMA_VERSION}"
        )
    ...
    for i, entry in enumerate(files):
        ...
        if entry["change"] not in patchkit.CHANGE_KINDS:
            bad.append(
                f"{where}.change {entry['change']!r} is not one of {list(patchkit.CHANGE_KINDS)}"
            )
        ...
        if has_patch == has_replacement:
            ...
            if not diff.is_file():
                bad.append(
                    f"{where}.patch names {entry['patch']!r}, which is not in apply/patches/"
                )
            else:
                ok, why = patchkit.looks_like_unified_diff(
...
def main() -> int:
    ...
    ap.add_argument(
        "--deploy-kit",
        required=True,
        help="m1's staged content directory, for the environment record",
    )
    ap.add_argument(
        "--operator-workset",
        default=None,
    ...
    if integration:
        ...
        print(
            "apply: workset declares "
            + (
                f"public_symbol {symbol!r}"
                if symbol
        ...
        print(
            f"apply: NOTE **{why}**. This is a DISAGREEMENT, not a missing block: "
        ...
        print(
            f"apply: NOTE no declared integration point to check against — {why}. "
    ...
    for i, entry in enumerate(entries):
        ...
        if patched.name.endswith(".py"):
            ...
            if (
                dropped
                and (integration or {}).get("substitution") == "call_site_fragment"
    ...
    for m, real in zip(mounts, real_paths):
        ...
        if got != m["sha256_patched"]:
            raise SystemExit(f"apply: {real} hashes {got} on {node} but {m['sha256_patched']} here")
...
""",
    ...
    subprocess.run(
        [
            ...
            "--inherit",
            str(Path(args.deploy_kit) / "items" / "codes" / "environment.yaml"),
            "--content-type",
            "reproducible",
            "--out",
            str(out),
