...
# Mock first (mission Brief item 2), real work second.
#
# `|| rc=$?` and not `; rc=$?`: under `set -e` a simple command exiting non-zero
# kills the script before the assignment runs, so the branch below would never
# be reached and a real-mode task would die with the mock's status. A `||` puts
# it in a condition context, where `set -e` does not fire.
rc=0
bash "$PKG/assets/lib/mock.sh" stage5-integration patch_overlay || rc=$?
if [ "$rc" -eq 0 ] && [ -n "$(ls -A "${AGENT_SYS_OUTPUT_PATCH_OVERLAY:?}" 2>/dev/null)" ]; then
