...
def judge(probe: dict, response: dict) -> list[str]:
    ...
    needs_json = any(key in expect for key in ("json_nonempty", "json_all", "json_present"))
    document = None
    if needs_json:
        try:
    ...
    for rule in expect.get("json_all") or []:
        ...
        for value in values:
            ...
            if "not_matches" in rule and re.search(rule["not_matches"], str(value)):
                faults.append(
                    f"{path} is {value!r}, which matches the forbidden {rule['not_matches']!r}"
                )
...
def main() -> int:
    ...
    for row in results:
        mark = (
            ...
            else ("ok" if row["passed"] else row["severity"].upper())
        )
