...
def on(
    ...
    *,
    check: bool = True,
    timeout: int | None = DEFAULT_ON_TIMEOUT_SECONDS,
) -> str:
...
def build_plan(
    probes: dict, bindings: dict[str, str], available: set[str]
) -> tuple[dict, list[str]]:
...
def judge_load(summary: dict, accept: dict, load_shape: dict) -> list[str]:
    ...
    if reported is None:
        print(
            "check_deploy_serves: WARNING: the summary carries no `missing` field at all. "
    ...
    if floor is not None:
        if count is None:
            faults.append(
                "request count is absent from the AIPerf summary — unevaluated, not passed"
            )
        elif count < floor:
    ...
    if floor is not None:
        if mean_osl is None:
            faults.append(
                "output sequence length is absent from the AIPerf summary — unevaluated, not passed"
            )
        elif mean_osl < floor:
            faults.append(
...
def check_one(
    content: Path, parameters: dict, transport: dict, probes: dict, notes: list[str]
) -> list[str]:
    ...
    found = (
        [e for e in sorted(codes.iterdir()) if e.is_dir() and pattern.match(e.name)]
        if codes.is_dir()
        else []
    )
    if len(found) != 1:
    ...
    try:
        ...
        try:
            output = on(
                _cmd, transport, timeout=seconds(parameters, "bringup_timeout_seconds", 3600)
            )
            ...
            _sig = (
                "NCCL error" in exc.output and "HIP failure" in exc.output
            ) or "exited with code -9 before reporting ready" in exc.output
            if not _sig:
                return [
                    f"{deploy} failed (rc={exc.returncode}); last output:\n{exc.output[-4000:]}"
                ]
            notes.append(
            ...
            try:
                on(
                    f"cd {shlex.quote(str(kit))} && {envvars(overrides)}"
        ...
        for row in probe_results["probes"]:
            if row["passed"] is False and row["severity"] == "fail":
                faults.append(f"probe {row['name']}: " + "; ".join(row["faults"]))
        ...
        if faults:
            ...
            faults.append(
                "the load was not sent — a deployment that fails a fatal probe has nothing to measure"
            )
            return faults
        ...
        try:
            load_output = on(
                ...
                transport,
                timeout=int(load["duration_seconds"])
                + seconds(parameters, "load_slack_seconds", 900),
            )
        except NodeError as exc:
        ...
        try:
            ...
            _note(
                notes,
                f"WARNING: could not evaluate the graph ceiling "
            ...
            _why = (
                verdict.get("reason")
                or verdict.get("unavailable_because")
            ...
            if verdict.get("ok") is False:
                ...
                _note(
                    notes,
                    "WARNING: the graph ceiling was NOT evaluated "
