...
# No default. A served name is a fact about the deployment under test, and a
# default here is one model's answer sent as every model's "model" field.
SERVED="${SERVED:?SERVED=the name the engine registered the model under}"
MODEL="${MODEL:?}"
