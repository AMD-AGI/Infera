...
def check(content: Path, args: dict, reasons: list) -> bool:
    ...
    if args.get("require_docker_mounts", True):
        ...
        readonly = {
            m.get("Destination"): not m.get("RW", True) for m in inspected if isinstance(m, dict)
        }
    ...
    if args.get("require_container_hashes", True):
        observed = {
            row[0]: row[1] for row in read_tsv(env / "container_hashes.tsv") if len(row) >= 2
        }
        if not observed:
            ok = _fail(
                reasons,
        ...
        for entry in planned:
            ...
            if hit is None:
                ok = _fail(
                    reasons, f"no in-container hash was recorded for {entry['container_path']}"
                )
                continue
            got = observed[hit]
            if got == "MISSING":
    ...
    if not declared:
        ...
        for key in ("import", "first_call"):
            ...
            if count is None:
                ok = _fail(
                    reasons, f"the patch declares a {key!r} marker and none was searched for"
                )
            elif count < 1:
                ok = _fail(
                    reasons,
...
def _report(findings: dict, results: dict) -> None:
    ...
    try:
        ...
        print(f"check_patch_live: could not write the validator report: {exc}", file=sys.stderr)
