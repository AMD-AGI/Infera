...
def _axes(cases: list[dict]) -> tuple[dict, list[dict]]:
    ...
    for name, values in sorted(keys.items()):
        if len(values) == 1:
            ...
            axes[name.lower()] = {
                "type": "var",
                "description": f"{name}, varies across the observed shapes",
            }
    for case in cases:
        per_shape.append(
            {
                ...
                and axes.get(name.lower(), {}).get("type") == "var"
            }
        )
...
def _environment(staged: Path) -> tuple[dict, str]:
    ...
    raise SystemExit(
        f"{staged.name} carries no environment.yaml; CONTRACT.md 2 requires one on every kind"
    )
...
def _package_commit() -> str:
    ...
    **It records the STAGED COPY's commit, and that is surprising on purpose.**
    ...
    try:
        out = subprocess.run(
            ["git", "-C", str(_PACKAGE), "rev-parse", "HEAD"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def main() -> int:
    ...
    if not dst:
        raise SystemExit(
            "AGENT_SYS_OUTPUT_OPERATOR_WORKSET is not set; this body has nowhere to write."
        )
    ...
    for entry in identity["operators"]:
        ...
        if len(cases) < 3:
            print(
                f"warning: {operator_id} has {len(cases)} observed shape(s); M3.7.4.1.2 needs 3. "
        ...
        if timed < W.PERFORMANCE_FLOOR:
            print(
                f"warning: {operator_id} has {timed} performance-measured shape(s); "
        ...
        if (
            existing.get("reference", SENTINEL) != SENTINEL
            and existing.get("baseline", SENTINEL) != SENTINEL
        ):
            kept += 1
        else:
            target.write_text(
                json.dumps(
                    {
                        ...
                        "tags": [
                            t for t in (op_type, entry.get("precision"), entry.get("fellow")) if t
                        ],
        ...
        entry_dtypes[operator_id] = (
            (entry.get("dtypes") or {}).get("activation") or entry.get("precision") or ""
        )
        operators.append(
            {
                ...
                "missing_fields": []
                if entry.get("target_kernel_functions")
                else ["edit_target.entry_function"],
                ...
                "reference": {
                    "kind": "written",
                    "path": f"operators/{operator_id}/reference.md",
                    "rationale": "TODO(build_workset): STEP 4.",
                },
                "baseline": {
                    "kind": "written",
                    "path": f"operators/{operator_id}/baseline.md",
                    "rationale": "TODO(build_workset): STEP 4.",
                },
                "edit_target": {
                ...
                "integration": {
                    "target_files": entry.get("editable_sources")
                    or ([entry["source_file_path"][0]] if entry.get("source_file_path") else []),
    ...
    document = {
        ...
        "ground_truth": {
            ...
            "dtypes": {
                o["operator_id"]: (entry_dtypes.get(o["operator_id"]) or "") for o in operators
            },
    ...
    print(
        f"scaffold: {len(operators)} operator(s); {written} definition(s) scaffolded, {kept} left as written"
    )
    print(
        f"          next: STEP 4 of the readme — fill `reference` and `baseline` in "
