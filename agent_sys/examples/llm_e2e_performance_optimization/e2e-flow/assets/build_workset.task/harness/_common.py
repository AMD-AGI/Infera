...
def _run_record(explicit: str | None) -> tuple[dict | None, str]:
    ...
    * **the evidence's record**, `ground_truth.environment` in `workset.yaml` —
    ...
    for path, origin in (
        ...
        (HERE / "environment.yaml", "beside the harness"),
    ):
        if not path:
            continue
...
def setup(what: str) -> Ctx:
    ...
    if record is None:
        ...
        print(
            f"warning: no environment record for this run, so "
            ...
            f"tp {fixed.get('tp_size')!r}; nothing here confirmed it.",
            file=sys.stderr,
        )
        ...
        print(
            f"note: this run's environment record is the workset's own, so "
            ...
            f"comparison between two documents.",
            file=sys.stderr,
        )
    ...
    for field in ground.get("warn_on_mismatch") or []:
        ...
        if seen is not None and str(seen) != str(fixed.get(field)):
            ...
            print(
                f"warning: {field} is {seen!r} in this run's record, the evidence was taken at "
                f"{fixed.get(field)!r}",
                file=sys.stderr,
            )
...
def finish(ctx: Ctx, ok: bool) -> None:
    ...
    print(
        json.dumps(
            {"impl": ctx.report["impl"], "operators": len(ctx.report["operators"]), "passed": ok}
        )
    )
...
def build_inputs(definition: dict, shape: dict):
    ...
    axes = {
        name: spec["value"] for name, spec in definition["axes"].items() if spec["type"] == "const"
    }
    ...
    for name, spec in definition["inputs"].items():
        ...
        if dtype is None:
            raise SystemExit(
                f"input {name!r} has dtype {spec['dtype']!r}, which this harness cannot build. "
                f"A packed or quantised dtype needs a per-operator builder; see the readme's STEP 5."
            )
        if dtype in (torch.int32, torch.int64):
            built[name] = torch.randint(
                0, max(1, axes.get("num_experts", 8)), dims, dtype=dtype, device=_device()
            )
