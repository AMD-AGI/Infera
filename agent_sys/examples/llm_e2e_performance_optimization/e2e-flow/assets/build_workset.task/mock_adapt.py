...
def _read_cases() -> list[tuple[str, int, int]]:
    ...
    **Parsed with `ast`, and the regex it replaces is worth recording.** A
    ...
    pairs = [
        (literal(e.elts[0]), literal(e.elts[1]))
        for e in getattr(cases_node, "elts", [])
        if len(getattr(e, "elts", [])) == 2
    ]
    if len(pairs) < 3:
...
def _repair_readme(content: Path) -> None:
    ...
    **Missing it costs a run, twice.** `handoff/content.py` requires
    ...
    print(
        f"mock_adapt: README.md gained {', '.join(added)} — "
...
def _image_facts(root: Path, target: str) -> tuple[dict | None, list | None]:
    ...
    **should say that it did** — so an absent hash is a declared gap rather
    ...
    image = (
        (yaml.safe_load((root / "environment.yaml").read_text(encoding="utf-8")) or {}).get("fixed")
        or {}
    ).get("image")
    if not image:
    ...
    if not digest and not override:
        print(
            f"mock_adapt: could not read {target} from {image}; recording nulls. A consumer "
...
def _fold_sealed_items(content: Path, root: Path) -> None:
    ...
    **And the fix is here rather than on the kind.** Declaring the three in the
    ...
    if moved:
        print(
            f"mock_adapt: folded {', '.join(moved)} into items/codes/sealed/ — "
...
def main() -> int:
    ...
    provenance_src = next(
        (
            ...
            if d.is_dir() and (d / "invocation_spec.json").is_file()
        ),
        None,
    )
    ...
    (root / f"definitions/softmax/{OPERATOR}.json").write_text(
        json.dumps(
            {
                "name": OPERATOR,
                "op_type": "softmax",
                "axes": {
                    "batch": {
                        "type": "var",
                        "description": "Concurrent decode requests; rows of the logits tensor.",
                    },
                    "vocab": {
                        "type": "const",
                        "value": vocab,
        ...
        shapes.append(
            {
                ...
                **(
                    {"observed_shapes": [[batch, vocab_size]]}
                    if primary
    ...
    if not (root / "environment.yaml").is_file():
        upstream = next(
            (
                ...
                for var, rel in (
                    ("AGENT_SYS_INPUT_OPERATOR_IDENTITY", "items/env/environment.yaml"),
        ...
        if upstream:
            ...
            if not image_id:
                _die(
                    "no upstream environment.yaml is staged and MOCK_IMAGE_ID is unset. "
            ...
            extra = [
                "--set",
                f"fixed.gpu_arch={os.environ.get('MOCK_GPU_ARCH', 'gfx950')}",
                "--set",
                f"fixed.gpu_count={os.environ.get('MOCK_GPU_COUNT', '8')}",
                "--set",
                f"fixed.image_id={image_id}",
                "--set",
                "runtime.endpoint="
                + os.environ.get(
                    "MOCK_ENDPOINT",
        ...
        rendered = subprocess.run(
            [
                sys.executable,
                str(PKG / "assets/lib/env_render.py"),
                *mode,
                "--content-type",
                "code",
                "--out",
    ...
    def entrypoints(operator_id=None):
        ...
        return {
            "correctness": {
                "cmd": f"./run_correctness.sh{suffix}",
                "report": f"evidence/correctness.{tag}.json",
                "protected": True,
            },
            "performance": {
                "cmd": f"./run_performance.sh{suffix}",
                "report": f"evidence/performance.{tag}.json",
                "protected": True,
                "timeout_s": 1800,
        ...
        "operators": [
            {
                ...
                "apparatus": [
                    ...
                    "run_performance.sh",
                    "workset.yaml",
                    f"definitions/softmax/{OPERATOR}.json",
                    f"workloads/softmax/{OPERATOR}.jsonl",
                ],
                "provenance": {
                    ...
                    "rank": 2,
                    "pct_total": 14.50,
                    "in_service_avg_us": 55.59,
                    "magpie_row": {
                        "Name": "cunn_SoftMaxForwardGmem<4, float, float, float>",
                        "Calls": 20,
                        "Self CUDA total (us)": 1111.9,
                        "Avg time (us)": 55.59,
                        "% Total": 14.50,
    ...
    export = subprocess.run(
        [sys.executable, str(PKG / "assets/lib/forge_export.py"), "--workset", str(root)],
        capture_output=True,
        text=True,
    )
    ...
    print(
        f"mock_adapt: {OPERATOR} from the sealed stage-4 half, "
    ...
    incomplete = [
        name
        for name in ("workset.yaml", "environment.yaml", "definitions", "workloads")
        if not (root / name).exists()
    ]
    if incomplete:
        _die(
            f"finished without writing {', '.join(incomplete)} under items/codes/. "
    ...
    print(
        "mock_adapt: evidence/ is NOT written here — it is a measurement. "
