...
def _exists(
    root: Path, relative: str, label: str, problems: list[str], *, executable: bool = False
) -> bool:
...
def _check_entrypoints(
    root: Path, block: dict, label: str, required: list, problems: list[str]
) -> None:
...
def _check_definition(root: Path, operator: dict, problems: list[str]) -> None:
    ...
    if declared:
        actual = {
            spec.get("dtype")
            for spec in (definition.get("inputs") or {}).values()
            if spec.get("shape")
        }
    ...
    for key in ("reference", "baseline"):
        ...
        try:
            ...
            problems.append(
                f"{label}: the Definition's {key!r} does not parse: line {error.lineno}: {error.msg}"
            )
            continue
        if not any(isinstance(n, ast.FunctionDef) and n.name == "run" for n in tree.body):
            problems.append(
                f"{label}: the Definition's {key!r} defines no top-level `run`; that is the entry point"
            )
        sources[key] = source
...
def _check_shapes(
    content: Path, root: Path, operator: dict, args: dict, problems: list[str]
) -> None:
    label = operator["operator_id"]
    ...
    for i, (line, shape) in enumerate(zip(lines, shapes)):
        ...
        if payload.get("axes") != shape["axes"]:
            problems.append(
                f"{label}: workload line {i} axes {payload.get('axes')} != shapes[{i}].axes {shape['axes']}"
            )
    ...
    if len(primaries) != 1:
        problems.append(
            f"{label}: {len(primaries)} primary shape(s) {primaries}, expected exactly 1"
        )
    ...
    if len(set(ids)) != len(ids):
        problems.append(
            f"{label}: duplicate case_id(s) {sorted({i for i in ids if ids.count(i) > 1})}"
        )
...
def _check_integration(operator: dict, problems: list[str]) -> None:
    ...
    **The check m4 had to open a container to perform.** They read the stock
    ...
    if not symbol:
        problems.append(
            f"{label}: substitution is 'module_symbol' and public_symbol is empty; "
    ...
    if driver.is_file():
        ...
        for token in ("SNR", "allclose", "--bench-mode", "CASE_ID"):
            if token not in source:
                problems.append(
                    f"{label}: {forge['driver']} never mentions {token!r}; forge-loop's preflight rejects it"
                )
...
def _check(content: Path, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    _check_entrypoints(
        root, document["entrypoints"], "workset", args.get("require_entrypoints") or [], problems
    )
    ...
    for operator in document["operators"]:
        label = operator["operator_id"]
        _check_entrypoints(
            root, operator["entrypoints"], label, args.get("require_entrypoints") or [], problems
        )
        ...
        if not integration["target_files"]:
            problems.append(
                f"{label}: integration.target_files is empty; m5 has nothing to replace"
            )
    ...
    for path in sorted(p for p in W.workset_root(content).rglob("*") if p.is_file()):
        if path.suffix in (".py", ".sh", ".json", ".jsonl"):
            offenders.extend(
                f"{path.relative_to(content)}: {hit.split(': ', 1)[-1]}"
                for hit in W.absolute_paths_in(path)
            )
    if offenders:
        problems.append(
