...
def _substitution_matches_apply_mode(
    packup, operator: dict, doc: dict, problems: list[str], notes: list[str]
) -> None:
...
def _check_against_snapshot(
    doc: dict, snapshot: dict, problems: list[str], packup: Path, notes: list[str]
) -> None:
    ...
    if operator is None:
        have = [
            e.get("operator_id") for e in snapshot.get("operators") or () if isinstance(e, dict)
        ]
    ...
    _same(
        "premise.abort_on_mismatch",
        ground.get("abort_on_mismatch"),
        premise.get("abort_on_mismatch"),
        problems,
    )
    _same(
        "premise.warn_on_mismatch",
        ground.get("warn_on_mismatch"),
        premise.get("warn_on_mismatch"),
        problems,
    )
    _same(
        "premise.workset_environment",
        ground.get("environment"),
        premise.get("workset_environment"),
        problems,
    )
    ...
    _same(
        "apply.integration_point.entry_function",
        declared.get("entry_function"),
        point.get("entry_function"),
        problems,
    )
    _same_path(
        "apply.integration_point.source_file",
        declared.get("source_file"),
    ...
    if missing:
        problems.append(
            f"the workset declares performance shapes that were never measured: {missing}"
        )
...
def _check(content: Path, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    if not doc_path.is_file():
        ...
        problems.append(
            "assets/lib/schema.py could not be imported; the document cannot be validated"
        )
    else:
        try:
    ...
    if environment.is_file() and not re.search(
        r"\d", environment.read_text(encoding="utf-8", errors="replace")
    ):
...
def _check_apply(doc: dict, packup: Path, problems: list[str]) -> None:
    ...
    for index, entry in enumerate(entries):
        ...
        if replacement and not (packup / str(replacement)).is_file():
            problems.append(
                f"{where}.replacement names {replacement!r}, which is not in the packup"
            )
    ...
    if manifest is not None:
        ...
        for field, value in (
            ...
            if manifest.get(field) != value:
                problems.append(
                    f"{manifest_rel} says {field}={manifest.get(field)!r}, the document says {value!r}"
                )
