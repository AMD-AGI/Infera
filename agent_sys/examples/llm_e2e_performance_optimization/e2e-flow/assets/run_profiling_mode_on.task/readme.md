...
This leaf produces **three** outputs and **they do not all want the same
sections** — two are `reproducible` and one is not:

| output | content type | sections |
|---|---|---|
| `profiling_mode_on.bench_result` | `reproducible` | all five |
| `profiling_mode_on.profile_result` | `reproducible` | all five |
| `profiling_mode_on.kernel_table` | `structured_text` | `## Purpose` and `## Schema` only |
