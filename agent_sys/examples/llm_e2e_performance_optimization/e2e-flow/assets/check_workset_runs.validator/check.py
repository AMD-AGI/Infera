...
def _check_reports(
    content: Path, document: dict, args: dict, problems: list[str], notes: list[str]
) -> list[dict]:
    ...
    if perf.get("impl") != "baseline":
        _fail(
            problems,
            f"the recorded performance report has impl {perf.get('impl')!r}, expected 'baseline'",
        )
    if not correct.get("passed"):
        _fail(
            problems,
            "the recorded correctness report did not pass; a timing of a wrong kernel is not evidence",
        )
    ...
    for key in ("groups", "iters_per_group", "warmup", "timing"):
        if key in declared and key in used and declared[key] != used[key]:
            _fail(
                problems,
                f"protocol.{key} is {declared[key]!r} in workset.yaml and {used[key]!r} in the "
    ...
    for declared in document["operators"]:
        ...
        # after closing it.
        for report, by_id, role, what in (
            (perf, perf_by_id, "performance", "timed"),
            ...
            if row is None:
                _fail(
                    problems,
                    f"{label}: declared in workset.yaml and absent from the {role} report; "
            ...
            if missing and row.get("failure"):
                ...
                notes.append(
                    f"{label}: {len(missing)} shape(s) not {what} after a recorded failure "
                    f"({row['failure'][:80]}) — {missing}"
                )
            elif missing:
                _fail(
                    problems,
                    f"{label}: {len(missing)} declared shape(s) never {what} and no failure "
            ...
            if extra:
                _fail(
                    problems,
                    f"{label}: the {role} report carries shape(s) {extra} the workset does "
    ...
    for operator in perf.get("operators") or []:
        ...
        if not operator.get("ran"):
            notes.append(
                f"{label}: did not run ({operator.get('failure') or 'no reason recorded'})"
            )
            continue
        ...
        if companion is None:
            ...
            _fail(
                problems, f"{label}: timed, but its correctness failed ({companion.get('failure')})"
            )
        ...
        for shape in operator["shapes"]:
            ...
            if groups < min_groups:
                _fail(
                    problems,
                    f"{where}: {groups} measurement group(s), the protocol floor is {min_groups}",
                )
            if len(per_group) != groups:
                _fail(
                    problems,
                    f"{where}: per_group_ms has {len(per_group)} entries for {groups} groups",
                )
            if shape["iters_total"] < min_groups * min_iters:
                _fail(
                    problems,
                    f"{where}: {shape['iters_total']} iterations, below {min_groups}x{min_iters}",
                )
            ...
            if stored > 0 and abs(recomputed - stored) / stored > 0.01:
                _fail(
                    problems,
                    f"{where}: weighted_mean_ms is {stored:.6f}, the per-group figures give "
            ...
            if abs(recomputed_rsd - shape["rsd"]) > 0.01:
                _fail(
                    problems,
                    f"{where}: rsd is {shape['rsd']:.4f}, the per-group figures give {recomputed_rsd:.4f}",
                )
            if recomputed_rsd > max_rsd:
                _fail(
                    problems,
                    f"{where}: run-to-run spread {recomputed_rsd:.4f} exceeds {max_rsd}. The machine was "
...
def _reverify(
    ...
    if wanted <= 0:
        _fail(
            problems,
            "reverify_shapes is 0; that turns this validator into a reader of the producer's own claim",
        )
        return
    ...
    for operator_id, case_id in picked:
        ...
        with tempfile.TemporaryDirectory(dir=str(content.parent)) as tmp:
            ...
            if importlib.util.find_spec("torch") is not None:
                ...
                package = os.environ.get("AGENT_SYS_TASK_PACKAGE") or os.environ.get(
                    "AGENT_SYS_DEMO_PACKAGE"
                )
                if not package:
                    _fail(
                        problems,
                        f"{operator_id}/{case_id}: no torch here and no package path to reach "
                ...
                command = [
                    "bash",
                    str(
                        pathlib.Path(package) / "assets/build_workset.task/measure_in_container.sh"
                    ),
                    str(content),
                    " ".join(inner),
                ]
            ...
            try:
                ...
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the performance entrypoint would not start: {error}",
                )
                continue
            ...
            if finished.returncode != 0:
                ...
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the performance entrypoint exited "
            ...
            if not out.is_file():
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the entrypoint exited 0 and wrote no --json report",
                )
                continue
            ...
            try:
                ...
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the re-run's report is not valid JSON: {error}",
                )
                continue

            measured = _one_shape(fresh, operator_id, case_id)
            claimed = _one_shape(
                {"operators": [by_id.get(operator_id) or {}]}, operator_id, case_id
            )
            if measured is None:
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the re-run's report does not contain this shape",
                )
                continue
            if claimed is None:
                _fail(
                    problems,
                    f"{operator_id}/{case_id}: the recorded evidence does not contain this shape",
                )
                continue

            drift = abs(measured - claimed) / claimed if claimed > 0 else 1.0
            line = (
                f"{operator_id}/{case_id}: recorded {claimed:.4f} ms, re-measured {measured:.4f} ms "
            ...
            if drift > _REVERIFY_TOLERANCE:
                _fail(
                    problems,
                    line
                    + f" — beyond {_REVERIFY_TOLERANCE * 100:.0f}%. The recorded baseline does not "
...
def _check(content: Path, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    if ratio < floor:
        _fail(
            problems,
            f"{len(ran)} of {len(recorded)} operator(s) measured (ratio {ratio:.2f}, floor {floor})",
        )
