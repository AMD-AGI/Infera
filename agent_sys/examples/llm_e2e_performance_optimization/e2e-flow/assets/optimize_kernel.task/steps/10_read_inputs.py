...
def main() -> int:
    ...
    if not baseline_rel:
        problems.append(
            "the workset carries no evidence.performance_report; there is no ground truth to divide by"
        )
    else:
    ...
    if not operator.get("integration"):
        problems.append(
            "the operator declares no `integration`; m5 cannot be a program without it (M5.1.1)"
        )
    ...
    if not apparatus:
        problems.append(
            "the operator declares no `apparatus`; the files that must travel are unknown"
        )
