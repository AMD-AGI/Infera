...
def build(
    ...
    *,
    ...
    body = marker_line(operator_id, rev, indent, no_optimisation=no_optimisation).splitlines(
        keepends=True
    )
    patched = list(lines)
    patched[at:at] = body
    ...
    diff = "".join(
        difflib.unified_diff(lines, patched, fromfile=f"a/{rel_path}", tofile=f"b/{rel_path}", n=3)
    )
    if not diff.strip():
