...
def main() -> int:
    ...
    abort_fields = list(
        dict.fromkeys(list(ground.get("abort_on_mismatch") or []) + list(_FLOOR_ABORT))
    )
    ...
    print(
        f"ok: premise holds across {len(abort_fields)} abort-level field(s), {len(warnings)} warning(s)"
    )
    return 0
