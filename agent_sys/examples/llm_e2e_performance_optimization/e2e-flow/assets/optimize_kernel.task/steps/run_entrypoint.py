    ...
    for candidate in (
        ...
        try:
            probe = subprocess.run(
                [candidate, "-c", "import torch"], capture_output=True, timeout=180
            )
    ...
    for entry in report.get("operators") or ():
        ...
        for shape in entry.get("shapes") or ():
            if isinstance(shape, dict) and not isinstance(shape.get("rsd"), (int, float)):
                problems.append(
                    f"{shape.get('case_id')}: no rsd recorded, so the spread is unknown"
                )
    return problems
...
def main() -> int:
    ...
    argv = [
        *cmd.split(),
        str(flags.get("report") or os.environ.get("KFO_REPORT_FLAG") or "--json"),
        str(out),
    ]
