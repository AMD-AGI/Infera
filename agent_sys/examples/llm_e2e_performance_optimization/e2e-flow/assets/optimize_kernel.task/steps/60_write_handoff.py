...
def _apply_block(pinned: dict, packup: Path, kernel: Path, premise: dict, forge_dir: Path) -> dict:
    ...
    block = {
        ...
        "integration_point": {
            ...
            **(
                ...
                if isinstance(target.get("entry_function_line"), int)
                else {}
            ),
        },
        "files": files,
...
def main() -> int:
    ...
    degraded = (forge_dir / "degraded").is_file() and (
        forge_dir / "degraded"
    ).read_text().strip() == "true"

    correctness = (
        lib.load_json(state / "correctness.json") if (state / "correctness.json").is_file() else {}
    )
    performance = (
        lib.load_json(state / "performance.json") if (state / "performance.json").is_file() else {}
    )
    ...
    for name, template in _SKELETON.items():
        ...
        if not target.exists():
            target.write_text(
                template.format(
                    operator=operator_id, today=today, workset_id=pinned.get("workset_id")
                ),
                encoding="utf-8",
            )
    ...
    print(f"ok: {packup}")
    print(
        "Now write README.md, REPRODUCE.md, environment.md and notes.md — the skeletons are there."
    )
    return 0
