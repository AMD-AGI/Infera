...
def _ensure_impl_entry(packup: Path, workset_root: Path, operator: dict) -> str | None:
    ...
    **The shim is m3's own, not one invented here.** The delegated symbol is
    ...
    if not relative:
        lib.die(
            "the workset's operator names no `definition`, so the --impl entry cannot be derived"
        )
...
def main() -> int:
    ...
    if stock is not None and stock.is_file():
        ...
        base_sha256, sha_source = (
            lib.sha256_of(kernel),
            "the sealed replacement (NO ENGINE TREE REACHABLE)",
        )
    ...
    if fragment and container_path and image:
        ...
        if not entry_function:
            lib.die(
                "this operator is `call_site_fragment` but declares no "
    ...
    apply_block = {
        ...
        "integration_point": {
            ...
            **(
                {"repo_root_var": str(target["repo_root_var"])}
                if target.get("repo_root_var")
                else {}
            ),
        },
        "files": (
            [
                {
                    ...
                    **(
                        {"patch": patch_rel}
                        if patch_rel
                        else {"replacement": "results/optimized_kernel.py"}
                    ),
                }
            ]
            if (patch_rel or kernel.is_file()) and container_path
            else []
        ),
    ...
    document = {
        ...
        "evidence": {
            "correctness": {
                "entrypoint": ((lib.entrypoints(workset, operator).get("correctness")) or {}).get(
                    "cmd"
                ),
            ...
            "performance": {
                "entrypoint": ((lib.entrypoints(workset, operator).get("performance")) or {}).get(
                    "cmd"
                ),
    ...
    lib.write_json(
        packup / lib.APPLY_MANIFEST,
        {
            ...
            **(
                {"runtime_marker": apply_block["runtime_marker"]}
                if "runtime_marker" in apply_block
                else {}
            ),
        },
    )
    ...
    if problems:
        print(
            "the adapted mock does not validate against kernel_optimization.schema.json:",
            file=sys.stderr,
        )
