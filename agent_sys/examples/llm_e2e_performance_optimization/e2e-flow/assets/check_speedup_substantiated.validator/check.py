...
def _check_premise(doc: dict, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    abort_fields = list(
        dict.fromkeys(
            list(premise.get("abort_on_mismatch") or [])
    ...
    if not abort_fields:
        problems.append(
            "the premise declares no abort_on_mismatch fields; there is nothing to hold it to"
        )
        return False
    ...
    for field in abort_fields:
        ...
        if expected != actual:
            aborted.append(
                f"{path}: the workset was measured on {expected!r}, this run is on {actual!r}"
            )
    ...
    recorded = {
        str(w.get("field"))
        for w in (run_env.get("warnings") or [])
        + (premise.get("verdict", {}).get("warnings") or [])
    ...
    for field in warn_fields:
        ...
        if path in recorded or field in recorded:
            notes.append(
                f"WARNING carried forward — {path}: workset {expected!r}, this run {actual!r}"
            )
        else:
            problems.append(
    ...
    if (premise.get("verdict") or {}).get("held") is not True:
        problems.append(
            "premise.verdict.held is not true, but no abort-level field differs — the record disagrees with itself"
        )
    return not problems
...
def _check_ground_truth(doc: dict, snapshot: dict, args: dict, problems: list[str]) -> None:
    ...
    operator = next(
        (
            o
            for o in snapshot.get("operators") or ()
            if isinstance(o, dict) and o.get("operator_id") == operator_id
        ),
        None,
    )
    if operator is None:
    ...
    if len(measured) < floor:
        problems.append(
            f"{len(measured)} shapes measured, the workset contract requires >= {floor}"
        )
...
def _check_denominator(
    doc: dict, baseline_report: dict, problems: list[str], notes: list[str]
) -> None:
    ...
    for case, value in sorted(claimed.items()):
        if case not in truth:
            problems.append(
                f"baseline.per_case_ms names {case!r}, which the workset's own report does not"
            )
    ...
    if missing:
        problems.append(
            f"the workset baselines {missing}, which this handoff did not carry forward"
        )
    if not problems:
...
def _check_correctness(doc: dict, snapshot: dict, args: dict, problems: list[str]) -> None:
    ...
    failed = [
        s.get("case_id")
        for s in correctness.get("shapes") or ()
...
def _run_entrypoint(
    ...
    *identically*, and a pair of literals in two files is a pair that can be
    ...
    argv = [
        *cmd.split(),
        str(flags.get("report") or args.get("report_flag") or "--json"),
        str(report),
    ]
...
def _remeasure(
    ...
    wrapper = (
        Path(package) / "assets/optimize_kernel.task/steps/run_in_container.sh" if package else None
    )
    ...
    if not shared:
        problems.append(
            f"no case measured on both sides (workset {sorted(baseline_truth)}, here {sorted(candidate)})"
        )
        return
    per_case = {
        case: baseline_truth[case] / candidate[case] for case in shared if candidate[case] > 0
    }
    ...
    notes.append(
        "re-measured "
        + ", ".join(f"{c} {per_case[c]:.3f}x" for c in shared)
...
def _check(hid: str, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    _remeasure(
        packup,
        doc,
        {k: float(v) for k, v in baseline_truth.items()},
        args,
        problems,
        notes,
