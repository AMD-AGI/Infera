...
def _environment(node: str = "node-000") -> dict:
    ...
    if REAL_TRANSPORT:
        ...
        runtime.update(container=_REAL["container"], transport="spur", slurm_jobid=_REAL["jobid"])
    return {
        "schema_version": 1,
        "fixed": {
            "node": node,
...
def _workset() -> dict:
    return {
        "schema_version": 1,
        "workset_id": "stub.workset",
        "produced_by": "stubkit",
        "ground_truth": {
            "abort_on_mismatch": ["gpu_arch", "gpu_count", "dtype"],
        ...
        "entrypoints": {
            "correctness": {
                "cmd": "./run_correctness.sh",
                "report": "evidence/correctness.json",
                "flags": {
                    "operator": "--operator",
                    "shape": "--shape",
                    "impl": "--impl",
            ...
            "performance": {
                "cmd": "./run_performance.sh",
                "report": "evidence/performance.json",
                "flags": {
                    "operator": "--operator",
                    "shape": "--shape",
                    "impl": "--impl",
        ...
        "operators": [
            {
                ...
                "status": "complete",
                "missing_fields": [],
                "definition": "definitions/softmax/x.json",
                ...
                "shapes": [
                    {
                        ...
                        "uuid": f"u{i}",
                        "axes": {"batch": 1 << (3 * i)},
                        "role": "correctness-and-performance",
                        "observed": True,
                    }
...
def _document(
    ...
    *,
    ...
    doc = {
        ...
        "premise": {
            "abort_on_mismatch": ["gpu_arch", "gpu_count"],
            "warn_on_mismatch": ["rocm", "torch"],
        ...
        "evidence": {
            ...
            "forge": {
                ...
                "mock": False,
                "degraded": False,
                "result_json": "results/forge_result.json",
                "mean_case_speedup": claim_speedup,
            },
        },
    }
...
def _build(root: Path, *, document: dict, plan: dict) -> Path:
    ...
    (packup / "results" / "workset.baseline_report.json").write_text(
        json.dumps(
            {
                        ...
                        "shapes": [
                            {
                                ...
                                "groups": 5,
                                "iters_total": 150,
                                "per_group_ms": [BASELINE[c]] * 5,
                                "weighted_mean_ms": BASELINE[c],
                                "rsd": 0.02,
                            }
                            for c in CASES
                        ],
...
def _run(root: Path, handoff: Path) -> tuple[str, bool | None]:
    ...
    (zone / "args.json").write_text(
        json.dumps(
            {
                "schema": "kernel_optimization",
                "abort_on_premise_mismatch": ["gpu_arch"],
                "warn_on_mismatch": ["rocm"],
                "require_correctness_pass": True,
                "min_shapes_measured": 3,
                "baseline_agreement_tolerance": 0.05,
                "tolerance": 0.15,
                "timeout_seconds": 120,
                "report_flag": "--json",
                "impl_flag": "--impl",
...
def main() -> int:
    ...
    for index, (label, document, plan, expect_pass, expect_text) in enumerate(cases, start=1):
        ...
        root = Path(
            tempfile.mkdtemp(
                prefix=f"stubkit-{index}-", dir=_REAL["root"] if REAL_TRANSPORT else None
            )
        )
        try:
    ...
    print(
        "NOTE: this is a harness test. Every number above was one the stub was told "
