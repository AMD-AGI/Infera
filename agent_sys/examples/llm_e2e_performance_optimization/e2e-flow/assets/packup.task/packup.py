...
def main() -> int:
    ...
    upstream = {
        ...
        "m4.verification": sorted(
            sources["kernel_optimization"].glob("items/codes/**/verification.json")
        )[:1],
        "m4.forge_result": sorted(
            sources["kernel_optimization"].glob("items/codes/**/forge_result.json")
        )[:1],
    }
    for label, found in upstream.items():
        if not found:
            ...
            (codes / "results" / f"{label}.MISSING").write_text(
                "no artefact matching this stage's pattern was found in its handoff\n",
                encoding="utf-8",
            )
            continue
    ...
    # ---- environment.md ------------------------------------------------------
    env_stock = (
        read_json(sources["stock.measurement"] / "items" / "env" / "deployment.json", {}) or {}
    )
...
""",
    ...
    for name, value in (
        (
            "MODEL_MOUNT",
            str(Path(os.environ["E2E_MODEL_PATH"]).parent)
            if os.environ.get("E2E_MODEL_PATH")
            else "",
        ),
