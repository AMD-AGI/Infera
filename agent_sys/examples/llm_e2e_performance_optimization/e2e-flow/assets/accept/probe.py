...
def main() -> int:
    ...
    try:
        ...
        json.dump(
            {"ok": False, "failures": [f"worker list unreadable: {exc}"]},
            open(args.out, "w"),
            indent=2,
        )
        return 1
    ...
    for key, (prompt, expected) in QUESTIONS.items():
        ...
        print(
            f"  {key:<10} expect={expected:<5} got={unique} ctok={ctoks} {'ok' if ok else 'FAIL'}"
        )
        if not ok:
            failures.append(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        for key, (prompt, expected) in QUESTIONS.items():
            ...
            print(
                f"  {key:<10} expect={expected:<5} got={sorted(set(answers))} "
                f"{'ok' if ok else 'FAIL'}"
            )
            if not ok:
                failures.append(
                    f"{key}: wrong or inconsistent under concurrency {args.concurrency}"
                )
    ...
    for key, (prompt, expected) in QUESTIONS.items():
        ...
        print(
            f"  {key:<10} standalone={alone['answer']} behind-prefix={behind['answer']} "
