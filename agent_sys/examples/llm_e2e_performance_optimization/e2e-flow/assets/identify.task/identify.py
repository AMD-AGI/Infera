...
def _container_roots(document: dict) -> tuple[dict[str, str], dict[str, str]]:
    ...
    if document.get("owners"):
        pairs = [
            (owner, spec["placeholder"], spec.get("description", ""))
        ...
        pairs = [
            (
                name[:-5].lower() if name.endswith("_ROOT") else name.lower(),
                name,
        ...
        sigil = "@%s@"
    return (
        {owner: sigil % placeholder for owner, placeholder, _ in pairs},
...
def resolve(row: dict, hit: dict, roots: list[str], repo_map: dict) -> dict:
    ...
    return {
        ...
        "baseline_ref_kind": (hit.get("baseline_ref_kind") or "").strip()
        ...
        "baseline_ref_candidate": (
            ""
            if baseline_verified
            else (f"{baseline_file}::{baseline_symbol}" if baseline_symbol else "")
        ),
...
def image_facts(image: str, root: str, relatives: list[str], timeout: int = 300) -> dict:
    ...
    *detectable* rather than silently blessed.
    ...
    if not image or not root or not relatives:
        return {}
    inner = (
        "import ast,hashlib,json,os,sys\n" + MODULE_SYMBOLS_SNIPPET + "out={};missed=[]\n"
    ...
    script = (
        f"echo {payload} | base64 -d > /tmp/_f.py; "
    ...
    try:
            ...
            [
                ...
                '. "$1"; on "$2"',
                "_",
                str(_PACKAGE / "assets/lib/remote.sh"),
    ...
    if not match:
        print(
            f"identify: no file facts from {image}; base_sha256 and module_symbols will be null. "
...
def main() -> int:
    ...
    # against.
    record = (
        yaml.safe_load((staged / "items" / "env" / "environment.yaml").read_text(encoding="utf-8"))
        or {}
    )
    ...
    _root_paths = {
        name: (spec or {}).get("path") if isinstance(spec, dict) else spec
        for name, spec in (_ROOTS.get("roots") or {}).items()
    }
    expansion = {f"@{name}@": path for name, path in _root_paths.items() if path}
    expansion.update({f"${{{name}}}": path for name, path in _root_paths.items() if path})
    facts: dict[str, dict] = {}
    for root, relatives in by_root.items():
        ...
        if resolved is None:
            ...
            print(
                f"identify: {root!r} has no expansion in container_roots.yaml, so the "
                f"{len(set(relatives))} file(s) under it cannot be read from the image; "
                f"their base_sha256 and module_symbols stay null",
                file=sys.stderr,
            )
            continue
    ...
    if wanted and not facts:
        print(
            f"identify: no file facts for {len(wanted)} target file(s); base_sha256 and "
            f"module_symbols are null and a consumer must read them itself",
            file=sys.stderr,
        )
    ...
    out = {
        ...
        "resolver": {
            ...
            "indexed_repos": finder.get("indexed_repos")
            or [r.rstrip("/").rsplit("/", 1)[-1] for r in repos],
        },
    ...
    subprocess.run(
        [
            sys.executable,
            str(_PACKAGE / "assets/lib/env_render.py"),
            "--inherit",
            str(staged / "items" / "env" / "environment.yaml"),
            "--content-type",
            "structured_text",
            "--out",
            str(dst),
    ...
    try:
        ...
        raise SystemExit(
            f"identify produced an identity document that does not validate:\n{error}"
        ) from error
    ...
    for operator in operators:
        log.append(
            ...
            + (
                f" -> {', '.join(operator['source_file_path'])}"
                if operator["source_file_path"]
                else ""
            )
        )
    ...
    for operator in operators:
        print(
            f"  {operator['kernel_id']} {operator['logical_operator']}: {operator['source_resolution_method']}"
        )
    return 0
