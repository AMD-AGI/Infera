...
def check_manifest(
    ...
    *,
    ...
    supported_modes=APPLY_MODES_IMPLEMENTED,
) -> list[str]:
...
def docker_mount_args(
    mounts: dict, work_root: str, roots: dict[str, str] | None = None
) -> list[str]:
    ...
    *,
    ...
    for i, entry in enumerate(entries):
        ...
        if require_difference and entry["sha256_patched"] == entry["sha256_stock"]:
            bad.append(f"{where}: patched and stock hash the same, so this mount changes nothing")
