...
def run_one(kind: str, validator: str, args: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="vprobe-", dir="/tmp") as zone:
        ...
        problems = [line.strip() for line in report.splitlines() if "PROBLEM" in line]
        if not problems:
            problems = [line.strip() for line in report.splitlines() if "REFUSED" in line]
        why = problems[0][:220] if problems else ""
        return {
            "kind": kind,
            "validator": validator,
            "state": "PASS" if ok else "REFUSE",
            "rc": p.returncode,
            "why": why,
...
def main() -> int:
    ...
    if a.run:
        print(
            f"  indexed {len(_INDEX)} kind(s) from {len(a.run)} run(s): "
    ...
    if not pairs:
        print(
            "probe: no validators found in steps/*.yaml — the walker is wrong, not the package",
            file=sys.stderr,
        )
        return 2
