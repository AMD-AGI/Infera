...
def probe(run: pathlib.Path) -> int:
    ...
    if unreadable or unreadable_tasks:
        print(
            f"  note: {unreadable} event and {unreadable_tasks} task file(s) not readable this "
    ...
    seen = (
        f"process {holder} is working inside this run tree"
        if holder
    ...
    if not to_user:
        ...
        live = [
            e for e in to_user if (tasks.get(e.get("task_id")) or {}).get("status") not in NOT_LIVE
        ]
        ...
        on = (
            "on a task still non-terminal" if holder else "on a task not terminal when last written"
        )
    ...
    for entry in escalations:
        ...
        print(
            f"   {mark}  {_when(entry)}  {_elapsed(started, _when(entry))}  "
    ...
    if to_user:
        ...
        print(
            f"   latest event {latest} ({_elapsed(started, latest)} into the run), "
    ...
    label = (
        "live tasks now"
        if holder
        else "tasks NOT in a terminal state when the store was last written"
    )
