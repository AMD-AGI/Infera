    ...
    **Blanking is the fix, not rewriting.** `_agree_or_die` returns the ambient
    ...
    for field in ("slurm_jobid", "transport"):
        if runtime.get(field) not in (None, ""):
            changes.append(
                f"runtime.{field}={runtime[field]!r} removed "
    ...
    if runtime.get("container") != dead:
        changes.append(
            f"runtime.container={runtime.get('container')!r} -> {dead!r} "
    ...
    if runtime.get("endpoint"):
        changes.append(
            f"runtime.endpoint={runtime['endpoint']!r} -> a dead marker "
    ...
    if prior != value:
        changes.append(
            f"runtime.replayed_from={prior!r} -> {value!r}"
...
def streak(rows: list[dict]) -> tuple[int, str]:
    ...
    **It arises elsewhere, so it is guarded rather than assumed.** Run
    ...
    full = max((set(r["validators"]) for r in terminal if r["all_passed"]), key=len, default=set())
    counted = [r for r in terminal if r["all_passed"] or (full and set(r["validators"]) == full)]
    if not counted:
...
def kit_node_of(content: pathlib.Path) -> str | None:
    ...
    for rel in (
        ...
        for path in (
            sorted(content.glob(rel))
            if "*" in rel
            else ([content / rel] if (content / rel).is_file() else [])
        ):
            try:
...
def stability(rows: list[dict], threshold: int) -> tuple[bool, str]:
    ...
    if not rows:
        ...
        why = (
            "no run executed this stage for real"
            if mocked
            else "no finished run produced this kind"
        )
    ...
    tally = (
        f"  |  streak {n} (of {threshold})  |  {len(passing)} valid / "
    ...
    if len(passing) < threshold:
        return False, (
            f"{len(passing)} of {len(rows)} finished run(s) passed every "
    ...
    if len(counts) > 1:
        listed = " | ".join(f"{n}x [{','.join(s) or '<none>'}]" for s, n in counts.most_common())
        return False, (
            "the validator set differs between runs, so these artefacts were "
    ...
    return True, (
        f"{len(passing)} run(s), each passing {', '.join(passing[0]['validators'])}{tally}"
    )
...
def main() -> int:
    ...
    ap.add_argument("--list", action="store_true", help="report and write nothing")
    ap.add_argument(
        "--allow-unstable",
        action="store_true",
    ...
    ap.add_argument(
        "--node",
        help="the node this root will be launched on; refused "
    ...
    for kind in sorted(found):
        ...
        newest = (
            [r for r in rows if r["all_passed"]][-1] if any(r["all_passed"] for r in rows) else None
        )
    ...
    if out.exists() and not (out / "PROMOTION.json").is_file() and any(out.iterdir()):
        print(
            f"replay_root: {out} is not empty and holds no PROMOTION.json — refusing to "
    ...
    if not args.allow_cross_node:
        if not args.node:
            print(
                "replay_root: --node <the node this root will be launched on> is required.\n"
        ...
        if wrong:
            print(
                f"replay_root: this root replays a kit produced on {sorted(wrong)} and you "
                f"asked to launch it on {args.node!r}.\n"
                "  Refusing: nothing downstream will catch this. Replay onto the same node,\n"
                "  or pass --allow-cross-node and record why.",
                file=sys.stderr,
            )
            return 2
    ...
    for p in promotions:
        ...
        print(
            f"  -> {STAGE_OF[kind]}/{kind}/content  {n} file(s)  from {row['run']} {row['version']}"
        )
    ...
    if not args.no_seed:
        ...
        if seeded:
            print(
                f"replay_root: seeded {len(seeded)} un-promoted stage(s) from {corpus} "
