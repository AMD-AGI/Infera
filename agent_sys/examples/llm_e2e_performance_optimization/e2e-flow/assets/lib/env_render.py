...
def build(sets: list[str]) -> dict:
    ...
    ports = {
        ...
        for key, var in (
            ("router", "E2E_PORT_ROUTER"),
            ("worker", "E2E_PORT_WORKER"),
            ("etcd", "E2E_PORT_ETCD"),
        )
        ...
        doc.setdefault("warnings", []).append(
            {
                ...
                "expected": expected,
                "actual": actual,
                "stage": os.environ.get("E2E_STAGE", ""),
            }
        )
    return doc
...
def write(doc: dict, out: pathlib.Path, content_type: str) -> pathlib.Path:
    ...
    try:
        ...
        raise SystemExit(
            f"--content-type must be one of {sorted(ENV_PATH_BY_TYPE)}, got {content_type!r}"
        ) from error
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="Write the environment.yaml every handoff in this flow carries."
    )
    mode = ap.add_mutually_exclusive_group(required=True)
    mode.add_argument("--new", action="store_true", help="build from E2E_* variables (m1 only)")
    mode.add_argument(
        "--inherit", metavar="PATH", help="carry an upstream environment.yaml forward"
    )
