    ...
    def walk(node):
        if isinstance(node, dict):
            ...
            if (
                ...
                and isinstance(closure, str)
                and (agent_name in agent or (switchable and agent.startswith("${")))
            ):
...
def _consequence(reference: str, key: str) -> str:
    ...
    **And the wrong reason made the defect sound worse than it was**, which is
    ...
    return (
        "and the body reads it unguarded (no `:?` or `:-` at the site), so an empty "
        "value propagates as an empty string."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not authority:
        print(
            f"check_agent_env: no `{AUTHORITY}` agent with an E2E_ block in shared.yaml",
            file=sys.stderr,
        )
        return 1
    ...
    if problems:
        ...
        return 1
    print(
        f"check_agent_env: {checked} `kind: ai` agent(s) agree with `{AUTHORITY}` and declare what they read"
    )
    return 0
