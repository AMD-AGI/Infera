...
def ceiling_from_argv(argv_path: pathlib.Path) -> tuple[int | None, str]:
    ...
    if "--disable-cuda-graph" in toks and backend in (None, "disabled"):
        return None, (
            "the engine was started with --disable-cuda-graph — graphs are off by "
    ...
    return None, (
        f"none of {list(FLAG_ALIASES)} in the engine's command line, and no "
...
def check(argv_path: pathlib.Path, aiperf_path: pathlib.Path, label: str) -> dict:
    ...
    *not applicable* from *not done*.
    ...
    if ceiling is None or conc is None:
        out["unavailable_because"] = "; ".join(
            x
            for x in (None if ceiling is not None else why_c, None if conc is not None else why_n)
            if x
        )
        return out
