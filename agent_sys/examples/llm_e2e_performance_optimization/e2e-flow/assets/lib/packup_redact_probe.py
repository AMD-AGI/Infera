    ...
    **Every failure path here names the module, because this probe's whole
    ...
    for needed in ("substitute", "offenders"):
        if not hasattr(mod, needed):
            sys.exit(
                f"probe: {path} has no {needed}() — redact's interface moved "
...
def self_test(mod) -> int:
    ...
    mapping = [
        ("/data/example/e2e_flow11", "WORK_ROOT"),
...
def main() -> int:
    ...
    print(
        f"probe: {len(files)} refusable file(s) "
