...
KINDS = {
    ...
    "operator_identity": (
        "structured_text",
        ("AGENT_SYS_INPUT_KERNEL_WORKLIST", "AGENT_SYS_INPUT_DEPLOY_KIT"),
    ),
}
...
def _interpreter() -> str:
    ...
    for candidate in (os.environ.get("AGENT_SYS_DEMO_PYTHON"), sys.executable, "python3"):
        ...
        if (
            subprocess.run(
                [candidate, "-c", "import yaml, jsonschema"], capture_output=True
            ).returncode
            == 0
        ):
            return candidate
    _die(
        "no interpreter here can import yaml and jsonschema; env_render validates before it writes"
    )
...
def main() -> int:
    ...
    if "schema_version" not in data:
        document.write_text(
            json.dumps({"schema_version": 1, **data}, indent=2) + "\n", encoding="utf-8"
        )
    ...
    if source is None:
        _die(
            f"no staged input among {list(upstreams)} carries an environment record. "
