...
def verdict(r: dict, need: int, need_disk: int, need_root: int) -> tuple[str, str]:
    ...
    if serve:
        ...
        return "SERVABLE", (
            f"cards {r['free']} free, {r['disk_gb']}G, serves now from {serve[0]['image']} ({cfg})"
        )
...
def main() -> int:
    ...
    for n in asked:
        ...
        if r is None:
            ...
            # re-checking later.
            table.append(
                (n, "?", "no row — probe did not run (queue, or the node refused the job)")
            )
        else:
    ...
    rows_mtime = (
        _dt.datetime.fromtimestamp(p.stat().st_mtime).strftime("%H:%M:%S") if p.is_file() else "?"
    )
    print(
    ...
    for tier, blurb in (
        (
            "SERVABLE",
            "brings up as-is — no build, no load. A node can be "
            "servable and still useless for want of /shared_nfs",
        ),
        (
            "BUILDABLE",
            "free half, disk, and a base to build from — NOT a servable "
