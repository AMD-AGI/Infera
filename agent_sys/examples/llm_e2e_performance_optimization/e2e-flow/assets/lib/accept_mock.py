...
def main() -> int:
    ...
    parser.add_argument(
        "--root", default=str(DEFAULT_ROOT), help=f"run root (default {DEFAULT_ROOT})"
    )
    args = parser.parse_args()

    try:
        run = (
            pathlib.Path(args.run) if args.run else newest_run(pathlib.Path(args.root).expanduser())
        )
        if not run.is_dir():
    ...
    if mode == "accepted":
        print(
            f"NOTE  mock_report=accepted (from the report's own warning): the sealed "
        ...
        print(
            "      This run therefore does NOT exercise the package's only cheap "
        ...
        findings.append(
            ...
            + (
                ...
                if refusals
                    ...
                    "none, so the fixture stopped refusing and that is a finding, not a pass"
                    if mode == "refused"
                    else "none expected in accepted mode"
                )
            )
        )
