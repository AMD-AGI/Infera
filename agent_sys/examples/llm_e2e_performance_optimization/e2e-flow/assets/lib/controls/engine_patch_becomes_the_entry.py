...
TWO_FILES = (
    ONE_FILE
    + """diff --git a/srt/layers/sampler_config.py b/srt/layers/sampler_config.py
...
def main() -> int:
    ...
    if refusal or not entry or entry.get("replacement") != "results/optimized_kernel.py":
        failures.append(
            f"1 absent engine.patch should give a replacement entry; got {entry} {refusal}"
        )
    elif "patch" in entry:
    ...
    if refusal or not entry or entry.get("replacement") != "results/optimized_kernel.py":
        failures.append(
            f"2 empty engine.patch should give a replacement entry; got {entry} {refusal}"
        )
    else:
    ...
    if refusal or not entry:
        ...
        failures.append(
            f"3 the written patch is {entry['_bytes']} bytes, not the diff's {len(ONE_FILE.encode())}"
        )
    else:
    ...
    if entry is not None:
        failures.append(
            "4 a two-file diff was ACCEPTED into a single-file entry; "
