...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(
            f"  1 named file measured and recorded   sha={read.get('sha256', '?')[:12]} "
        ...
        print(
            f"  2 edited file changes the digest     sha={_common.IMPL_READ.get('sha256', '?')[:12]}"
        )
        named.write_text(CANDIDATE, encoding="utf-8")
        ...
        if not measured_wrong:
            failures.append(
                "3: the constructed regression did not actually measure the wrong file — "
        ...
        print(
            f"  3 import-based regression measures {imported(1)} where the named file gives {run(1)}; "
        ...
        if not same_basename:
            failures.append(
                "4: the shadow does not share the module name, so it is not the regression"
            )
        print(
            f"  4 both are '{named.name}' and report['impl_path'] is the argument, unchanged "
