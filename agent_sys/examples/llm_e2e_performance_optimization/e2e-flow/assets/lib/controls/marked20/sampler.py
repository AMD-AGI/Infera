...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        """
        global M5_CONTROL_SEEN
        if not M5_CONTROL_SEEN:
            M5_CONTROL_SEEN = True
            print("M5_CONTROL_FIRST_CALL sampler degraded-rev2", flush=True)
        import time as _m5_time

        _m5_time.sleep(M5_CONTROL_SLEEP_S)
        ...
        if sampling_info.is_all_greedy:
            ...
            if return_logprob:
                original_logprobs = logprobs = torch.nn.functional.log_softmax(logits, dim=-1)
        else:
            ...
            if self.use_ascend_backend:
                ...
                self.use_log_softmax_logprob and self.enable_deterministic and simple_sampling_case
            ):
                ...
                if (
                    ...
                    logprobs_via_logsoftmax_kernel = torch.nn.functional.log_softmax(logits, dim=-1)
...
def create_sampler(backend: str | None = None) -> "Sampler":
    ...
    if backend in _CUSTOM_SAMPLER_FACTORIES:
        ...
        if not isinstance(sampler, Sampler):
            raise TypeError(f"Custom sampler factory for backend '{backend}' must return a Sampler")
        return sampler
...
def top_k_top_p_min_p_sampling_from_logits_ascend(
    ...
    if hasattr(torch_npu, "npu_top_k_top_p") and torch.all((top_ks <= 1024) & (top_ks >= 1)):
...
def sampling_from_probs_torch(
    ...
    if sampling_seed is None:
        ...
        sampled_index = multinomial_with_seed(torch.log(probs), sampling_seed, positions)
...
# --- m5 CONTROL OVERLAY: DEGRADED + RUNTIME MARKERS ---------------------------
# Deliberate regression, plus the two markers check_patch_live looks for.
# Not an optimisation.
import logging as _m5_log  # noqa: E402  (appended overlay, not a module header)

_m5_log.getLogger(__name__).warning("M5_CONTROL_IMPORT sampler degraded-rev2")
print("M5_CONTROL_IMPORT sampler degraded-rev2", flush=True)
M5_CONTROL_OVERLAY = "degraded_marked"
M5_CONTROL_SLEEP_S = 0.020
M5_CONTROL_SEEN = False
