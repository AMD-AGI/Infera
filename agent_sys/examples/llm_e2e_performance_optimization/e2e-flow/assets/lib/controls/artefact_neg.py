...
def main() -> int:
    ...
    for p in (packup, arm):
        if not p.is_dir():
            print(
                f"missing {p} — set ARMS to a directory holding e2e_packup/ and "
    ...
    ok.append(
        show(
            "the one file a reproducer executes is gone",
            "check_packup_shape",
            {"pk": b},
            PS,
            expect=False,
    ...
    ok.append(
        show("summary export fails its schema", "check_bench_report", {"sm": d}, BR, expect=False)
    )
    ...
    ok.append(
        show("a required frozen check is absent", "check_acceptance", {"sm": e}, AC, expect=False)
    )
