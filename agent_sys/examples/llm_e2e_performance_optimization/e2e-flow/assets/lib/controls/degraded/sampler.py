...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        """
        import time as _m5_time  # m5 CONTROL: deliberate cost
...
# --- m5 CONTROL OVERLAY: DEGRADED ---------------------------------------------
# A DELIBERATE regression, and the positive half of a control experiment. This
# is not an optimisation and not a mistake: the gate has never been shown to
# detect a real difference on a real deployment, and a gate proven only against
# a null sample has not been shown to detect anything at all.
M5_CONTROL_OVERLAY = "degraded"
M5_CONTROL_SLEEP_S = 0.002
