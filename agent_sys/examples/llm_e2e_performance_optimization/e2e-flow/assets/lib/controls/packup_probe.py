    ...
    for doc in yaml.safe_load_all((PKG / "steps" / "m5_integration.yaml").read_text()):
        for entry in doc if isinstance(doc, list) else [doc]:
            if (
                ...
                and entry.get("module") == "validator"
...
def run(label: str, mutate, *, stage: bool = True) -> dict:
    ...
    with tempfile.TemporaryDirectory(dir=SCRATCH) as tmp:
        ...
        r = subprocess.run(
            ["/bin/sh", str(ENTRY)], cwd=zone, env=env, capture_output=True, text=True, timeout=300
        )
...
def main() -> int:
    if not KIT.is_dir():
        print(
            f"no kit at {KIT} — set E2E_PACKUP_KIT to a handoff content directory", file=sys.stderr
        )
        return 2

    rows = [(label, expect, run(label, mut, stage=stage)) for label, mut, expect, stage in CASES]
    ...
    dead = [label for label, _, r in rows if not r["verdict_file"]]
    wrong = [label for label, e, r in rows if r["result"] is not e]
    print("\n" + "=" * 100)
    ...
    if not any(r["result"] is True for _, _, r in rows):
        print(
            "!! this battery never observed a PASS — it cannot tell a working "
