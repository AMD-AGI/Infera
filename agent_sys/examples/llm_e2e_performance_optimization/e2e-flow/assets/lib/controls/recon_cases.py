...
def main() -> int:
    ...
    for name, m2v, stv, m2bs, stbs, conc, want_ok, want_ceil in CASES:
        ...
        blocks[name] = block
        rows = "  ".join(
            f"{r['metric'][:12]:12s}{r['rel_delta']:+.3f}/{'T' if r['within_tolerance'] else 'F'}"
            for r in block["metrics"]
        )
        got_ceil = (
            gc.check(part / "env" / "engine_argv.txt", part / "profile_export_aiperf.json", "m2")[
                "ok"
            ],
            gc.check(
                st / "items" / "env" / "engine_argv.txt", r1 / "profile_export_aiperf.json", "stock"
            )["ok"],
        )
        ok = block["ok"] is want_ok and got_ceil == want_ceil
        print(
            f"{'OK ' if ok else 'BAD'} {name} recon={str(block['ok']):5s} "
            f"ceiling(m2,stock)={got_ceil}  {rows}"
        )
        if not ok:
            bad.append(
                f"{name}: recon={block['ok']} want {want_ok}; ceiling={got_ceil} want {want_ceil}"
            )
    ...
    if not (same_keys and same_verdicts and not mentions):
        bad.append(
            "case 1 and case 3 are no longer indistinguishable — if that is "
    ...
    if bad:
        ...
        return 1
    print(
        f"{len(CASES)}/{len(CASES)} as specified; case 3 passes the reconciliation and "
