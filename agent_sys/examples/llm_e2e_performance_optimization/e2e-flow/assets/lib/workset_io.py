...
_EXPANSION = re.compile(
    r"""(?:\$\{[A-Za-z_][A-Za-z0-9_]*\}|\$[A-Za-z_][A-Za-z0-9_]*|@[A-Z_][A-Z0-9_]*@)["']?"""
)
...
def assign_roles(shapes: list[dict], floor: int = PERFORMANCE_FLOOR) -> list[str]:
    ...
    timed = {
        i for i in range(len(shapes)) if shapes[i].get("is_primary") or shapes[i].get("observed")
    }
    for i in order:
        if len(timed) >= floor:
            break
        timed.add(i)
    return [
        "correctness-and-performance" if i in timed else "correctness" for i in range(len(shapes))
    ]
