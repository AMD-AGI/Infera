...
def parse(text: str) -> dict[str, dict[str, int]]:
    ...
    for row in csv.DictReader(line for line in text.splitlines() if line.strip()):
        ...
        def num(key: str, row=row) -> int:
            try:
                return int(float(row.get(key) or 0))
            except (TypeError, ValueError):
                return -1

        out[dev] = {"vram_pct": num("GPU Memory Allocated (VRAM%)"), "use_pct": num("GPU use (%)")}
    return out
...
def record(text: str, ours: set[str], step: str) -> dict:
    ...
    busy = sorted(
        c for c, v in theirs.items() if v["vram_pct"] > BUSY_PCT or v["use_pct"] > BUSY_PCT
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--append", default="", help="append the record to this jsonl instead of stdout"
    )
    a = ap.parse_args()
