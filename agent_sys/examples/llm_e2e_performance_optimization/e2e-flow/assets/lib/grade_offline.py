...
Do not read a level count out of this script.

**A frozen pre-registration snapshot must not be read as current state**, and
the shape of that mistake is worth keeping: a qualifier such as *"the
distribution before the results arrived"* sits one line ABOVE the claim people
actually quote, which is bold, self-contained, numeric and therefore portable.
Readers move the bullet and leave the header behind -- the `tail -1` shape in
prose, the same failure as a run whose cause sat directly above its last line.
Not a
    ...
    for p in glob.glob(str(run / "store" / "handoff" / "*.json")):
        ...
        for v in d.get("versions", []):
            ...
            candidates = sorted(
                (
                    p
                    for p in (run / "handoffs" / d["id"]).glob("v*/content")
...
def grade(package: Path, spec: dict, handoff: dict, variables: dict, python: str):
    ...
    # **`entry.sh`, never `check.py` directly.** `entry.sh` resolves
    ...
    with tempfile.TemporaryDirectory(prefix="grade.", dir=os.path.expanduser("~")) as zone:
        ...
        (z / "materials.json").write_text(
            json.dumps({handoff["id"]: os.path.relpath(handoff["content"], z)}, indent=2),
            encoding="utf-8",
        )
        env = dict(os.environ, AGENT_SYS_TASK_PACKAGE=str(package), AGENT_SYS_DEMO_PYTHON=python)
        proc = subprocess.run(
            ["sh", str(body)], cwd=z, env=env, capture_output=True, text=True, timeout=900
        )
        ...
        if not ok and report.is_file():
            detail += " | " + " / ".join(
                report.read_text(encoding="utf-8").strip().splitlines()[:2]
            )
...
def main() -> int:
    ...
    print(
        f"{'run':<24} {'closure':<24} {'kind':<32} {'dir':<4} {'validator':<30} {'verdict':<8} detail"
    )
    for r in a.run:
        run = Path(r).resolve()
        for h in sealed(run):
            ...
            for vname in kinds.get(h["kind"], {}).get("validators") or []:
                ...
                print(
                    f"{run.name:<24} {h['closure']:<24} {h['kind']:<32} {h['vdir']:<4} "
