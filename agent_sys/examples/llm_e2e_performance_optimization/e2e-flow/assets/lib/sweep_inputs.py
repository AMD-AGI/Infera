...
RUN_ROOTS = [Path(p) / "runs" for p in os.environ.get("E2E_RUN_ROOTS", "").split(":") if p]
...
def from_runs() -> dict[str, tuple[Path, str]]:
    ...
    **Largest wins within a run, not lowest-numbered.** Measured:
    ...
    runs = [r for root in RUN_ROOTS if root.is_dir() for r in sorted(root.iterdir(), reverse=True)]
    for run in runs:
...
def from_mock_m5(
    kinds: list[str], pool: Path, pkg: Path, envyaml: Path
    ...
    for what in wanted:
        ...
        r = subprocess.run(
            ["bash", str(pkg / "assets" / "lib" / "mock_m5.sh"), what, str(envyaml)],
            env=env,
            capture_output=True,
            text=True,
        )
        for k in produced:
            ...
            if r.returncode == 0 and _files(d):
                ...
                tail = [
                    line
                    for line in (r.stderr or r.stdout or "").strip().splitlines()
                    if line.strip()
                ]
                sys.stderr.write(
                    f"sweep_inputs: mock_m5.sh {what} did not produce {k}: "
...
def resolve(kinds: list[str], pkg: Path, pool: Path) -> dict[str, tuple[Path, str] | None]:
    ...
    if todo:
        if envyaml and envyaml.is_file():
            ...
            sys.stderr.write(
                "sweep_inputs: no deploy_kit carrying items/codes/environment.yaml, "
...
def main() -> int:
    ...
    for k in kinds:
        ...
        print(
            f"  {k:36s} {'MISSING' if v is None else v[1]:44s} "
