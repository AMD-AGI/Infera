...
def _classify_frame(path: str) -> tuple[str, str, str, str, str]:
    ...
    if path.startswith("/"):
        for owner, root, placeholder in sorted(CONTAINER_ROOTS, key=lambda row: -len(row[1])):
            ...
            if path == stem or path.startswith(stem + "/"):
                return owner, f"${{{placeholder}}}", path[len(stem) + 1 :], FORM_ABSOLUTE, ""
    ...
    placeholder = next(
        (f"${{{row[2]}}}" for row in CONTAINER_ROOTS if row[0] == owner),
        "",
    )
...
def _innermost_user_frame(frames: list[tuple[float, float, str]]) -> tuple[str, int, str] | None:
    ...
    """
    for _start, _duration, name in sorted(
        frames, key=lambda item: (item[0], -item[1]), reverse=True
    ):
    ...
    for trace in traces[: max(1, int(max_files))]:
        ...
        for kernel, picked in probes.items():
            ...
            if len(ranked) > 1 and ranked[1][1] == count:
                _note(
                    f"{kernel[:60]}: probes disagree ({count} vs {ranked[1][1]}); left unresolved"
                )
                continue
...
def _wanted_from_csv(path: Path, top_n: int) -> list[str]:
    ...
    with path.open(encoding="utf-8", newline="") as handle:
        ...
        if not reader.fieldnames or "Name" not in reader.fieldnames:
            raise SystemExit(
                f"launchers: {path} has no 'Name' column (header: {reader.fieldnames})"
            )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--top-n", type=int, default=50, help="kernels to resolve, by self CUDA time"
    )
    ...
    for row in unmapped:
        print(
            f"  (unpublished) {row['file_name']}:{row['line']} {row['function']}: {row['reason']}"
        )
    for note in notes:
        print(f"  (note) {note}")
    return 0
