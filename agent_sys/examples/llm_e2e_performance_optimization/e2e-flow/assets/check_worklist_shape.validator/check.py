...
def _check(content: Path, args: dict, problems: list[str]) -> bool:
    ...
    if not carried.is_file():
        problems.append(
            "items/schema is absent; a structured_text handoff carries its own schema (CONTRACT 3.4)"
        )
    else:
    ...
    for bucket, declared in buckets.items():
        ...
        if declared["kernels"] != actual:
            problems.append(
                f"buckets.{bucket}.kernels is {declared['kernels']}, the list holds {actual}"
            )
    ...
    if len(set(ids)) != len(ids):
        ...
        problems.append(
            f"duplicate kernel_id(s): {duplicated}. kernel_id is the join key every later artefact uses"
        )
    ...
    if ranks != list(range(1, len(selected) + 1)):
        problems.append(
            f"the selected kernels rank {ranks}, expected 1..{len(selected)} with no gap or repeat"
        )
...
def _check_csv(content: Path, kernels: list[dict], problems: list[str]) -> None:
    ...
    if len(rows) != len(kernels):
        problems.append(
            f"items/worklist.csv has {len(rows)} row(s), items/text.json has {len(kernels)} kernel(s)"
        )
    ...
    for row in rows:
        ...
        if kernel is None:
            problems.append(
                f"items/worklist.csv row {row['kernel_id']!r} is not in items/text.json"
            )
            continue
