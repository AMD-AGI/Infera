...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"the staged kernel_table is not readable: {error}") from error
    ...
    print(
        f"rank: {len(rows)} kernels, buckets {dict((k, v['kernels']) for k, v in buckets.items())}"
    )
    print(
