...
def stock_vs_m2_block(stock: Path, evidence: str | None, tolerance: float) -> dict:
    ...
    **The blocker of the two new blocks**, because everything downstream assumes
    ...
    if not r1.is_file():
        return {
            "source": str(found[0].relative_to(root)),
            "tolerance": None,
            "metrics": [],
            "ok": None,
    ...
    for metric, column in (
        ...
        rel = (
            (b - a) / a
            if isinstance(a, (int, float)) and a and isinstance(b, (int, float))
            else None
        )
...
def kernel_reconciliation_block(
    evidence: str | None, kopt: str | None, perf_rows: list[dict], operator_id: str | None
) -> dict:
    ...
    **Warning, never a blocker**, exactly as the mission puts it: 作为
    ...
    return {
        "kernel_speedup": round(float(speedup), 6),
        ...
        "unavailable_because": None,
        "note": (
            "the share is measured with the profiler attached and CUDA graph off and "
...
def main() -> int:
    ...
    ap.add_argument(
        "--kernel-optimization", default=None, help="m4's, for the single-kernel speedup (M5.1.3.2)"
    )
    ...
    for name in sorted(set(loaded["stock"]["evals"]) | set(loaded["patched"]["evals"])):
        ...
        if not a or not b or a["score"] is None or b["score"] is None:
            eval_rows.append({"name": name, "verdict": "unmeasured", "stock": a, "patched": b})
    ...
    for round_name in rounds:
        summaries = {
            arm: (read_json(loaded[arm]["bench"] / round_name / "summary.json", {}) or {}).get(
                "metrics", {}
            )
            for arm in ("stock", "patched")
        }
        for metric, (label, higher_better) in PERF_METRICS.items():
            for column in PERF_COLUMNS.get(metric, ("avg",)):
                ...
                bar = (
                    args.max_throughput_regression if higher_better else args.max_latency_regression
                )
                row = eval_stats.perf_verdict(
                    a, b, max_regression=bar, higher_is_better=higher_better
                )
    ...
    for metric in PERF_METRICS:
        for column in PERF_COLUMNS.get(metric, ("avg",)):
            ...
            for arm in ("stock", "patched"):
                ...
                for round_name in rounds:
                    summary = (
                        read_json(loaded[arm]["bench"] / round_name / "summary.json", {}) or {}
                    ).get("metrics", {})
    ...
    for metric, (label, higher_better) in PERF_METRICS.items():
        for column in PERF_COLUMNS.get(metric, ("avg",)):
            ...
            counts = {
                arm: [
                    r["stock" if arm == "stock" else "patched"]
                    for r in perf_rows
            ...
            by_median = eval_stats.perf_verdict(
                med_a, med_b, max_regression=bar, higher_is_better=higher_better
            )
            row.update(
                metric=metric,
                column=column,
                label=label,
                bar=bar,
                reduction="pooled_mean",
                rounds=detail_a["n"],
                pooled_n_stock=n_a,
                pooled_n_patched=n_b,
                median_stock=med_a,
                median_patched=med_b,
                median_verdict=by_median["verdict"],
                stock_detail=detail_a,
                patched_detail=detail_b,
            )
            ...
            floors = {
                arm: eval_stats.noise_floor(
                    disp.get(f"{arm}_rsd"), pooled_n[arm] or disp.get(f"{arm}_n")
                )
                for arm in ("stock", "patched")
            }
            ...
            row.update(
                noise_floor=None if floor is None else round(floor, 6),
                stock_rsd=disp.get("stock_rsd"),
                patched_rsd=disp.get("patched_rsd"),
                n_stock=disp.get("stock_n"),
                n_patched=disp.get("patched_n"),
            )
    ...
    for arm, r in ceilings.items():
        if r.get("ok") is False:
            ...
            print(
                f"compare: NOTE graph ceiling unchecked for the {arm} arm — "
                f"{r.get('unavailable_because')}",
                file=sys.stderr,
            )
    ...
    reconciliation = kernel_reconciliation_block(
        args.profiling_evidence,
        args.kernel_optimization,
        perf_rows,
    ...
    if reconciliation.get("unavailable_because"):
        warnings.append(
            f"kernel reconciliation not computed: {reconciliation['unavailable_because']}"
        )
    ...
    if args.environment:
        ...
        print(
            ...
            "(see this task's readme, STEP 10).",
            file=sys.stderr,
        )
    ...
    for row in smoke_rows:
        lines.append(
            f"| smoke: {row['check']} | {row['stock']} | {row['patched']} | {row['verdict']} |"
        )
    ...
    lines += [
        "",
        "| eval | scored | stock | patched | delta | 95% CI of delta | verdict |",
        "|---|---|---|---|---|---|---|",
    ]
