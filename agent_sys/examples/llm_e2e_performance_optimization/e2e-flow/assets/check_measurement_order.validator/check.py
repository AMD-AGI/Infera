...
def spans(record: dict, reasons: list) -> list[tuple[str, dt.datetime, dt.datetime]]:
    ...
    for i, step in enumerate(record.get("steps") or []):
        ...
        if started is None:
            reasons.append(
                f"step {name!r} has no readable `started` timestamp ({step.get('started')!r})"
            )
            continue
...
def aiperf_windows(
    content: Path, reasons: list, arm: str
    ...
    **The corroborating record, and the reason it corroborates: AIPerf wrote it,
    ...
    for export in sorted(content.glob("items/result/r*/profile_export_aiperf.json")):
        ...
        try:
            ...
            reasons.append(
                f"{arm}: {export.parent.name}/profile_export_aiperf.json is unreadable: {exc}"
            )
            continue
...
def corroborate(
    arm: str,
    timeline: list[tuple[str, dt.datetime, dt.datetime]],
    ...
    **Arm against its own record is conditional on `runtime.replayed_from`.**
    ...
    for arm in ARMS:
        ...
        if declared.get("replayed_from"):
            print(
                f"  {arm}: record is a replay of {declared['replayed_from']}; "
    ...
    if len(seen) == len(ARMS):
        ...
        if ok:
            print(
                f"  both arms on {seen[first].get('node')} (jobid {seen[first].get('slurm_jobid')})"
            )
    return ok


def check_pair(
    records: dict[str, dict], contents: dict[str, Path], args: dict, reasons: list
) -> bool:
    ...
    if args.get("require_arms_disjoint_in_time", True):
        ...
        if patched_start < stock_end:
            ...
            print(
                f"  arms are disjoint; the patched arm began {gap:.0f}s after the stock arm ended"
            )
...
def main() -> int:
    ...
    for hid in ids:
        ...
        if arm not in ARMS:
            problems.append(
                f"{hid}: steps.json declares arm={arm!r}, which is not one of {list(ARMS)}"
            )
            continue
        if arm in records:
    ...
    findings = {
        hid: ([] if verdict else list(reasons), notes + (list(reasons) if verdict else []))
        for hid in ids
    }
