...
def _resolved(operator: dict) -> bool:
    ...
    return (
        bool(operator.get("source_file_path"))
        and operator.get("source_resolution_method") in _RESOLVING_METHODS
    )
...
def _check(content: Path, args: dict, problems: list[str], notes: list[str]) -> bool:
    ...
    if summary["operators"] != len(operators):
        problems.append(
            f"summary.operators is {summary['operators']}, the list holds {len(operators)}"
        )
