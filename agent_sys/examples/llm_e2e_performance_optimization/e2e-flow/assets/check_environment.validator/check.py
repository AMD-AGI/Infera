...
_PKG = pathlib.Path(
    os.environ.get("AGENT_SYS_TASK_PACKAGE") or os.environ.get("AGENT_SYS_DEMO_PACKAGE", "")
)
    ...
    for rel in CANDIDATES:
        if "*" in rel:
            ...
            if len(found) > 1:
                return (
                    None,
                    f"more than one environment.yaml under items/codes/*: {[str(p) for p in found]}",
                )
        else:
            path = content / rel
            if path.is_file():
...
def main() -> int:
    ...
    for hid in zone.inputs():
        ...
        if container:
            findings.append(
                f"{hid}: runtime.container={container} (reported, not judged — see module 5's two arms)"
            )
