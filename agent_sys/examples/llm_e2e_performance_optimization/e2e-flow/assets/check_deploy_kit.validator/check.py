...
def check_entry(entry: dict, roots: dict[str, Path], layout: dict) -> list[str]:
    ...
    if floor is not None:
        ...
        if len(commands) < floor:
            faults.append(f"{where}: {len(commands)} command lines in code blocks, needs {floor}")
    ...
    if rendering:
        ...
        try:
            ...
            faults.append(
                f"{where}: cannot read {rendering['document']} to check it renders it: {exc}"
            )
        else:
...
def check_shared_identifiers(rule: dict, roots: dict[str, Path]) -> list[str]:
    ...
    flag_use = re.compile(
        r"(?<![\w-])("
        + "|".join(re.escape(f) for f in sorted(binding, key=len, reverse=True))
        + r")[=\s]+(\S+)"
    )
...
def check_runtime_contract(contract: dict, scan_rule: dict, roots: dict[str, Path]) -> list[str]:
    ...
    for parameter in contract.get("parameters") or []:
        ...
        faults.append(
            f"{scan_rule['dir']}/: nothing reads ${{{parameter['name']}:=…}}. {' '.join(why.split())}"
        )
