...
def _mentioned(needles: list[list[str]], stated: list[str]) -> bool:
    ...
    **Not a prefix match on prose, and that is a fix rather than a preference.**
    ...
    return all(
        any(alt.lower() in text for alt in group for text in lowered) for group in needles if group
    )
...
def recompute(report: dict, args: dict, reasons: list) -> list[str]:
    ...
    for row in report.get("correctness", {}).get("evals", []):
        ...
        if row.get("verdict") == "uninterpretable":
            found.append(
                f"eval {row.get('name')!r} is uninterpretable: {row.get('uninterpretable_because')}"
            )
            continue
    ...
    throughput_bar = float(
        args.get("max_throughput_regression", bars.get("max_throughput_regression", 0.05))
    )
    ...
    found.extend(
        judge_comparison(
            report, args, throughput_bar, latency_bar, higher_better, per_round_breaches
        )
    )
...
def judge_comparison(
    ...
    **The reduction is recomputed here from `performance`, not read.** Same rule
    ...
    for row in report.get("performance", []):
        ...
        slot = by_key.setdefault(key, {"stock": [], "patched": [], "n_stock": [], "n_patched": []})
    ...
    for row in rows:
        ...
        floors = [
            eval_stats.noise_floor(row.get(f"{arm}_rsd"), pooled_n[arm] or row.get(f"n_{arm}"))
        ...
        by_median = eval_stats.perf_verdict(
            med_a, med_b, max_regression=bar, higher_is_better=up_is_good
        )
        disagrees = (
            again["verdict"] != "unmeasured"
            and by_median["verdict"] != "unmeasured"
            and by_median["verdict"] != again["verdict"]
        )
        floor_exceeds = floor is not None and floor > bar and again["verdict"] != "unmeasured"
    ...
    for key in by_key:
        if key not in judged:
            found.append(
                f"{key[0]} ({key[1]}) has per-round rows and no comparison row, "
                "so nothing judged it"
            )

    if per_round_breaches and not found:
        print(
            "  individual rounds breached and the reduction did not: "
            + ", ".join(per_round_breaches)
        )
        print(
            "    Reported, not judged. If this is common, the rounds disagree more than "
...
def unlisted_findings(report: dict, args: dict) -> list[str]:
    ...
    for row in report.get("comparison", []):
        ...
        if not _mentioned(groups, stated):
            missing.append(
                f"{metric} ({row.get('column')}) regressed and the report's reasons do not name it"
            )
    ...
    for row in report.get("correctness", {}).get("evals", []):
        ...
        if again["verdict"] == "REGRESSED" and not _mentioned(
            [[str(row.get("name") or "")]], stated
        ):
            missing.append(
                f"eval {row.get('name')!r} regressed and the report's reasons do not name it"
            )
    return missing
...
def reconcile_with_kernel(report: dict, args: dict) -> list[str]:
    ...
    **Where the hard floor actually lives.** The mission's floor — the patched arm
    ...
    if k is None or s is None:
        return [
            "kernel reconciliation carries no kernel_speedup / kernel_share_of_profile "
    ...
    print(
        f"  reconciliation: kernel {k:.3f}x at {s:.2%} of the profile predicts {amdahl:.4f}x "
        f"end to end; observed {f'{float(observed):.4f}x' if observed is not None else 'not recorded'}"
    )
    if observed is None:
...
def check(content: Path, args: dict, reasons: list) -> bool:
    ...
    if args.get("reconcile_with_kernel_speedup") == "warn":
        ...
        for warning in warnings:
            if not any(warning[:40] in str(s) for s in stated_warnings):
                print("    (the report does not carry this warning)")
    ...
    print(
        f"  recomputed: {'accepted' if not recomputed else 'REJECTED'} "
...
def main() -> int:
    ...
    for hid in zone.inputs():
        ...
        if content is None:
            ...
            findings[hid] = (
                [] if results[hid] else list(reasons),
...
def _report(findings: dict, results: dict) -> None:
    ...
    try:
        ...
        print(f"check_no_regression: could not write the validator report: {exc}", file=sys.stderr)
