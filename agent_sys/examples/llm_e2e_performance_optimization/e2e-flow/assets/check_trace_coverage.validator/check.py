...
def check(content: Path, args: dict, reasons: list) -> bool:
    ...
    on_disk = (
        {p.name: p.stat().st_size for p in traces.glob("*.trace.json.gz")}
        if traces.is_dir()
        else {}
    )
    if len(on_disk) != len(ranks):
        ok = _fail(
            reasons, f"{len(on_disk)} trace file(s) on disk against {len(ranks)} in the manifest"
        )
    for row in ranks:
        name = row.get("file")
...
def _check_stack_window(content: Path, args: dict, reasons: list) -> bool:
    ...
    for row in ranks:
        if row.get("file") not in on_disk:
            ok = _fail(
                reasons, f"stack trace {row.get('file')!r} is in the manifest and not on disk"
            )
