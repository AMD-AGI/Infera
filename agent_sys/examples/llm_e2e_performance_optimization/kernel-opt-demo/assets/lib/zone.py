...
def find_one_dir(content: Path) -> tuple[Path | None, str]:
    """The one directory inside a `code`-typed handoff's `items/codes/`.

    The same rule as `find_packup` and for the same reasons, minus the naming
    convention: a **workset** directory is named after its operator, not
    `<name>.packup_<YYYYMMDD>`, so the date-anchored pattern would reject every
    valid one. Zero and two are both real failures — zero means the producer
    wrote its directory somewhere the content type does not put it, and two
    means a consumer has to guess which one to optimize.
    """
    codes = content / "items" / "codes"
    if not codes.is_dir():
        return None, "no items/codes directory"
    found = sorted(e for e in codes.iterdir() if e.is_dir())
    if not found:
        loose = sorted(e.name for e in codes.iterdir())
        return None, f"no directory under items/codes (found: {loose})"
    if len(found) > 1:
        return None, f"more than one directory under items/codes: {[e.name for e in found]}"
    return found[0], found[0].name
