...
|---|---|---|
| 1 | `python3 -m cli.main show --package examples/env_checker --var nonce=x --var uv_root=$RUN_ROOT/x` | `2 tasks in the graph`, and both `output validation runs 2` |
...
| 4 | `python3 -m cli.main run --package examples/env_checker --demo-root $RUN_ROOT/dryrun --var nonce=x --var uv_root=$RUN_ROOT/x --dry-run` | completes, dispatches nothing, no `REJECTED` |
