# `ok.two_ai_tasks.3` — two AI tasks and one handoff
...
agent-sys run --package agent_sys/examples/ok.two_ai_tasks.3
```
...
`../ok.filetree_grounded_report.4/` is a chain of three and `../ok.algorithms_solve_grade.14/` is a seven-node graph with
...
`../ok.algorithms_solve_grade.14/shared.yaml` exists for has no user in this package.
