...
Write exactly **`$TWO_TASK_N_PROBLEMS`** problems. That is an environment variable
...
input and writes standard output.** A solver of this set compiles one
self-contained program per problem and runs it against your worked examples, so
there is no function signature, no test harness with a class to fill in, and no
framework — a program, a stream in, a stream out.
...
| key | what it is |
|---|---|
| `id` | `p01`, `p02`, … Unique across the set, and **filesystem-safe**: lower-case letters, digits and underscores only. A consumer of this set uses it as a directory name, one per problem, so a space or a slash in it is a broken path rather than an untidy label |
