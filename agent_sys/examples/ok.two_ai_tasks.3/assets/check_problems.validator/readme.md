...
2. `store.latest_of_kind("directions")` — *the newest `directions` anywhere in
   ...
   wherever the producer's row is out of scope.

In this package nothing consumes `problems`, so only the output phase runs and
route 1 is the one that fires. Route 2 is kept because the file is shared with
`examples/ok.algorithms_solve_grade.14/`, where a consumer does exist.

If neither resolves, **every handoff is False**. A check that could not run has
not found nothing. Same shape, same reason, as
`examples/ok.filetree_grounded_report.4/assets/check_grounded.validator/check.py`'s fallback.
...
would read exactly like a setter who invented every one of them.
...
It runs **once per graph**, in `problems`' output phase, because nothing here
consumes `problems`. `expected_count` still comes through `args.json` rather
than the `setter` agent's `env`: `args.json` does not vary by phase, so the same
spelling keeps working when a consumer is added, and its neighbour
`check_directions` — which does run in an input phase — has no other route.
