...
def package_root() -> Path:
    ...
    a validator body's environment depends on the phase that ran it. Here only
    the producer's output phase runs this validator; the file is shared with
    packages where an input phase does too."""
...
def catalog_slugs(relative: str) -> set | None:
    ...
    exactly like a setter who invented every one of them."""
...
def direction_ids(kind: str) -> set | None:
    ...
    **Two routes, and the first is the exact one.** `declared_dir` reads
    ...
    row on the OUTPUT phase only (`validator/environment.py:116-142`). In any
    input phase it is simply absent.
    ...
    fallback in `examples/ok.filetree_grounded_report.4`, for the same reason.
...
def main() -> int:
    ...
    # misspelled knob rather than quietly meaning the fallback.
