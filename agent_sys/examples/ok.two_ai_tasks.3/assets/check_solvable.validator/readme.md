...
`examples/ok.filetree_grounded_report.4/steps/describe.yaml` makes the argument this file is obeying: a
...
no store. It therefore behaves identically in every phase, which is not true of
its neighbour `check_problems`. Here only `problems`' output phase runs it.
