...
**All three must be registered somewhere, and the registration is the module's
own.** `env_mgr` design §11.4 and `validator` design §5.5 do it, and
...
| `test_a_task_the_scheduler_watched_can_report`, `test_the_runner_does_not_call_set_task` | Every planned advance raising `ScopeViolation`, so no task advances a phase — invisible to a unit suite whose stub `set_task` is a no-op. The call is `Scheduler._dispatch_pass`'s; these assert the fixture models it and that building it here again is a red test |
