...
Design: `../docs/superpowers/specs/2026-09-03-agentsview-o11y-design.md`.
...
like `~/.local`. Nothing is installed anywhere else:
...
| `AGENTSVIEW_DATA_DIR` | `<root>/state/agentsview` | `sessions.db`, our `config.toml`, `serve.log` |
| `AGENT_SYS_RUN` | `<root>/run` | `agentsview.port`, the record of which port *we* started a daemon on |
...
`AGENT_SYS_BIN` is not added to a child's `PATH` by default. `PATH` here is
...
| | |
|---|---|
| `--agentsview-port N` | the port. Resolution order: this flag, then `AGENTSVIEW_PORT`, then `18888` |
...
| case | warning says |
|---|---|
| the port is already in use by something else | which port, and to pass `--agentsview-port` |
...
terminal inherits nothing from us and keeps using `~/.claude` exactly as before.
Measured, not asserted: a real redirected turn left `~/.claude/projects` at 801
files with an empty `diff`, and `test_agent_environment_does_not_touch_this_process`
...
`$AGENT_SYS_RUN/agentsview.port` must name that port. The second gate is why a
genuine AgentsView you started yourself is refused rather than reused — it has
its own `AGENTSVIEW_DATA_DIR` and lists every session on the machine, and
showing you that would break the one property the panel exists for. If you were
expecting reuse and got the "port in use" warning instead, this is why.
...
tracking "latest" — two machines running different panels with no record of
which is not an observability story. Upstream's `install.sh` is *not* used: it
hardcodes `/usr/local/bin`, falling back to `~/.local/bin` and then to `sudo`,
with no override point anywhere in the script. Nothing of ours is written to
either path.
...
### Known gap: a second writer of `CLAUDE_CONFIG_DIR` wins

**For any task with an agent spec — which is every AI task — the transcripts do
not currently land in the prefix, and the panel will therefore be empty.**
`prepare` sets `CLAUDE_CONFIG_DIR` to `$AGENT_SYS_CLAUDE_HOME` at step 6
(`prepare.py:480`), and then `material.deploy` overwrites it at step 6b
(`prepare.py:533`, `material.py:89`) with a per-attempt `config/` directory
inside the zone. That relocation predates this component and has its own good
reason — *a run whose transcript changes with the reviewer's dotfiles is not
reproducible* — so the two are not simply reconcilable by deleting one.

Neither writer points at `~/.claude`, so the promise above is unaffected either
way; what is affected is only whether the panel has anything to show. Recorded
here rather than left for a reader to discover from an empty session list.
