...
of the call, or the recursion is explicitly bounded — the design picks one,
...
**The cascade raised on its first entry.** Found in the cross-module consistency
...
§4.1 defers it to *nobody* — the phrase appears at `closure/docs/spec.md`
...
recursion is explicitly bounded — **the design picks one, and the choice is
not optional.**"
