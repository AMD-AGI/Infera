...
|---|---|---|---|---|
| prefill | 14320 | `crsuse2-m2m-074` | 10.245.154.156 | 30000 |
| decode | 14321 | `crsuse2-m2m-072` | 10.245.144.119 | 30001 |

Router ran on the **prefill** node, port **8130**, prometheus **29130**.
...
prefill node of the Exp-1 run on the same day and cluster — **not** on this arm's own
nodes. All nodes in the 2026-07-30 runs were allocated from the same spur partition with
the same `-G8` request, are the same SKU, and passed the same GPU health gate, but the
CPU/RAM/firmware figures here are inherited rather than directly measured. Re-run
`collect_env.sh` on this arm's nodes if an exact per-node record matters.
